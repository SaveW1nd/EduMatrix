...
- Given `TranscodeComplete(fail)` 事件被消费任务处理完毕，When 上传人进入管理端，Then 收到站内提醒，媒资列表该条 status=3 并提供重试入口；引用它的课时无法置为可见。
...
6. **推进一致性校验**：**仅在连续两次 `seeked=false` 的心跳之间**校验——有效心跳的位置推进量须与墙钟一致；长期零推进而间隔正常（期间无任何 `seeked=true`）判定为挂机或纯脚本上报，只留痕不计时长并累加风控分。不设此条时，脚本无需下载任何视频分片、仅按 10s 节奏发包即可 1:1 刷满时长。
   ...
   **`seeked` 自身必须限频**：连续 `seeked=true` 与单课时每小时 `seeked=true` 均有次数上限，超出按规则 5 丢弃并计风控分。否则脚本每次都置 `true` 即可永久关闭本条校验，挂机成本重新归零。
7. **并发课时数上限**：单学生同一时刻的活跃课时数有上限，超出的课时心跳直接丢弃并计风控分。此前的并发防护只作用于"同一课时"，跨课时并行不受限是防刷模型的最大缺口——多个课时并行的脚本一晚可刷完整门课程。
8. **时长封顶**：单学生单日累计与单课时累计均有上限，超出部分只记录、不计入完播判定，并触发风控标记供机构核查。
...
- Given 脚本连续 3 次以 `seeked=true` 上报，When 服务端接收第 3 次，Then 返回 code=20002 丢弃并计风控分（连续 `seeked=true` 上限 2 次）。
