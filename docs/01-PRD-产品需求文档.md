...
EduMatrix 是面向教育机构、学校及企业培训部门（ToB）的**私域督学管理系统**。区别于面向 C 端的公开课平台，本产品的核心是"一对一/一对多的私域监管式学习"：机构买单、导师执行、学生被监管、**学生之间互不可见**。平台通过~~禁快进~~（**F-113 取消**）、~~跑马灯水印~~（**F-114：第一版不做**）、心跳进度追踪、作业自动判卷与数据看板，把"学生是否真的学了、学到什么程度"变成机构与导师可量化、可追责的数据。

**组织形态：统一组织树。** 系统中的每一个人——平台超管、机构各级管理员、教师、学生——都是**同一棵树上的节点**，一棵树到底。**不设不绑账号的组织单元节点**：要表达"华东校区"这一层，就建一个管理员节点、节点名叫"华东校区"、`ref_user_id` 指向该片区负责人。也没有独立的教学分组单元：学生直接挂在导师（教师节点）下，教师挂在管理员节点下。由此得到两条极简规则：
...
1. **看得住**：心跳进度追踪 + **全量留痕**（`vod_heartbeat_log` 按月分区留 6 个月）。~~跑马灯水印~~ **F-114：第一版不做**。
   > **⚠ 第一版「看得住」只剩留痕**：不防刷课（F-113）、不加密、不水印（F-114）。
   > 这不是描述失准，是需方逐条定下来的取舍 —— 写清楚是为了**不让人以为还有别的防线**。
   > **⚠ F-113 定案：不防刷课**（需方原话「刷课不需要防」）。**原文那句「使刷课不快于真实播放」已删除，不是漏写。**
   > 原文列举的三道防线——禁快进、时长封顶、倍速不加速计时——**处置各不相同**：
   > **禁快进已废止**（F-113）；**时长封顶已废止**（F-117，需方 2026-08-22）；**倍速不加速计时保留且比以前更要紧**（计时挪到前端后，它是错误实现方式的唯一约束）。
   > **推进一致性校验（规则 6）保留，但降级为只留痕、不影响时长。**
   > 这些机制留下来是为了**留痕与诊断**，**不再是为了防刷**——理由写实，免得下一个人照着一条已经不成立的理由做决定。
...
7. **算得清**：~~完播率~~（**F-113：第一版不可用**，见 F2-9）、按时提交率、高频错题榜、学习档案与 Excel 报表，按**导师/节点**两维度上卷，教学效果与团队业绩数字化闭环。
   > **第一版可用的学习类指标只有「累计观看时长」**（`vod_watch_progress.watched_duration`）与「看到多深」（`max_position`）。完播率相关的一切第一版恒为 0。
...
| 前端视频播放与进度上报 | **Aliplayer**（~~DPlayer~~，随加密路线改为阿里云私有加密，见契约 §1 订正段与 04 §E F-112） | 播放器控制、~~禁用拖拽/快进控制~~（**F-113 取消**）、**前端计时器（墙钟）**、播放进度事件监听与定时心跳触发 | 模块 2（前端） |
| 音视频云端存储与分发 | 阿里云 VOD | 云端自动化转码 HLS（`.m3u8`）、鉴权 URL 生成、流媒体 CDN 加速 | 模块 2（云端） |

### 2.5 成功指标

**North Star（北极星指标）**：**周活跃学习学生的"名下学员平均完播率"** —— 它同时反映了机构采买价值（学生真的在学）与产品管控能力（数据真实）。口径分母由导师名下在读学员数给出。

> **⚠ F-113：第一版没有北极星指标。** 完播判定延后（F2-9），`avg_complete_rate` 恒为 0。
> **这一条写明是因为它不能悬着**——一个恒为 0 的北极星比没有北极星更坏：它看上去有数据。
> 第一版可用的替代观测是**累计观看时长**，但它**不是**北极星的降级版，两者口径不同，不要混用。

| 指标 | 定义 | 目标（上线后 6 个月） |
| --- | --- | --- |
| **名下学员平均完播率** | **先汇总分子分母、再相除**（课时数加权）：`Σ名下学员已完成视频课时数 ÷ Σ名下学员应学视频课时数 × 100`，落 `stat_teacher_daily.avg_complete_rate`；节点维度同法上卷至 `stat_node_daily`（同样是课时数加权，不是学员数加权） | ≥ 75%（口径变更后需重新校准，见 F4-1） |
| 作业按时提交率 | deadline 前提交的答卷数 / 应交答卷数（应交 = 命中 `hw_homework_target` 的在读学生数） | ≥ 85% |
| 心跳数据有效率 | 通过服务端合理性校验的**周期心跳**数 / **周期心跳**总数（**F-116 收窄口径**：只统计 10 秒一次的周期心跳，**不含事件心跳**）| ≥ 97%（低于此值说明网络异常显著） |
...
系统只有 **4 个固定角色**。管理员**分层但角色相同**——机构管理员与其下级管理员共用 `org_admin`，差别仅在树上的位置（子树范围不同）；**不存在"节点管理员""校区管理员"等派生角色**，新增管理层级只需在树上加一个 node_type=1 的管理员节点（节点名即该层级的名称）。

| 角色 | role_key | user_type | 绑定节点类型 | 定位 | 数据范围（唯一规则） |
| --- | --- | --- | --- | --- | --- |
| 平台超管（运营方） | `super_admin` | 0 | 平台（树根） | 租户开通、续期、停用、平台配置 | 跨租户（平台级），不进业务页面 |
| 管理员（各级） | `org_admin` | 1 | 管理员节点（机构根节点即机构最高管理员） | 建下级管理员、建教师、建学生、分配学员、资源下发、全模块管理 | **其所在节点的子树** |
...
| 题库**分类树**维护（增/改/删） | — | ✔（分类树全租户共享，故仅管理员可改） | — | — |
| 题库**题目**管理 | — | ✔（本节点拥有 + 被授权可见的题目） | ✔（同左，范围随授权收缩） | — |
...
1. **只有这一条数据权限规则**，所有角色适用，不存在第二套过滤逻辑；标签、角色、资源授权**都不得扩大可见范围**（契约 §9.2 铁律 2）。
...
```mermaid
flowchart TD
    A[平台超管开通机构\nsys_tenant + 机构最高管理员节点\norg_node node_type=1 + sys_user] --> B[机构管理员登录\n初始化组织树]
    B --> C[按需建下级管理员节点\nnode_type=1 + sys_user，可无限嵌套\n节点名即组织层级名]
    C --> D[建教师节点\nnode_type=2 + sys_user + org_teacher]
    D --> E[建学生 / Excel 批量导入\n默认挂创建者节点下=未分配导师]
...
**功能描述**：平台超管在平台端为付费客户开通机构。机构即租户（`sys_tenant`），记录名称、联系人（contact_name/contact_phone）、有效期（expire_time）、状态（status：0正常 1停用）、学生规模上限（max_student_count）。开通时同步创建**机构最高管理员**——一个 `org_node`（node_type=1，parent_id=0，`ref_user_id` 指向其账号）加一个 `sys_user`（user_type=1，`node_id` 指向该节点），二者互为反向引用；该节点即机构根节点，回填至 `sys_tenant.root_node_id`。
...
5. 机构在读学生数（`org_student.status=0`）达到 max_student_count 时，新增/导入学生被拒绝（**10207**）。
...
- Given 平台超管创建机构成功，When 查看该机构组织树，Then 存在且仅存在一个 node_type=1、parent_id=0 的机构根节点，`sys_tenant.root_node_id` 指向它，该节点的 `ref_user_id` 与初始管理员 `sys_user.node_id` 互为反向引用。
...
### F1-2 组织树节点管理（改名 / 移动 / 停用 / 删除）

**功能描述**：管理员在**其子树范围内**维护组织树 `org_node`：重命名、调整同级排序、**移动节点**、停用、删除。**树上每个节点都是一个人**，共四类，承载关系是硬约束：
...
> **不设"校区/年级/部门"这类空节点**：`node_type` 与 `sys_user.user_type` 取值完全一致，`ref_user_id` 全部非空。要表达"华东校区"这一层，就建一个管理员节点、把节点名叫作"华东校区"、`ref_user_id` 指向该片区负责人——**这个节点既是组织层级，也是这一层的责任人**。代价是每一层组织都必须有一个具体负责人的账号；负责人更替时改 `ref_user_id` 与节点名，子树不动。
>
> 由此**没有独立的"新建节点"功能**：建节点 = 建人，一律走 **F1-3 人员管理**（建下级管理员 / 教师 / 学生），账号与档案同事务落地。

**业务规则**：

1. **父子类型硬校验**：违反上表即拒绝——把管理员/教师挂到教师节点下返回 10105，在学生节点下建任何节点返回 10106，其余非法组合返回 10104。教师尚无学员时其自身即为叶子，合法。
...
8. **树深度**上限 4 级**（F-114 由 50 收到 4，见下方注）。建节点与移动节点时**必须在服务层校验深度**，超限返回 `400`——不校验的话第 51 级会在写入时 `Data too long` 让整个移动事务回滚，而移动节点是要连带重算整棵子树 `ancestors` 的复合事务，失败点难以定位。** `ancestors` 冗余祖级路径（逗号串，如 `0,100,101,205`），子树查询用 `FIND_IN_SET(#{nodeId}, ancestors)` 单条件命中，不走递归 CTE。

   > **F-114 定案二（需方 2026-08-22）：机构下只允许一层管理员，树高最多 5 层**（`L0 平台根 → L1 机构根 → L2 普通管理员 → L3 教师 → L4 学生`，不含平台根即深度 4）。50 是 `ancestors VARCHAR(1000)` 的**物理**上界，物理上界没变；4 是业务上界。**两条改动缺一不可**：光有深度上限的话管理员仍可嵌套；光有一层管理员的话教师/学生仍可被挪到更深处。需方理由与推论（管理员节点等于不可移动）见 03-02 §2.2。
9. **停用**（status=1）**按节点类型区分**：
   ...
   - **学生节点**：仅本人
   实现走"登录时两段校验"（契约 §2.3）：①**本人所在节点** `status=1` 即拒登（教师/学生的"仅本人"停用由此生效）；②拆 `ancestors` 主键 IN 查询，命中 `node_type=1 且 status=1` 的祖先即拒登（管理员的分支冻结由此生效）。两段任一命中返回 **10017**。**只写 `org_node.status` 一行，不碰 `sys_user.status`**——后者是仅超管可写的账号级封禁，若停用时顺手也置 1，启用时无人复位（启用只改回 `org_node.status`），用户就永久登不进去且机构侧无接口可救。**不做级联写库**——否则停一个 1.1 万人的分支要写 2.2 万行，且恢复时分不清谁是被级联停的、谁是本来就单独停的。
   停用节点不可作为新建/移动的目标父节点（10109）；历史数据保留可查；停用可恢复。
10. **删除**为逻辑删除（`deleted_at` 写入毫秒时间戳，非 0/1 标志）；**节点下存在未删除子节点时禁止删除**（10108），须先迁移或删除下级；机构根节点（即机构最高管理员节点）不可删除。
...
| --- | --- | --- | --- | --- | --- |
| 下级管理员 | 1 | 1 | `org_admin` | — | 管理员节点 |
| 教师（导师） | 2 | 2 | `teacher` | `org_teacher` | 管理员节点 |
| 学生 | 3 | 3 | `student` | `org_student` | 管理员节点 / **教师**节点 |

> `node_type` 与 `user_type` 恒等，两列并列只为对照，实现中不允许出现二者不一致的行。
...
| 教师调岗（带走学员） | 教师节点 → 移到目标管理员节点下，**其学员子树整体跟随** | 4 教师调岗 |

**业务规则**：

1. **源节点与目标父节点必须都在操作人子树内**，否则 10107。跨子树的转交只能由共同上级执行。
2. 目标父节点类型必须合法（学生只能挂管理员或教师；教师只能挂管理员），否则 10104/10105/10106。
...
1. **标签绝不参与任何权限判断**（契约 §9.2 铁律 2）：打标既不改变谁能看到该学员，也不改变该学员能用哪些资源。任何"按标签授权"最终仍落到具体 `org_resource_grant` 行上。
...
**功能描述**：`org_node_change_log` 记录节点全生命周期异动：node_id、change_type、from_parent_id、to_parent_id、change_time、operator_id、reason。change_type 枚举：**1建档 2分配导师 3转交管理员 4教师调岗 5毕业归档 6归档恢复 7退课 8节点移动（管理员节点自身改父，不出现在学生轨迹中）**。
...
5. 轨迹页支持按 change_type、时间范围、操作人筛选。**不提供导出**——异动轨迹是低频查询、数据量小，为它单开一个导出接口不值得；确有需要时走通用报表导出后续扩展。
...
4. `UK` 保证同一资源对同一节点只有一行；重复授权返回 **10303**。~~**修改有效期走独立的"修改授权有效期"接口**（02-组织机构接口 40 修改授权有效期），既不重复插入、也不走"撤销+重授"——撤销强制级联整棵子树，用它来续期会连带清空下级全部授权。缩短有效期时子树内晚于新值的授权行一并截断（防时间维度悬挂），延长则不级联。~~ **已被需方 2026-08-21 定案推翻**：授权一律永久有效，接口 40 已删除、`valid_start` / `valid_end` 两列保留但永远不写。原措辞留在这里，是因为将来若恢复有效期（例如「试听一个月」），上面那段论证仍然成立 —— 用「撤销 + 重授」续期会连带清空下级全部授权。
...
6. 授权目标可以是任意类型节点：授给管理员/教师节点 = 赋予其"使用 + 继续下发"的能力；**授给学生节点 = 该生可学**。**只有授到学生节点，学生端才看得见。**
...
- Given 课程 K 已授权给节点 N，When 再次对 N 授权 K，Then 返回 **10303**；修改有效期须走"修改授权有效期"接口。
- Given 教师 T 对课程 K 的授权有效期至 12-31、其名下 30 名学员各自持有该课程授权，When 把 T 的有效期改为 06-30，Then T 与 30 名学员的授权行**全部原地更新**、一条不删，晚于 06-30 的子树行截断为 06-30；若改为延长至次年 12-31，则学员行不变。
...
1. `valid_start` 必填（默认取授权时刻），`valid_end` 可空表示永久；`valid_start >= valid_end` → **400**（参数校验类，不占业务码；口径同 02-组织机构 接口 38 授权资源给节点 的校验表第 6 条与接口 50 第 4 条）。
...
4. ~~**对正在学习的学员即时生效**：撤销后，正在播放的学员在**下一次心跳或播放凭证续签**时被拒（20013 / 20001），前端提示"课程授权已被收回"并停止播放~~；**已累计的 watched_duration 与 max_position 不清零**，已完成课时保持 watch_status=2。
   ...
   > **⚠「即时生效」这半句已被需方 2026-08-21 定案推翻**（定案三，F-106）：原话「我觉得无需判授权，授权只需要最初判断一次，也就是说看这个视频的时候，但如果没权限了下一个视频就不允许看了」。
   >
   > **现行口径：撤销从「下一次点开视频」起生效，正在播放的那个视频可以看完。心跳不重判授权，播放凭证续签也不重判。** 理由是公司内部学生使用，最坏多看半节课的后半段，不值得在每 10 秒一次的心跳上加一次授权查询。
   >
   > **后半句（学习记录不清零）不受影响**，与契约 §2.5 规则 10 一致。
...
2. 模板归属创建者所在节点（`owner_node_id`），**可见范围 = 我自己建的 ∪ 我的祖先节点建的 ∪ 我子树内节点建的**：上级建的模板下级可见可用（否则机构统一制定的模板下级根本看不到，模板就失去了抵消操作成本的意义）。这是"数据范围只由树决定"这条铁律的**唯一显式例外**，仅作用于模板对象本身。
3. **祖先节点建的模板，明细必须脱敏**：查看其条目时**只返回我当前拥有的资源**，其余折叠为计数 `{总数, 我可用数, 已隐藏数}`；套用响应中的跳过清单同理，只给数量不给清单。**我自己建的与我子树内建的模板不受此限，明细全量可见。**
   为什么必须单独做这一条：FR-6 的取交集只保证"授不出去"，保证不了"看不见"。模板条目就是一张 `(资源类型, 资源ID, 资源名称)` 清单，若祖先模板明细全量返回，下级管理员打开机构建的"高三数学包"，**当场就能枚举出上级拥有但从未授予他的全部资源与名称**——而 FR-2 规则 2 要求未授予资源一律按"不存在"处理、不暴露存在性（边界 B18）。**套不出来 ≠ 看不见。**
   脱敏只作用于展示，**不改变套用行为**：服务端仍按完整模板清单取交集，授权结果与不脱敏时完全一致。
4. 模板内容可包含创建者当时拥有的任意资源；**模板本身不校验使用者是否拥有**——收缩发生在**应用时刻**（FR-6）。
5. 模板可编辑（增删条目）、停用（**status=1**，停用后不可再套用 → 10305；`status` 为 0 启用 1 停用，DDL 默认 0，故新建模板默认可用）、逻辑删除；**已按模板产生的授权行不随模板变更或删除而回滚**（授权一经授出即独立存在）。
6. `item_count` 冗余维护；单模板条目上限 2000（与导入上限同量级，防止单次授权过大）。**套用时另有总量硬上限**：交集后写入的授权行数 ≤ 5000（= 交集资源数 × 目标节点数），超出整体拒绝并提示分批——2000 条目 × 500 节点 = 100 万行同步事务足以拖垮主库（契约 §2.5）。
7. 典型模板："高三数学包" = 20 课程 + 500 题目 + 80 视频；"新教师标配" = 10 课程 + 200 题目。

**验收标准**：

- Given 机构管理员在机构根节点创建模板 T，When 其下三级的教师打开"套用模板"，Then 能看到并可选择 T（祖先建的模板可见）。
- Given 机构建的模板 T 含 600 项，某下级管理员当前只拥有其中 120 项，When 他查看 T 的明细，Then 列表**只返回那 120 项**，另外 480 项不出现在列表中，仅以 `{总数 600, 可用 120, 已隐藏 480}` 的计数呈现；他无法从任何响应中得到那 480 项的资源 ID 或名称。
- Given 同一管理员**自己**建了模板 S 且其中部分资源的授权已过期，When 他查看 S 的明细，Then **全量返回**（含已过期项，标注不可授出与原因）——脱敏只针对祖先建的模板。
...
1. 应用模板时**逐条校验授权人是否拥有该资源**；不拥有的部分**自动跳过**，并在响应中按资源类型分组列出被跳过项及原因，**绝不放大权限**（契约 §9.2 铁律 3）。
...
**功能描述**：管理端上传视频至云端 VOD（**阿里云为主、本期唯一实现**；`provider` 列保留 1腾讯 2阿里 两个取值以备兼容，默认 2，契约 §1）。媒资记录于 `vod_video`（含 **`owner_node_id` 归属节点**，创建时写入上传者所在节点），状态机：0上传中 → 1转码中 → 2正常；异常 → 3转码失败；人工可置 9禁用。云端自动转码为**加密 HLS（.m3u8）**。

> **归属与授权**：媒资带 `owner_node_id`，视频作为受管资源可独立下发（`org_resource_grant` resource_type=3），用于"下级只能引用上级授予的视频建课"的场景。上传直传、转码事件消费、状态机、防下载等全部逻辑不变。

**业务规则**：

1. 上传流程：前端向后端申请上传凭证（阿里云返回的 `cloudVideoId` 此时即写入 `vod_file_id`）→ 前端直传云端（不经业务服务器）→ `FileUploadComplete` 事件置 status=1 → `TranscodeComplete(success)` 事件置 status=2 并回填 hls_url、duration、size_bytes、cover_url。**事件由 XXL-Job 拉取轻量消息队列消费，不是 HTTP 回调**（03-课程与视频 §7.2）。
2. 转码模板强制：**单一清晰度**的 HLS 输出，禁止生成可直接下载的 MP4 播放地址。
   > **⚠ F-114（2026-08-21 需方定案）：第一版【不加密】，且【加密的代码留着】。**
   > 需方原话：「目前给不加密的视频，之后再决定加密方案。加密的代码留着，以后可能启用，甚至可能两个模板并存、自适应选择。」
   >
   > **实现方式不是「写死不加密」，是【不猜】**（F-114 第二半）：挑流条件放宽为 `Format == "m3u8"`（加不加密都收），
   > `vod_video.encrypt_type` **由转码完成时实际观测到的值回填**，上传时写的只是占位。
   > 于是**需方在控制台换模板组、改 `ALIYUN_VOD_TEMPLATE_GROUP_ID` 即可切换加密与否，代码一个字不动**——
   > 加密的代码因此天然留着，不用删也不用加开关。切换步骤见 04 §E F-114。
   >
   > **⚠ 为什么不能写死**：写死等于在代码里存一个「模板组配成了什么」的假设，而配置在需方侧。
   > 原先那条 `Encrypt == true` 的挑流规则就是这个形态 —— 需方换成不加密模板后，
   > **转码成功的视频会被我们判成失败，而控制台显示一切正常**。
   >
   > **「单一清晰度」与「禁止 MP4」两条不受影响，第一版就要照做** —— 它们与加不加密无关。**不支持清晰度切换**——`vod_video.hls_url` 是单值列，配多档转码会让学生实际看到的清晰度不可控；要支持切换须先改数据模型，属后续需求（契约 §1 部署约定）。
3. status=3 转码失败：**管理端媒资列表红色标记（A11）+ 工作台「转码失败」待办计数（A2）**，按 `upload_user_id` 归属到上传人；**不建消息表、不新增接口**——A11 已有红色标记与重试入口、A2 已有该待办卡片，用现有接口即可承载。支持"重新发起转码"与"重传源文件"。
...
- Given 视频上传完成且 `TranscodeComplete(success)` 事件被消费任务处理完毕，When 管理员刷新媒资列表，Then 该视频 status=2、duration 与 hls_url 已回填，可被课时选用。
- Given `TranscodeComplete(fail)` 事件被消费任务处理完毕，When 上传人进入管理端，Then 工作台「转码失败」待办计数 +1、媒资列表该条 status=3 红色标记并提供重试入口；引用它的课时无法置为可见。
...
1. 凭证有效期固定 300 秒（authExpire=300，与 03-课程与视频分册 §9.1 一致）；到期后播放器续播需静默重取凭证（无感续签）。
...
> **F-113 定案（2026-08-21，需方逐条拍板）**：**禁快进不做、倍速允许、观看时长由前端计时、完播判定第一版延后。**
> 本节原标题为「禁快进与完播状态机」，两件事第一版都不做了，故整节改写并改名。
> **前提是「不防刷课」**（需方原话「刷课不需要防」）——本节保留的机制**都换了理由**，不再是防刷。

**功能描述**：**Aliplayer** 二开。**拖拽完全放开**（想拖哪拖哪，不回弹）；**允许倍速**（UI 提供，但计时不受倍速影响）。学生在课时上花的时间由**前端计时器**累计、随心跳上报，服务端记账（见 F2-7）。

**业务规则**：

1. **拖拽不受限**：任意位置自由拖拽，**不回弹、不提示**。原「拖拽目标 > max_position 强制回弹」的规则**已废止**（对应 F2-7 校验规则 3 墓碑）。
2. **`max_position` 仍然记录并取大**（服务端以有效心跳的 `currentTime` 取 max），复看不回退。
   > **它换了用途，不是顺手留着**：禁快进取消后原用途消失，但它是**第一版唯一能反映"看到多深"的数据**。与 `watched_duration` 配合可识别"反复看开头刷时长"——时长 60 分钟 + `max_position` 10 分钟 = 一直在看开头；时长 60 分钟 + `max_position` 58 分钟 = 正常看完。**不用做内容覆盖率、不加任何字段。**
3. **`last_position` 记录上次离开位置**（直写，不取大），供 F2-8 断点续播使用。
4. **允许倍速，但计时不受倍速影响**（见 F2-7 校验规则 4）。**这条现在比以前更要紧**：计时挪到前端后，错误的实现方式会让倍速自动加速计时——约束见 F2-7「前端计时三条硬性要求」。
5. **完播状态机第一版不启用**：`watch_status` 只在 `0 未开始` → `1 学习中` 之间流转，**不写 2**；`complete_time` 恒空。判定逻辑与阈值配置保留在 F2-9，留待将来启用，**且需方已定不做历史回算**。
6. 断网期间前端停表；恢复后从服务端进度续播（见 F2-8）。

**验收标准**：

- Given 学生当前 `max_position=300` 秒，When 把进度条拖到 **500 秒（未看过）**，Then **正常从 500 秒播放，不回弹、不提示**；`max_position` 更新为 500。
- Given 学生把进度条拖回 120 秒，Then 正常从 120 秒复播，`max_position` **保持 500 不回退**。
- Given 课时 `duration=600` 秒，学生 `watched_duration` 累计达到 540 秒，When 落盘，Then **`watch_status` 仍为 1**、`complete_time` 仍为空——**第一版不判完播**（F-113 定案四）。
- Given 学生 `watched_duration=3600`、`max_position=600`（视频总长 3600 秒），When 导师查看，Then 可据此判读"该生反复观看开头"——**这是第一版识别该场景的唯一手段**。
- Given 学生从未打开过课时 L1，When 教师查看进度列表，Then 该生 L1 行 `watch_status=0`、`watched_duration=0`。

### F2-6 跑马灯水印（防录屏）—— **第一版不做（F-114）**

> **⚠ F-114（2026-08-21 需方定案）：水印第一版不做。** 需方原话「水印不做，之前其实说过」——
> 与「第一版先上不加密、没水印的版本」是同一次决定。
> **本节规则原样保留待启用**，但第一版：接口 28 不下发 `watermark` 字段、播放器不挂水印、
> `watermark_phone_mask` 配置键不被消费。
>
> **⚠ 将来启用前必读 `poc/r1a-aliplayer/RESULT.md`**：2026-08-21 实测
> **iOS 微信全屏下 DOM 水印不成立**（`webkitbeginfullscreen` 由系统播放器接管，画面上没有水印），
> **而且水印自检在那期间一路报绿** —— 那套自检不能作为水印有效的证据。
> 这一节将来重启时**不是照抄就能用**：iOS 那一半要另想办法（伪全屏，或转码时把水印烧进画面）。
...
1. 水印内容：`real_name + phone`。**学生端播放一律脱敏**（中间四位打星，如 `李子墨 138****5678`），**不可配置、不可关闭**——受《个人信息保护法》第 31 条约束的是未成年人信息，"允许租户选择不脱敏"本身不成立（契约 §7.2 第 2 条）。**管理端预览可配**（租户配置 `watermark_phone_mask`，默认 1 脱敏；置 0 显示完整手机号以强化威慑）——此时被水印的是机构自己的员工，威慑对象也是他自己。
...
**功能描述**：播放器每 10 秒发送一次心跳（契约 6.4 固定签名）。**观看时长由前端计时器给出，服务端照单收账、不自己计时**（F-113 定案二）；服务端只做参数校验与物理上限截断，写 Redis 缓冲，XXL-Job 每 60 秒批量落盘 `vod_watch_progress`，明细写 `vod_heartbeat_log`（按月分区，可归档，无逻辑删除）。

> **⚠ F-113 前提：不防刷课（需方定案「刷课不需要防」）。** 本节保留下来的规则**大多换了理由**——留痕、诊断、兜 bug——**不再是"使刷课不快于真实播放"**。
> **理由必须写实**：一条理由已经不成立的规则留在文档里，下一个人会照着那个假理由做决定（F-100、F-111 各栽过一次）。

**接口（契约 §6.4；`sessionPlayedSeconds` 为 F-113 明知推翻后新增，C15 核三处一致）**：

```
POST /api/v1/vod/heartbeat
Body: {"lessonId":"1953827104412590081","videoId":"1953827104412590082","currentTime":123.4,"duration":600,"playRate":1.0,"seeked":false,"sessionId":"3d5b81c04e7a4a92b6f0c8e2d1a97b53","sessionPlayedSeconds":118,"trigger":"tick"}
Resp: {"code":200,"msg":"操作成功","data":{"watchedDuration":130,"watchStatus":1,"maxPosition":135}}
```

**服务端校验规则（九条，编号与 03-课程与视频 §8.3.1 逐条对应）**

> **规则编号以 03-03 §8.3.1 为准**，本节与 `vod_heartbeat_log.reject_rule` 均引用该套编号，**三处必须一致**。
> 之所以要把这句话写死：`reject_rule` 已经把规则编号**持久化进审计表**，而该表按月分区保留 6 个月、累计留存数年。
> 一旦某一侧改号，历史日志里的 `reject_rule=N` 就会指向另一条规则——运维查到 `reject_rule=9` 对着本节读成"参数越界"、
> 实际是"多端会话冲突"，两者的处置动作完全相反（一个让用户检查播放器，一个让用户关掉另一端）。**历史日志无法回溯修正。**
>
> 本节写**业务语义与为什么**；阈值与判定细节（区间、次数、秒数）一律以 03-03 §8.3.1 为准，不在此处重复定义。

1. **凭证有效**：心跳必须携带 F2-4 发放且未过期的播放凭证，且凭证绑定的学生与课时与本次上报一致；不通过返回 **20001**，前端无感续签后重试，播放不中断。
2. **间隔与会话级限流（F-116 重写）**：**①会话级上限**——每会话每分钟被采纳的心跳 **≤30 条**，已达上限一律丢弃（`20002`）；**②`trigger = tick` 的周期心跳另走间隔闸**——与上一次**被采纳**心跳的服务器接收时间差 ≥8s 有效，过短判异常按规则 5 处理；**③事件心跳不走间隔闸，但计入①**。异常明细每会话每分钟最多写 20 条，超出后仍拒绝但不再写明细，精确条数看指标 `heartbeat_reject_total{rule}`。基准是"上次被采纳"而非"上次收到"，这一点决定了多端并发下的行为（见规则 9）。
   > **F-113 删掉了本条原有的半个理由。** 原文写「间隔过短意味着伪造加速**或重复请求**」——改报累计值之后**重复投递已经天然幂等**（同一个累计值再来一次 `delta = 0`），"防重复把时长算两遍"这个理由**不成立了**。**本条保留**，剩下的理由是**限流**（挡住高频空包、保护 Redis 与日志表）。留着一条理由已经不对的规则，比删掉它更危险。
   > **F-116（需方 2026-08-22）把本条改成按 `trigger` 分流。** F-113 之后前端除周期心跳外，还会在**停下来 / 恢复 / 播完 / 离开页面**四类时刻各发一条**事件心跳**（需方「暂停期间不允许心跳」那条要求的直接产物），而事件心跳本来就不看表。硬 8 秒闸会把它们大量判为异常 ——**真机实测 23 条丢 12 条（52%），前端 76.3 秒而服务端入库 69.3 秒，差的 7.0 秒丢在最后一条暂停心跳上，它之后没有心跳来补**。<br>**为此给签名加了第 9 个字段 `trigger`**（`tick`/`pause`/`resume`/`ended`/`leave`）：8 个老字段里没有任何信息说明心跳类型，`seeked` 只在"拖动后/恢复后第一条"为 `true`，盖不住暂停与离开页面。**这是该签名第四次变更，前三次是 `seeked`、`sessionId`、`sessionPlayedSeconds`，每次都在文档里写明"明知推翻"。**<br>**⚠ 两件事分开做**：`trigger` 负责**分类**（解决指标口径、让日志看得出学生在干什么），**会话级上限负责限流**。上一版试图用一个机制同时干两件事，结果是「要么事件心跳被卡死、要么限流形同虚设」。<br>**上限取 30 的依据（真机实测）**：正常观看峰值 **10 条/分钟**（那一分钟含 5 次拖动），30 有 3 倍余量。注意**拖动会成对产生事件心跳**（`seeking`→`pause`、`seeked`→`resume`），要撞到 30 需要一分钟拖 12 次；撞到之后只是该分钟后续心跳被丢，**下一分钟的累计值会把时长补回来**。**对脚本是硬闸**：无论把 `trigger` 报成什么，一分钟最多被采纳 30 条。
3. **【已废止 · 墓碑（F-113）】拖拽校验**：原文为「`currentTime` 不得超过 `max_position` 加一个单周期的最大合法推进量」。**禁快进取消后本条失去对象。**
   **编号 3 从此不写入 `vod_heartbeat_log.reject_rule`，且不得复用；4~9 号不上移**——编号已随该列持久化进审计表，重编号收益为零而指错了没有任何症状（照 03-03 §8.2 接口 29 的墓碑先例）。
   原文附带的那句「本条对 `seeked=true` 同样生效、放开等于绕过禁快进」**一并删除**：它论证的对象已经不存在。
4. **倍速不加速计时**：`playRate` 仅作记录，时长按**真实墙钟**计，2 倍速看 10 分钟视频仍只累计约 5 分钟。**本条不拒绝任何心跳**，只约束计时口径。
   > **F-113 之后本条比以前更要紧**：计时挪到前端，而前端**更容易想到的写法**（监听 `timeupdate` 累加 `currentTime` 增量）会让倍速自动加速计时，**正好推翻本条**。本条现在是那个错误写法的**唯一**约束，实现约束见下方「前端计时三条硬性要求」。
5. **异常心跳丢弃（F-113 收窄）**：凡未通过规则 2，或参数明显非法（`currentTime` 超出 `[0, duration+2]`、`duration` 与媒资实际时长偏差过大、`playRate` 超出合法区间、**`sessionPlayedSeconds` 为负 / 非整数 / 缺失**），一律丢弃并返回 **20002**；明细仍写 `vod_heartbeat_log`。
   **删去"未通过规则 3"**（规则 3 已废止）。
   > **⚠ 明确禁止：不得因为规则 6 的判定结果去扣时长。** 本条只处理**参数非法**。规则 6 只留痕、不影响时长——理由见规则 6。
6. **推进一致性校验（F-113 降级为只留痕）**：**仅在连续两次 `seeked=false` 的心跳之间**校验——有效心跳的位置推进量须与墙钟一致；长期零推进而间隔正常（期间无任何 `seeked=true`）判定为挂机或纯脚本上报。

   > **⚠ 本轮最容易实现错的一条。判定逻辑不变，处置变了：**
   >
   > ```
   ...
   > ```
   >
   > **为什么必须这样**：时长既然由前端给，后端一旦说「这条我不给你算」，**后端就等于又在自己算时长**，
   > 那就变回**两个数字表示同一件事**——本项目反复吃亏的形态。
   > 留痕仍计入 `vod_heartbeat_log.reject_rule = 6`，由风控报表按 `(student_id, lesson_id, 时间窗)` 聚合呈现。

   **`seeked` 字段的由来**：拖动完全放开后（规则 3 已废止），学生拖动或暂停恢复后的下一次心跳，`Δ currentTime` 必然落在校验区间之外。故前端在 seek 或暂停恢复后的第一次心跳置 `seeked=true`，服务端据此重置校验基准：本次不做推进判定。

   **`seeked` 自身必须限频**：连续 `seeked=true` **不超过 2 次**（不变），单课时每小时 **不超过 180 次**（**F-113 由 20 提高**）。超出按规则 5 丢弃并计入 `reject_rule`。
   > **20 → 180 的理由**：20 是在**禁快进**前提下定的——那时学生只能在已看范围内小幅回拖。**放开拖动后，一小时拖 20 次是完全正常的观看行为**，再按旧值卡，会把**被明确允许的行为持续判为异常**，而学生只看到时长莫名其妙少了、前端没有任何提示——与规则 9 那个「永久归零」是同一类失效。180 次 ≈ 平均每 30 秒可拖一次，仍挡得住"每 10 秒 seek 一次"的脚本模式。
   >
   > **这个上限保护的对象也变了，理由要写实**：时长已经不由规则 6 决定，所以它**保护不了时长的准确性**；
   > 它保护的是**日志的诊断价值**——脚本若恒置 `true`，规则 6 永不判定，`vod_heartbeat_log` 就再也看不出
   > 这个学生有没有在真的播放，而**日志是第一版唯一的行为事实来源**。

7. **并发课时数上限**：单学生同一时刻的活跃课时数有上限，超出的课时心跳直接丢弃，并计入 `vod_heartbeat_log.reject_rule`，由风控报表按 `(student_id, lesson_id, 时间窗)` 聚合呈现。此前的并发防护只作用于"同一课时"，跨课时并行不受限是防刷模型的最大缺口——多个课时并行的脚本一晚可刷完整门课程。
8. **【已废止 · 墓碑（F-117）】时长封顶**：原文为「单学生单日累计与单课时累计均有上限，超出部分只记录、不计入」。
   **需方 2026-08-22 定案废止。** 物理上限截断 `min(delta, Δt + 5)`（契约 §6.4）已经保证一整天的累加不超过真实流逝时间，
   **不会放出「999 小时」**——本条要兜的「前端计时器一口气报出几小时」那一类，在截断那一步就被削掉了。
   **编号 8 从此不写入 `vod_heartbeat_log.reject_rule`，且不得复用；9 号不上移**——理由与规则 3 的墓碑逐字相同（编号已随该列持久化进审计表）。
   > **⚠ 废止的代价只有一项，需方已知情并接受**：10 分钟的视频累计出 8 小时将**原样记录**（原 `duration × 3` 会把它压到 30 分钟）。
   > 该数值意味着循环播放整晚或前端故障，**此后不再有任何提示**。
   > **这一项是本条唯一还在起作用的东西**——它拦不住的（锁屏虚增、跨天累加）本来就由截断和前端计时守卫承担。
9. **同课时会话择一**（多端并发）：F2-4 发凭证时下发 `sessionId`（续签不变），心跳携带；服务端按学生 + 课时维护当前活跃会话——同会话正常计时；不同会话且当前会话仍活跃时返回 **20020**，前端**暂停播放并明确提示**"该课时正在其他设备播放"；当前会话超过静默期无心跳（关页面/断网/换设备）则新会话自动接管，并**重置规则 6 的推进基准与 `sessionPlayedSeconds` 的累计值基准（置 0）**——后者不重置，新设备第一条心跳会因累计值小于上一端而恒 `delta = 0`，时长永远不涨。
   **⚠ F-113 之后本条的理由整段换了，旧论证不再成立。**

   **旧论证（保留供回溯，已失效）**：不做会话择一的真实后果是**永久归零**——两端交替时，另一端的心跳会被**规则 6 判为位置跳变而拒绝**，而被拒的心跳不推进基准，下一次仍以陈旧位置为基准，就此锁死，学生此后一小时时长累计约等于 0。

   **为什么失效**：**规则 6 已降级为只留痕、不再拒绝任何心跳**（F-113 定案），那条锁死链路的**第一环就断了**。**照抄旧理由会推出"`sessionId` 可以不要"的错误结论。**

   **新理由——`sessionId` 现在承担两件事，都还成立**：
   - **① 累计值的作用域**：`sessionPlayedSeconds` 是**按会话**累计的，服务端必须知道这个累计值属于哪一次播放才能算出 `delta`。没有 `sessionId`，两端的累计值互相顶，`delta` 时正时负。
   - **② 防止两端同时计时重复累加**：两台设备同时播放、各自计时器都在走，若都被采纳，**同一段真实时间会被计两次**。这一条比旧论证更直接——旧的是"算不出来"，新的是"**算多了**"。
   **被拒绝的心跳一律不推进任何基准**（时间基准、位置基准、`max_position`）；唯一例外是规则 9 的会话接管，接管时必须重置位置基准，否则新设备第一条心跳即被判跳变，接管等于没发生。

**心跳处理流程（另起编号；以下各条不拒绝任何心跳，不参与 `reject_rule` 取值）**

1. **前端触发时机与计时器**：仅在真实播放（playing 且未暂停、页面可见）状态下每 10 秒触发；暂停、卡顿缓冲、页面切后台（Page Visibility 隐藏）时**停止心跳并停表**。

   **⚠ 前端计时三条硬性要求（F-113，缺一条就会算错）**
   ...
   **① 累计的是墙钟，不是媒体时间。**
   ...
   | 累计 `currentTime` 的**增量** | 算 **10 分钟** | ❌ 推翻规则 4 |

   第二种是**更容易想到的写法**（监听 `timeupdate` 把增量加起来），代码更短、看上去更"准"。**`playRate` 不参与计时**，它只作为报文字段上报。

   **② 报累计值，不报增量。**`sessionPlayedSeconds` = 本会话至今累计真播秒数。报增量的话，网络重发＝多算一次、丢一个包＝永远少算；报累计则**重发天然幂等**（`delta = 0`）、**丢包自愈**（下次的累计值把量带回来）。页面刷新 = 新会话，累计从 0 重起。

   **③ 收尾心跳（beacon）必须带最终累计值**，否则最后一段观看会丢。
2. **单次计入时长（F-113 整条改写）**——服务端**不再按到达间隔计时**：
   ...
   **`min(delta, Δt + 5)` 是上界，不是第二个真相。** 它存在的唯一理由是兜前端 bug：浏览器会冻结后台标签页的定时器，页面醒来时写得糙的计时器可能一口气报出几小时。**服务端不重算时长，只拒绝物理上不可能的增量**——**一个数、一个封顶，不构成两个数字打架**。不要把它"补全"成第二套计时。
   截断发生时写 `vod_heartbeat_log` 留痕，**不占用 `reject_rule`**（心跳被采纳了，只是增量被削）。

   **墙钟有效累计：复看已看区间同样计时长**——`watched_duration` 语义为"累计学习投入时长"。**第一版不用它判完播**（F2-9 已延后）。
3. **`max_position` 取大**：`max_position = max(当前值, 本次 currentTime)`，仅被采纳的心跳可推进；复看不回退。
   > **F-113 之后它换了用途，不是"顺手留着"**：禁快进取消后原用途消失，但它成了**第一版唯一能反映"看到多深"的数据**。与 `watched_duration` 配合即可识别"反复看开头刷时长"：<br>时长 60 分钟 + `max_position` **10 分钟** → 一直在看开头；<br>时长 60 分钟 + `max_position` **58 分钟** → 正常看完。<br>**不用做覆盖率、不加任何字段，这个数本来就在存。**

3.1. **`last_position` 直写**：`last_position = 本次 currentTime`（不取大）。供 F2-8 断点续播使用。
4. **写路径**：被采纳的心跳更新 Redis（key：`vod:hb:{studentId}:{lessonId}`，含 watchedDuration / maxPosition / lastPosition / sessionPlayedSeconds 基准 / lastHeartbeatTime）→ XXL-Job 每 60s 批量 upsert `vod_watch_progress`（UK student_id + lesson_id）。
   **第一版不判完播**（F-113 定案四）：`watch_status` 只在 0/1 流转、不写 2，`complete_time` 恒空。F2-9 的落盘判定留待将来启用。
5. **响应回传**：心跳响应实时回传 Redis 中最新 `watchedDuration` / `watchStatus` / `maxPosition`，供前端界面展示进度。
6. **Redis 降级**：Redis 不可用时心跳直写数据库（限流保护），保障计时不中断（性能目标详见第 7 章）。

**验收标准**（F-113 后整套改写）：

- Given 学生正常播放 60 秒、前端计时器累计到 60，When 心跳按 10 秒到达 6 次，Then `watched_duration` 增加约 60 秒，`vod_heartbeat_log` 新增 6 行。
- Given 学生以 **2.0 倍速**播放 **300 秒墙钟**时间，When 观察 `watched_duration` 增量，Then 增量约 **300 秒（不是 600 秒）**，`playRate=2.0` 记在心跳日志里。**这条钉的是前端计时器累计的是墙钟而非 `currentTime` 增量**——若实现成后者，本条必红。
- Given 同一条心跳因网络重试被投递两次（`sessionPlayedSeconds` 相同），When 服务端处理第二条，Then `delta = 0`，**时长不会翻倍**——**不依赖规则 2 挡它**，累计值口径本身即幂等。
- Given 心跳丢失一次（`sessionPlayedSeconds` 从 100 直接跳到 120），When 下一条到达且 `Δt = 20s`，Then `delta = 20` 全额计入，**丢的那次自动补回**。
- Given 页面切后台 5 分钟、定时器被浏览器冻结，恢复后前端一口气报出 `sessionPlayedSeconds` 增加 **3600**，而 `Δt = 300s`，When 服务端处理，Then **`delta` 被截断为 305**，`vod_heartbeat_log` 留痕，**心跳仍被采纳**（`reject_rule = 0`，不是拒绝）。
- Given 学生把进度条从 500 秒拖到 **未看过的** 400 秒，When 拖动后第一次心跳以 `seeked=true` 上报，Then **正常处理、不回弹**（规则 3 已废止），`max_position` 保持 500 不回退。
- Given 学生一小时内拖动 **150 次**，When 每次拖动后置 `seeked=true`，Then **全部正常处理**（上限 180）；**第 181 次**起按规则 5 丢弃并记 `reject_rule=6`。
- Given 学生长时间挂机（连续 5 次 `Δ currentTime ≈ 0` 而 `Δt` 正常、期间无 `seeked=true`），When 服务端判定，Then 记 `vod_heartbeat_log.reject_rule = 6` 留痕，**但 `watched_duration` 照常按前端累计值增长**——**规则 6 不影响时长**（F-113 定案，本条是它的护栏用例）。
- Given 攻击脚本每 3 秒发一次心跳，When 服务端接收，Then 除首次外均返回 **20002** 丢弃（规则 2 限流），`vod_heartbeat_log` 记 `interval_sec=3`。
- Given 学生在电脑上播放课时 L（会话 S1）后又在手机上打开同一课时（S2），When S2 心跳到达且 S1 在 60 秒内仍活跃，Then S2 返回 **20020**，手机端暂停并提示。
- Given 学生关闭电脑满 60 秒后手机端 S2 继续上报，Then S2 **自动接管**，推进基准**与累计值基准（置 0）**一并重置，此后手机端正常计时——**不会因为 S2 的累计值小于 S1 而恒 `delta = 0`**。
- Given 任意学生任意课时，When 第一版运行期间查询 `vod_watch_progress`，Then `watch_status ∈ {0, 1}`、`complete_time` 恒空——**第一版不判完播**（F-113 定案四）。
...
1. 拉起播放器时调用进度查询接口，取 **`last_position`（上次离开位置）**作为续播起点。
   > **F-113 改**：原文取 `max_position`（最远触达），理由是「复看语义下保证不丢进度」。**放开拖动后这条不成立**——`max_position` 可能只是学生随手拖到过的位置，从那里续播会**跳过他实际没看的中间部分**，且体验上莫名其妙。故改取真正"上次离开的地方"。
2. 若 `watch_status=2` 已完成，默认从 0 开始复看并提示"本课时已完成"。**第一版不会出现状态 2**（F-113 定案四），本条留待完播判定启用后生效。
3. 进度查询与心跳同源（Redis 优先，落库兜底），换设备续播误差 ≤ 落盘周期 60 秒 + 一个心跳周期。
4. 学生离开页面时前端补发一次收尾心跳（beacon），尽量缩小续播误差。

**验收标准**：

- Given 学生在手机上播到 `last_position=420` 秒后关闭页面，When 3 分钟后在电脑浏览器打开同一课时，Then 播放器从 ≥405 秒（误差窗口内）位置续播。
- Given 学生播到 200 秒后**拖到 500 秒看了一眼又拖回 210 秒**然后离开（`max_position=500`、`last_position=210`），When 再次进入，Then 从 **210 秒**续播——**不是 500 秒**。
- Given 学生课时已完成（`watch_status=2`），When 再次打开该课时，Then 从 0 秒播放并显示已完成标识，进度状态不发生变化。**（第一版不触发）**

### F2-9 完播判定与租户配置

> **⚠ F-113 定案四：本节整节【第一版不启用】，且【将来也不做历史回算】。**
> 需方原话：「我们不会重算的，如果后续要，也只会要每个学生在那堂课上花了多少时间」。
> **那个数第一版就在存**——`vod_watch_progress.watched_duration`，逐 `student_id` + `lesson_id`，
> 从模块 13 上线那天起，**不需要任何额外工作**。
> 本节规则与阈值配置**原样保留**，留待将来启用；启用后**只对此后的落盘生效**，不回溯。

**功能描述**：完播规则统一为 `watched_duration >= duration × complete_rate_threshold`，阈值默认 90%，租户级可配置（配置键 `complete_rate_threshold`，建议可配区间 60%~100%）。**第一版不执行本判定**：`watch_status` 只在 0/1 流转，`complete_time` 恒空。

**业务规则**：

1. 判定时机：XXL-Job 落盘 `vod_watch_progress` 时逐条判定；满足即 watch_status 置 2、写 complete_time。**（第一版不执行）**
2. 阈值调整仅对调整后的落盘判定生效，历史已完成记录不回滚、未完成记录按新阈值继续判定。
   > **F-113 补充：本项目不做完播回算。** 这一条与「将来启用完播判定」不冲突——启用时同样只对此后的落盘生效。
   > **能补与不能补要分清**：**完播状态**（某学生某课时算不算完成）随时能从 `watched_duration` 终值比一次阈值得出；
   > 而 **`stat_*_daily` 的每日历史序列**只能靠重放 `vod_heartbeat_log` 补，且**只能补到该表保留期为止**（按月分区留 6 个月）。
   > **需方已定不回算，此处只作事实记录，不是待办。**
...
| --- | --- | --- | --- | --- |
| 1 | 单选 | 题干 + 选项数组（A/B/C/D…，2~8 项） | `{"answer": "B"}` | 自动 |
| 2 | 多选 | 题干 + 选项数组 | `{"answer": ["A","C"]}`，比较前排序去重 | 自动 |
| 3 | 判断 | 题干 | `{"answer": true}`——**JSON 布尔，不是字符串 `"true"`** | 自动 |
| 4 | 填空 | 题干含空位标记，1~10 空 | 标准答案 `{"blanks":[{"index":1,"accepts":["北京","北京市"]}]}`；学生作答 `{"blanks":[{"index":1,"text":"北京"}]}`，按 index 对齐 | 自动 |
| 5 | 简答 | 题干（可含图片） | `{"text":"……"}`（参考答案，供批改参照） | 人工 |
| 6 | 材料题(父题) | 材料正文 + 子题排序 | 无（分数=子题合计） | 按子题各自方式 |

> 完整结构见契约第 5 节「答案 JSON 结构」。`correct_answer` 与学生作答 `student_answer` 在题型 1/2/3 上同形；
> **题型 4 填空不同形**（标准答案是同义答案集 `accepts`，学生作答是单个 `text`）——
> 见契约 §5 的订正段与 `04-实施计划.md` **F-70**。本节规则 5 的验收标准（第 1 空同义答案集为
> `["鲁迅","周树人"]`）正是依赖 `accepts` 的那一条。
> **判断题必须是布尔字面量**：`true != "true"`，两端不一致会导致全部判断题判错，而客观题不开放
> 教师改分（F3-6 规则 3），错了没有救济路径。服务端收到字符串直接 400，不做隐式转换。
...
8. **分类树的增删改仅管理员可操作**，教师只读。`qb_category` 不带 `owner_node_id`、不进资源授权，是全租户共享的目录结构，落在"数据范围由树决定"这条规则的管辖之外——树能限制谁看到哪些题目，限制不了谁改分类树。若开放给教师，任一教师改个分类名或删个空分类，全机构题库目录立刻跟着变，没有任何子树边界拦得住。教师需要新分类时走管理员申请。
...
5. 已批改答卷允许批改教师在**首次批改完成（`grade_time`）后 7 天内**修正打分（记录 grade_time 更新与操作日志）；修正后总分与统计重算。
   起算点取 `grade_time` 而非 `deadline`：作业允许逾期补交时（`allow_late_submit=1`），学生可能在 deadline 之后很久才提交、教师随即批改——若按 `deadline + 7 天`，这类答卷在**首次批改那一刻窗口就已经关闭**，而客观题不开放教师改分（PRD F3-6 规则 3），主观题的唯一救济就是本窗口，于是分数一旦打错，系统内没有任何路径能改。窗口的意义是"给批改人留出改正自己刚才那次打分的时间"，锚点自然是那次打分。
...
> **⚠ F-113：本节所有「完播率 / 已完成课时数 / 课程进度百分比」指标第一版不可用**（完播判定延后，见 F2-9），相关列恒为 0。**第一版可用的学习类指标只有累计观看时长与 `max_position`。**
...
| 指标 | 公式 | 说明 |
| --- | --- | --- |
| **名下学员平均完播率** | `avg_complete_rate = Σ finished_lesson_count ÷ Σ should_lesson_count × 100`——**先把名下全体学员的分子、分母各自汇总，再相除**（课时数加权），**不是**"每人先算比率再取算术平均" | 统计范围仅限视频课时（lesson_type=1）；**应学视频课时总数 = 该生节点被显式授权（`org_resource_grant` resource_type=1，有效期内）的课程中，可见（status=1、未删除）视频课时总数**——分母按**逐生授权**计算；分母为 0 时记 0 并显示"—"；结果保留两位小数，落 `stat_teacher_daily.avg_complete_rate`(0-100) |
...
- Given 教师张三名下 40 名在读学员、每人被授权课程的可见视频课时均为 20 个，合计已完成 600 个课时，When 看板计算完播率，Then 显示 600 ÷ (40×20) = 75.00%。（分母人人相同的场景下，两种口径结果一致，故不能用它验证口径。）
- Given 张三名下学员甲被授权 3 门课（共 20 个视频课时、已完成 15 个）、学员乙被授权 1 门课（共 5 个视频课时、已完成 1 个），When 计算平均完播率，Then **合并分子分母后相除** = (15+1) ÷ (20+5) = **64.00%**；
  **不是**甲乙各自算比率（75% 与 20%）再取算术平均的 47.50%。两种口径在分母不齐时相差 16.5 个百分点，而分母不齐才是常态（学员被授权的课程数本就不同）。
  口径唯一权威见 03-05 §2.3(2)：`stat_*` 表落的是**分子列与分母列**（`finished_lesson_count` / `should_lesson_count`）而非比率，正因为比率不可跨天、跨人平均；前端**不得**用 `Σ学员比率 ÷ 人数` 复核本字段。
...
| **节点排行** | 子树内全部下级管理员节点按 `avg_complete_rate` 降序 → `ontime_submit_rate` 降序，取 Top/Bottom 各 5（可配至 50）；仅统计子树内在读学员数 > 0 的节点 |
...
| A8 | **节点异动轨迹** | 按 change_type / 时间 / 操作人筛选的轨迹流水 | 组织管理-异动轨迹 |
...
| A21 | 用户管理 ★ | 账号列表（跨租户，平台侧）、创建/修改/删除账号、重置密码、启用停用；写操作仅平台超管（03-01 §2.1~2.6） | 主导航-系统 |
| A22 | 文件管理 | 已上传文件列表与详情、时效签名 URL 下载（03-01 §7.1~7.3）；归属校验只在下载接口实现一次（00-通用约定 §7.4） | 主导航-系统 |
...
| B3 | **学员学习进度** | **学员列表 → 单个学员档案 → 导出进度报表**三件套（**不做矩阵视图**）：从 B2 选学员进入其档案（F4-3），批量口径走 `exportType=1 学员学习进度` 报表——该报表的列（学号 / 姓名 / 归属路径 / 导师 / 课程名 / 应学课时数 / 已完成课时数 / 课程进度 / 累计观看时长 / 最近学习时间）即该矩阵的行式表达 | 我的学员-进度 |
...
| C5 | 视频播放页 | **Aliplayer**（禁快进/跑马灯水印/倍速）、断点续播提示、心跳静默上报、**授权被收回时的中止提示** | 课时点击 |
...
1. 全年可用性目标 **99.9%**（年不可用 ≤ 8.76 小时）；发布采用滚动/灰度，避免停机。
   〔**当前为单实例部署**（`05-工程结构.md` §J1），发布需重启服务、**单次停机数分钟**，且因 Sa-Token 会话存在 **JVM 进程内存**（§J2 ②）会导致**全员重新登录**。滚动/灰度的前置条件是 §J2 那张清单全部处理完毕（会话外置到 Redis 等）。**99.9% 这个数字本身不改**——经核算它在单实例下算术上可达：systemd `Restart=on-failure` 自动拉起，加上每周发版一次、单次约 1 分钟，约 52 分钟/年，而 99.9% 的预算是 525 分钟/年。**问题不在数字，在于没有冗余余量**：一次需要人工介入的故障（比如要改配置再重启）就可能吃掉几十分钟，而单实例没有第二个副本兜着。已登记为 `04-实施计划.md` §E 的 **F-32**。〕
...
4. 逻辑删除统一走 `DELETE` 方法（后端将 `deleted_at` 由 0 置为当前毫秒时间戳）。
...
> **本节写的是可测的功能需求，不是合规声明。** 合规声明在契约 §7.2；那里的五条中有三条实质上是功能需求，只是被写在合规语境里，导致**没有任何人负责测**。本节把它们各自补上验收标准——写不出验收标准的条款等于没有实现。

`org_student` 收集**未成年人姓名、手机号与监护人手机号**（`guardian_name` / `guardian_phone`），落在《个人信息保护法》第 31 条"不满十四周岁未成年人个人信息"的敏感个人信息范畴。

**F7-1 监护人同意留痕**

学员建档（含 Excel 批量导入）时必须记录"机构已确认取得监护人同意"的操作人与时间戳。机构线下签署的知情同意书由机构自行留存，系统侧只承担**留痕**责任。

- Given 管理员通过 Excel 导入 100 名学生，When 任务完成，Then `org_import_task` 中记录了确认监护人同意的操作人（`create_by`）与时间戳（`create_time`），且该记录**不可编辑**（导入任务表无更新入口）。
- Given 管理员单个创建学生且未勾选"已取得监护人同意"，When 提交，Then 返回 `400`，学生未被创建。

**F7-2 最小必要与水印脱敏** —— **水印部分第一版不做（F-114）**

> **⚠ F-114：水印第一版不做**，本节的水印脱敏规则随之第一版不生效（`watermark_phone_mask` 不被消费）。
> **「最小必要」那一半不受影响，第一版照常适用。** 规则原样保留待启用。

不收集与教学无关的信息（住址、身份证号、人脸、精确位置一律不采集）。

- Given 租户**未修改任何配置**，When 学生在 H5 端播放视频，Then 跑马灯水印展示"姓名 + 手机号中间四位打星"（`李子墨 138****5678`），**完整手机号不出现在任何 DOM 节点、也不出现在任何接口响应中**。
- Given 租户把 `watermark_phone_mask` 置为 `0`，When 学生播放，Then 水印**仍然脱敏**——该配置对学生端无效（契约 §7.2 第 2 条）。
- Given 同一租户配置下，When **管理员**预览同一课时，Then 水印按 `watermark_phone_mask` 显示完整手机号——此时被水印的是机构员工本人，不涉及未成年人信息。

**F7-3 删除请求：归档 + 30 日不可逆脱敏**

监护人有权要求删除。系统路径是"因删除请求归档 → 30 日撤回窗口 → 不可逆脱敏"。
...
- 视频经**阿里云 VOD**（单一受托方）存储与分发，须在隐私政策中**具名**列出。
- 存储区域仅中国大陆、加速域名的加速区域仅中国内地，二者均为上线前置核对项，变更须走合规评审（契约 §1 部署约定）。
...
| B2 | **非法父子类型组合** | 教师节点下建管理员/教师 → 10105；学生节点下建任何节点 → 10106；其余非法组合 → 10104。教师无学员时其自身为叶子，合法 |
...
| B20 | **重复授权同一资源给同一节点** | 命中 `UK(resource_type, resource_id, target_node_id)` → **10303**。~~调整有效期走"修改授权有效期"接口~~ —— 授权已无有效期（需方 2026-08-21 定案），接口 40 已删除，重复授权只有一种含义（已经授过了） |
| B21 | **撤销授权时学员正在学习该课程** | ~~撤销即时生效~~ **撤销从「下一次点开视频」起生效**（需方 2026-08-21 定案三，F-106；心跳与凭证续签**都不重判授权**，正在播放的那个视频可以看完），级联至整棵子树。~~正在播放的学员在**下一次心跳或凭证续签**时被拒（20013/20001），前端提示"课程授权已被收回"并停止播放~~；**已累计 watched_duration 与 max_position 不清零**，已完成课时保持 watch_status=2；`vod_watch_progress`、答卷、错题本一律保留 |
...
| B37 | 视频转码失败（video_status=3） | 管理端媒资列表红色标记 + 工作台待办计数（按 upload_user_id 归属）；**不建消息表**；引用该视频的课时禁止置为可见（20008）；提供重新转码/重传入口；不影响其他课时 |
...
| B43 | 学生跨设备同时播放同一课时 | **由 F2-7 校验规则 9「同课时会话择一」处置**：后开的会话返回 **20020**，前端**暂停播放并明确提示**"该课时正在其他设备播放"；原会话静默 60 秒后新会话自动接管，并重置推进校验基准。**F-113 后本条的理由已更换**（旧理由「永久归零」依赖规则 6 会拒绝心跳，而规则 6 已降级为只留痕）：现在的理由是 ① `sessionPlayedSeconds` 的累计值需要会话作用域才算得出 `delta`；② 两端同时播放会把同一段真实时间**计两次**。接管时须**同时重置推进基准与累计值基准（置 0）** |
