...
4. `UK` 保证同一资源对同一节点只有一行；重复授权返回 **10303**。**修改有效期走独立的"修改授权有效期"接口**（02-组织机构接口 40 修改授权有效期），既不重复插入、也不走"撤销+重授"——撤销强制级联整棵子树，用它来续期会连带清空下级全部授权。缩短有效期时子树内晚于新值的授权行一并截断（防时间维度悬挂），延长则不级联。
...
| B20 | **重复授权同一资源给同一节点** | 命中 `UK(resource_type, resource_id, target_node_id)` → **10303**；调整有效期走"修改授权有效期"接口（02-组织机构接口 40 修改授权有效期），不得靠重复插入，更不得靠"撤销+重授"——撤销级联子树 |
