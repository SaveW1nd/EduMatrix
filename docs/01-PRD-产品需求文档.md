...
2. **间隔校验（F-116 按 `trigger` 分流）**：**仅对 `trigger = tick` 的周期心跳**校验间隔——以"本次心跳与该学生该课时**上一次被采纳心跳**的服务器接收时间差"为实际间隔，间隔 ≥8s 有效，**过短判异常按规则 5 处理**。**`trigger ≠ tick` 的事件心跳一律采纳，不做任何间隔限制。**异常明细每会话每分钟最多写 20 条，超出后仍拒绝但不再写明细，精确条数看指标 `heartbeat_reject_total{rule}`。基准是"上次被采纳"而非"上次收到"，这一点决定了多端并发下的行为（见规则 9）。
