...
1. **看得住**：禁快进 + 心跳校验 + 服务端间隔合理性校验，**使刷课不快于真实播放，并全量留痕**，完播数据真实可信。（措辞不用"杜绝"是因为做不到：脚本只要让 `currentTime` 每次按墙钟推进就能通过推进一致性校验——那恰恰是最容易伪造的部分。真正兜住上限的是**时长封顶**（单日 10h、单课时 3×duration）与**倍速不加速计时**，它们保证刷课的收益率不高于老老实实播放；推进一致性校验防的是完全不推进的空跑脚本。）
...
| 前端视频播放与防刷 | **Aliplayer**（~~DPlayer~~，随加密路线改为阿里云私有加密，见契约 §1 订正段与 04 §E F-112） | 播放器控制、禁用拖拽/快进控制、播放进度事件监听与定时心跳触发 | 模块 2（前端） |
...
**功能描述**：**Aliplayer**（~~DPlayer~~）二开实现禁快进：以 `vod_watch_progress.max_position`（最远触达秒）为界——**maxPosition 之前的区间允许自由拖拽（重播已看部分），maxPosition 之后禁止向前拖拽**。完播状态机：0未开始 → 1学习中 → 2已完成。
...
5. 前端禁用倍速 >1 的选项不作硬限制（允许 UI 提供倍速），但计时不受倍速影响（见 F2-7 **校验规则 4** 倍速不加速计时）。
...
Body: {"lessonId":"1953827104412590081","videoId":"1953827104412590082","currentTime":123.4,"duration":600,"playRate":1.0,"seeked":false,"sessionId":"3d5b81c04e7a4a92b6f0c8e2d1a97b53"}
...
2. **间隔校验**：以"本次心跳与该学生该课时**上一次被采纳心跳**的服务器接收时间差"为实际间隔——间隔过短意味着伪造加速或重复请求，判异常按规则 5 处理。基准是"上次被采纳"而非"上次收到"，这一点决定了多端并发下的行为（见规则 9）。
3. **拖拽校验**：`currentTime` 不得超过 `max_position` 加一个单周期的最大合法推进量；超出视为绕过前端"禁用快进"的非法拖拽或报文篡改，按规则 5 处理。**本条对 `seeked=true` 的心跳同样生效**——`seeked` 解的是"向后拖回复看被误判"，本条拦的是"向前跳过没看的内容"，方向相反，放开等于绕过禁快进（F2-5）。
4. **倍速不加速计时**：`playRate` 仅作记录与风控参考，时长按真实墙钟间隔计，2 倍速看 10 分钟视频仍只累计约 5 分钟。**本条不拒绝任何心跳**，只约束计时口径。
5. **异常心跳丢弃**：凡未通过规则 2 / 3，或参数明显非法（`currentTime` 超出 `[0, duration+2]`、`duration` 与媒资实际时长偏差过大、`playRate` 超出合法区间等），一律**不计入时长**并返回 **20002**；明细仍写 `vod_heartbeat_log` 供风控回溯。
6. **推进一致性校验**：**仅在连续两次 `seeked=false` 的心跳之间**校验——有效心跳的位置推进量须与墙钟一致；长期零推进而间隔正常（期间无任何 `seeked=true`）判定为挂机或纯脚本上报，只留痕不计时长，并计入 `vod_heartbeat_log.reject_rule`，由风控报表按 `(student_id, lesson_id, 时间窗)` 聚合呈现。不设此条时，脚本无需下载任何视频分片、仅按 10s 节奏发包即可 1:1 刷满时长。
   **`seeked` 字段的由来**：F2-5 规则 1 明确允许把进度条拖回 `max_position` 之前自由复看，本节**处理流程 2** 也规定复看同样计时长。但学生从 500 秒拖回 120 秒后，下一次心跳的位置推进量为负数且绝对值很大，必然落在校验区间之外——**一个被明确允许的行为会被持续判为作弊且不计时长**；暂停后原地续播同理。故前端在 seek 或暂停恢复后的第一次心跳置 `seeked=true`，服务端据此重置校验基准：本次不做推进判定，但**正常计时**。
   **`seeked` 自身必须限频**：连续 `seeked=true` 与单课时每小时 `seeked=true` 均有次数上限，超出按规则 5 丢弃，并计入 `vod_heartbeat_log.reject_rule`，由风控报表按 `(student_id, lesson_id, 时间窗)` 聚合呈现。否则脚本每次都置 `true` 即可永久关闭本条校验，挂机成本重新归零。
...
8. **时长封顶**：单学生单日累计与单课时累计均有上限，超出部分只记录、不计入完播判定，并计入 `vod_heartbeat_log.reject_rule`，由风控报表按 `(student_id, lesson_id, 时间窗)` 聚合呈现供机构核查。
9. **同课时会话择一**（多端并发）：F2-4 发凭证时下发 `sessionId`（续签不变），心跳携带；服务端按学生 + 课时维护当前活跃会话——同会话正常计时；不同会话且当前会话仍活跃时返回 **20020**，前端**暂停播放并明确提示**"该课时正在其他设备播放"；当前会话超过静默期无心跳（关页面/断网/换设备）则新会话自动接管，并**重置规则 6 的推进基准**。
   **不做会话择一时的真实后果是永久归零，不是"变相单端观看"**：规则 2 按"上一次被采纳心跳"算间隔，两端交替时确实只有一端被采纳、时长满速率累计——但被采纳的那端一停播，另一端的心跳虽能过规则 2，其位置推进量却是相对**前一端最后位置**算的（两端播放位置无关，动辄相差数百秒），必然被规则 6 判为位置跳变而拒绝；**被拒的心跳不推进基准**，下一次仍以同一个陈旧位置为基准，就此锁死。学生只是在手机上多开了一个标签页，此后一小时学习时长累计约等于 0，且前端没有任何提示——用户只会认为系统在吞数据。
...
1. **前端触发时机**：仅在真实播放（playing 且未暂停、页面可见）状态下每 10 秒触发；暂停、卡顿缓冲、页面切后台（Page Visibility 隐藏）时停止心跳。
2. **单次计入时长** = min(实际间隔, 15) 秒，累加进 `watched_duration`。8~15 秒为正常窗口全额计入；>15 秒（中途暂停/切后台/断线恢复）仍有效但单次封顶计入 15 秒，超出部分视为空窗不补算、不报错。**墙钟有效累计：复看已看区间同样计时长**——`watched_duration` 语义为"累计学习投入时长"，完播判定用它对比 `duration ×` 阈值。
3. **`max_position` 取大**：`max_position = max(当前值, 本次 currentTime)`，仅被采纳的心跳可推进；复看不回退（F2-5 规则 2）。
4. **写路径与完播判定**：被采纳的心跳更新 Redis（key：`vod:hb:{studentId}:{lessonId}`，含 watchedDuration / maxPosition / lastHeartbeatTime）→ XXL-Job 每 60s 批量 upsert `vod_watch_progress`（UK student_id + lesson_id）→ **完播判定在落盘时执行**（唯一判定时机，见 F2-9 规则 1）并回填 `watch_status` / `complete_time`。
...
- Given 学生把进度条从 500 秒拖回 120 秒复看，When 拖动后第一次心跳以 `seeked=true` 上报 `currentTime=120`，Then 该心跳**有效并正常计入时长**，不因 `Δ currentTime = -380` 被判异常；此后 `seeked=false` 的心跳恢复推进一致性校验，`max_position` 保持 500 不回退。
- Given 脚本连续 3 次以 `seeked=true` 上报，When 服务端接收第 3 次，Then 返回 code=20002 丢弃，且 `vod_heartbeat_log` 落一行 `reject_rule=6`（连续 `seeked=true` 上限 2 次）。
- Given 学生在电脑上播放课时 L（会话 S1）后，又在手机上打开同一课时（会话 S2），When S2 的心跳到达且 S1 在 60 秒内仍活跃，Then S2 返回 **20020**，手机端暂停并提示"该课时正在其他设备播放"；电脑端不受影响，继续正常计时。
- Given 学生关闭电脑（S1 停止心跳）满 60 秒后，手机端 S2 继续上报，Then S2 **自动接管**为活跃会话，推进校验基准随之重置，此后手机端正常计时——**不会因两端播放位置相差数百秒而被规则 6 永久拒绝**。
- Given 同一学生同一课时的同一会话内因网络重试收到重复心跳，When 两条心跳间隔 <8 秒，Then 后一条按**校验规则 5** 返回 20002 丢弃——**时长不会翻倍**；间隔基准是"上一次被采纳"而非"上一次收到"，故重复报文不会把有效心跳的基准顶走。（**跨设备**的并发不走本条，先由**校验规则 9** 以 20020 拦下。）
...
| B43 | 学生跨设备同时播放同一课时 | **由 F2-7 校验规则 9「同课时会话择一」处置**：后开的会话返回 **20020**，前端**暂停播放并明确提示**"该课时正在其他设备播放"；原会话静默 60 秒后新会话自动接管，并重置推进校验基准。**不可退回"并发端被 20002 丢弃、仅一端累计"的旧结论**——那条路的真实后果是永久归零而非"变相单端"：被采纳的那端一停播，另一端的位置推进量是相对前一端最后位置算的，必然被规则 6 判为跳变而拒绝，被拒的心跳又不推进基准，就此锁死，学生此后一小时时长累计约等于 0 且前端毫无提示 |
