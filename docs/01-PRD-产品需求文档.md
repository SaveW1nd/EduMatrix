...
EduMatrix 是面向教育机构、学校及企业培训部门（ToB）的**私域督学管理系统**。区别于面向 C 端的公开课平台，本产品的核心是"一对一/一对多的私域监管式学习"：机构买单、导师执行、学生被监管、**学生之间互不可见**。平台通过~~禁快进~~（**F-113 取消**）、跑马灯水印、心跳进度追踪、作业自动判卷与数据看板，把"学生是否真的学了、学到什么程度"变成机构与导师可量化、可追责的数据。
...
1. **看得住**：心跳进度追踪 + 跑马灯水印 + **全量留痕**（`vod_heartbeat_log` 按月分区留 6 个月）。
...
2. 转码模板强制：**单一清晰度**的加密 HLS 输出（~~AES-128 标准加密~~ → **阿里云私有加密 `AliyunVoDEncryption`**，见契约 §1 订正段；**「单一清晰度」与「禁止 MP4」两条不受影响**），禁止生成可直接下载的 MP4 播放地址。**不支持清晰度切换**——`vod_video.hls_url` 是单值列，配多档转码会让学生实际看到的清晰度不可控；要支持切换须先改数据模型，属后续需求（契约 §1 部署约定）。
