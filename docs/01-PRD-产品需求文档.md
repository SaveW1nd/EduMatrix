...
Body: {"lessonId":"1953827104412590081","videoId":"1953827104412590082","currentTime":123.4,"duration":600,"playRate":1.0,"seeked":false,"sessionId":"3d5b81c04e7a4a92b6f0c8e2d1a97b53","sessionPlayedSeconds":118}
...
2. **间隔校验（F-116 改为配额制）**：以"本次心跳与该学生该课时**上一次被采纳心跳**的服务器接收时间差"为实际间隔；间隔 ≥8s 直接有效；**间隔过短不再直接判异常，改为消耗配额**——每会话每分钟 6 条，配额内有效，**用尽才按规则 5 处理**。基准是"上次被采纳"而非"上次收到"，这一点决定了多端并发下的行为（见规则 9）。
   ...
   > **F-116（需方 2026-08-22）把硬阈值改成了配额。** F-113 之后前端除周期心跳外，还会在**停下来 / 恢复 / 离开页面**三类时刻各发一条**事件心跳**，而事件心跳本来就不看表。硬 8 秒闸会把它们大量判为异常 ——**真机实测 23 条丢 12 条，前端 76.3 秒而服务端入库 69.3 秒，差的 7.0 秒丢在最后一条上，它之后没有心跳来补**。改配额之后：周期心跳天然 ≥8s、永不碰配额；事件心跳走配额；高频空包迅速耗尽配额被拒，限流目的不变。**服务端不需要分辨心跳类型，只看它来得早不早**——签名固定，加字段与之冲突。
