...
1. **看得住**：心跳进度追踪 + **全量留痕**（`vod_heartbeat_log` 按月分区留 6 个月）。~~跑马灯水印~~ **F-114：第一版不做**。
   ...
   > **禁快进已废止**；**时长封顶保留但理由改为「兜前端计时器 bug」**；**倍速不加速计时保留且比以前更要紧**（计时挪到前端后，它是错误实现方式的唯一约束）。
...
4. **倍速不加速计时**：`playRate` **不参与计时、也不落库**（`vod_heartbeat_log` 没有这一列），只在当场判定时用（规则 5 校验范围、规则 6 算区间）；时长按**真实墙钟**计，2 倍速看 10 分钟视频仍只累计约 5 分钟。**本条不拒绝任何心跳**，只约束计时口径。
...
8. **时长封顶（F-113 理由改写）**：单学生单日累计与单课时累计均有上限，超出部分只记录、不计入，并计入 `vod_heartbeat_log.reject_rule`。
   > **理由由「防刷」改为「兜 bug」**：前提已取消（不防刷课）。本条现在防的是**前端计时器算错**——浏览器冻结后台标签页定时器，页面醒来时写得糙的计时器可能一口气报出几小时。**那不是作弊，是 bug**，而它会污染报表。
