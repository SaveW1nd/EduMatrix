...
2. 转码模板强制：**单一清晰度**的 HLS 输出，禁止生成可直接下载的 MP4 播放地址。
   > **⚠ F-114（2026-08-21 需方定案）：第一版【不加密】。** 加密方式的两次订正（~~AES-128 标准加密~~ → **阿里云私有加密 `AliyunVoDEncryption`**，契约 §1 订正段）**原样保留待启用**，但**第一版模板配成不加密输出**、`vod_video.encrypt_type` 写 `0`。
   > **「单一清晰度」与「禁止 MP4」两条不受影响，第一版就要照做** —— 它们与加不加密无关，而且**现在配对，将来打开加密只是加一个开关；模板结构变了才要重转**。
   > **⚠ `encrypt_type` 是事实记录不是意图声明**：必须与模板实际产出的东西一致，写错的表现是「播不了」（模块 12 的 play-auth 按它决定下发给 Aliplayer 的 `encryptType`）。**不支持清晰度切换**——`vod_video.hls_url` 是单值列，配多档转码会让学生实际看到的清晰度不可控；要支持切换须先改数据模型，属后续需求（契约 §1 部署约定）。
