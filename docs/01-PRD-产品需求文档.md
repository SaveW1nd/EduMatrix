...
6. **推进一致性校验**：**仅在连续两次 `seeked=false` 的心跳之间**校验——有效心跳的 `Δ currentTime` 须落在 `[0.5, 1.5] × Δt × playRate` 区间；连续 5 次位置零推进、间隔正常**且期间无任何 `seeked=true`**，判定为挂机或纯脚本上报——只留痕不计时长并累加风控分。（不设此条时，脚本无需下载任何视频分片、仅按 10s 节奏发包即可 1:1 刷满时长。）
   **`seeked` 字段的由来**：F2-5 规则 1 明确允许把进度条拖回 `max_position` 之前自由复看，本节规则 3 也规定复看同样计时长。但学生从 500 秒拖回 120 秒后，下一次心跳的 `Δ currentTime = -380`，必然落在上述区间之外——**一个被明确允许的行为会被持续判为作弊且不计时长**；暂停后原地续播同理。故前端在 seek 或暂停恢复后的第一次心跳置 `seeked=true`，服务端据此重置校验基准：本次不做推进判定，但**正常计时**。
   **`seeked` 自身必须限频**：连续 `seeked=true` 不超过 2 次、单课时每小时不超过 20 次，超出按异常丢弃并计风控分。否则脚本每次都置 `true` 即可永久关闭本条校验，挂机成本重新归零。
   **规则 3 的拖拽上限不受 `seeked` 影响**：`currentTime ≤ max_position + 15` 对 seek 心跳照常生效——`seeked` 解的是"向后拖回复看被误判"，拖拽上限拦的是"向前跳过没看的内容"，两者方向相反，放开后者等于绕过禁快进。
...
9. currentTime 超出 [0, duration+2] 的心跳判非法丢弃。
   **同一学生同一课时的多端并发走"会话择一"**：8.1 发凭证时下发 `sessionId`（续签不变），心跳携带；服务端按学生+课时维护当前活跃会话——同会话正常计时；不同会话且当前会话 **60 秒**内仍活跃时返回 **20020**，前端**暂停播放并明确提示**"该课时正在其他设备播放"；当前会话超 60 秒无心跳（关页面/断网/换设备）则新会话自动接管，并重置推进校验基准。
   **不做会话择一时的真实后果是永久归零，不是"变相单端观看"**：规则 2 按"上一次被采纳心跳"算间隔，两端交替时确实只有一端被采纳、时长满速率累计——但被采纳的那端一停播，另一端的心跳虽能过间隔校验，其位置推进量却是相对**前一端最后位置**算的（两端播放位置无关，动辄相差数百秒），必然被规则 6 判为位置跳变而拒绝；**被拒的心跳不推进基准**，下一次仍以同一个陈旧位置为基准，就此锁死。学生只是在手机上多开了一个标签页，此后一小时学习时长累计约等于 0，且前端没有任何提示——用户只会认为系统在吞数据。
   **被拒绝的心跳一律不推进任何基准**（时间基准、位置基准、max_position）；唯一例外是会话接管时必须重置位置基准，否则新设备第一条心跳即被判跳变，接管等于没发生。
...
- Given 同一学生在两台设备上并行播放同一课时，When 两端心跳交替到达，Then 时长按"距上一次**被采纳**心跳的间隔"累计，先到者被采纳、后到者因间隔 <8 秒返回 20002——**时长不会归零，也不会翻倍**（间隔基准是上次被采纳而非上次收到，故不存在两端互相把对方顶掉的情况）。
