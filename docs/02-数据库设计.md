...
| **软删除（逻辑删除）** | 核心业务数据禁止物理删除，统一置 `deleted_at = 1`。唯一索引与软删的冲突采用【方案A：唯一索引末尾追加 `deleted_at`】，详见 §7。日志/心跳明细表（`sys_login_log`、`sys_oper_log`、`vod_play_auth_log`、`vod_heartbeat_log`）按契约例外允许按时间/分区物理归档；其中仅 `vod_heartbeat_log` 不带 `deleted_at`。 |
...
2. **软删除语义冲突**：本系统"删除"是 `deleted_at=1`，父行逻辑删除后子行仍物理存在，物理外键的 `RESTRICT/CASCADE` 语义与之天然矛盾；
...
3. **撤销是逻辑删除**：置 `deleted_at=1` 而非物理删除，保留"谁在什么时候撤了谁的权"的审计链；注意唯一索引 `uk_resource_target` 末尾含 `deleted_at`，同一业务键**最多容纳一条已删除记录**，因此重新授权时应走 `INSERT ... ON DUPLICATE KEY UPDATE deleted_at=0`（复用原行）而不是插新行，详见 §7。
...
SELECT ..., x.id, #{statDate}, COUNT(*),
 ...
 WHERE x.node_type IN (1, 2) AND x.deleted_at = 0 AND x.tenant_id = #{tenantId}
 GROUP BY x.id;
```
...
| deleted_at | TINYINT | N | 0 | 逻辑删除：0否 1是 |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_role_menu | (role_id, menu_id, deleted_at) | 同一角色不可重复绑定同一菜单（追加 deleted_at 兼容逻辑删除） |
...
| tenant_id | BIGINT | N | 0 | 租户（机构）ID |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| deleted_at | TINYINT | N | 0 | 逻辑删除：0否 1是（日志业务恒为 0，清理走物理归档） |
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| deleted_at | TINYINT | N | 0 | 逻辑删除：0否 1是（child_count>0 时禁止删除，须先移走子节点） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_ref_user_id | (ref_user_id, deleted_at) | 一个账号在树上仅占一个节点（用户→节点反查亦走此索引最左前缀；机构节点 ref_user_id 为 NULL，MySQL 唯一索引不约束 NULL 故可多条并存） |
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| deleted_at | TINYINT | N | 0 | 逻辑删除：0否 1是（节点仍有子节点即名下仍有学员时禁止删除，须先移走学员） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| deleted_at | TINYINT | N | 0 | 逻辑删除：0否 1是（学习记录禁止随之删除） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| deleted_at | TINYINT | N | 0 | 逻辑删除：0否 1是（轨迹业务上禁止删除，恒为 0） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| deleted_at | TINYINT | N | 0 | 逻辑删除：0否 1是（撤销授权=置 1，级联回收对子树内同资源行批量置 1） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| deleted_at | TINYINT | N | 0 | 逻辑删除：0否 1是（删除模板不影响已生成的授权行） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
用途：机构内课程的定义主表，承载课程名称、封面、科目、简介与上架状态，并冗余课时总数与视频总时长供列表页直读。`owner_node_id`（归属节点，创建时写入创建者所在节点），它是契约 §2.5 的**授权前提**——只有 owner 本人、或已被显式授权且在有效期内的节点，才能把该课程再向下授权。课程是三类**受管资源**之一（`resource_type=1`），对节点与学生的授权统一走 `org_resource_grant`，逐级显式、无继承，本表不设配套授权表。授权模型细节见主文档 §3.3 专栏。课程禁止物理删除，一律 `deleted_at=1`。
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| deleted_at | TINYINT | N | 0 | 逻辑删除：0否 1是（课程禁止物理删除） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| deleted_at | TINYINT | N | 0 | 逻辑删除：0否 1是（课时禁止物理删除） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
用途：每次发放播放凭证（PlayAuth／Key）的审计流水，用于排查取证频次异常与凭证盗用。`expire_time` 是错误码 20001（凭证过期）的校验依据，`auth_token` 脱敏截断后存储。本表按契约 2.2 的日志例外精简 `update_by` / `remark` / `create_by`（写入方为系统，创建人即请求学生本人，记于 `student_id`），仍携带 `deleted_at`（业务上恒为 0），清理走按时间物理归档而非逻辑删除。
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| deleted_at | TINYINT | N | 0 | 逻辑删除：0否 1是（题目禁止物理删除） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| deleted_at | TINYINT | N | 0 | 逻辑删除：0否 1是（版本快照业务上禁止删除，恒为0） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| deleted_at | TINYINT | N | 0 | 逻辑删除：0否 1是（作业禁止物理删除） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
用途：作业的分发对象清单，**全量精确到学生**。批量入口不落库为"批量行"，而是在服务层展开为逐个学生行，再由 `grant_source` + `source_ref_id` 记录来源以支持整批撤回与追溯。`grant_source` 枚举为 **1手动选择 2按节点批量 3按标签批量 4按名下全体**——作业分发**不使用 5按权限模板**（模板只管课程/题目/视频三类受管资源）。展开时目标学生必须落在分发者子树内（契约 §2.4），标签仅用于筛选，不得扩大可见范围；请求体中显式指定的学生越界时返回业务码 10107 而非静默忽略。撤回分发走逻辑删除（`deleted_at=1`）。
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| deleted_at | TINYINT | N | 0 | 逻辑删除：0否 1是（撤回分发走逻辑删除） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| deleted_at | TINYINT | N | 0 | 逻辑删除：0否 1是（答卷禁止物理删除） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| deleted_at | TINYINT | N | 0 | 逻辑删除：0否 1是（答卷明细禁止物理删除） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| 业务表（deleted_at=1 的软删数据） | 永久在线 | 不清理 | 契约禁止物理删除；确需瘦身时仅可迁移至同构归档库并保留主键 |
...
| **稳态合计** | | **~4.1 万** | **~410 万** |
| **3 年累计**（含撤销留存的 `deleted_at=1` 行、人员流动重授、模板重复应用） | 稳态 × 2~2.5 | **~8~10 万** | **~800~1000 万** |
...
**问题**：业务表禁止物理删除，仅置 `deleted_at=1`。若唯一索引不含 `deleted_at`（如 `uk_username(username)`），软删一条记录后，再新建同业务键的记录会触发唯一冲突——被"删除"的数据挡住了新数据。
...
- 软删后（`deleted_at=1`），可再插入一条同业务键的存活记录，互不冲突；
- UPSERT（`INSERT ... ON DUPLICATE KEY UPDATE`，如心跳落盘、日汇总、资源授权续期）继续依赖唯一索引幂等命中存活行。

**已知局限与约束**：同一业务键**最多容纳一条已删除记录**（两条 `deleted_at=1` 的同键记录仍会唯一冲突）。因此：若需再次删除同键记录，应用层须先将旧的已删除记录归档转移，**或恢复复用原记录**（把旧记录改回 `deleted_at=0` 并更新内容，不再插入新行）。该约束由服务层统一封装（删除入口先探测同键已删记录），DDL 不再额外处理。

**特别提示：`org_resource_grant` 必须走"复用原行"路径**。资源授权是**高频反复授予/撤销**的对象（转导师、模板重复应用、有效期续期），若每次重新授权都插新行，很快会因"已存在一条 `deleted_at=1` 的同键行"而唯一冲突。正确写法是重新授权时用 UPSERT 把原行复活：
