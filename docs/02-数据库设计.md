...
**因此结论收敛为**：目标规模（100 机构 × 1 万学生）下**仍不需要分表**，单表 1 亿行配 `idx_target_resource` 的点查依然是毫秒级；但**必须纳入压测范围**，且容量预警线设在**单机构 100 万行 / 全平台 1.5 亿行**——达到该线时先做冷热分离（把 `deleted_at != 0` 的历史撤销行迁到归档表），再考虑按 `tenant_id` 分片。原先"可以忽略不计"的措辞会让人在压测时直接跳过这张表，那才是真正的风险。
...
5. **目标规模下不需要分表，但必须纳入压测**。100 机构 × 1 万学生（§6.1 基础假设）3 年累计约 1 亿行、约 25 GB——单表 1 亿行配 `idx_target_resource` 的点查仍是毫秒级，但 25 GB 已超出常见 16~32 GB buffer pool 的从容区间，热点索引页会开始换出。**容量预警线：单机构 100 万行 / 全平台 1.5 亿行**；达线时先做冷热分离（`deleted_at != 0` 的历史撤销行迁归档表），再考虑按 `tenant_id` 水平拆库。**不要再写"可以忽略不计"**——那句措辞会让人在压测计划里直接跳过这张表，而它是心跳之外命中最频繁的鉴权表。
