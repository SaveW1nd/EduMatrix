...
用途：逐条心跳原始流水，是防刷审计与进度争议回溯的唯一依据，可按学生×课时回放完整心跳序列。有效性规则与 `vod_watch_progress` 一致：`interval_sec ≥ 8s` 视为有效，单次计入 `min(实际间隔, 15)` 秒，>15s 封顶但不报错。`reject_rule` 记录本条心跳的判定结果（0 采纳，**实际取值 2/5/6/7/9**；冻结基线的列注释写的是 2/3/5/6/7/8/9，**3 已随 F-113、8 已随 F-117 废止留墓碑**，以本节字段表为准）——**此前本表没有任何一列表达"算没算、为什么没算"**，只能从 `interval_sec < 8` 反推规则 2，规则 6/7/9 的拒绝完全不可追溯，而"学生说学了一小时系统只记了十分钟"正是最典型的争议场景。1 字节换来全链路可回放。
