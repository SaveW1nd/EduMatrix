...
用途：逐条心跳原始流水，是防刷审计与进度争议回溯的唯一依据，可按学生×课时回放完整心跳序列。有效性规则与 `vod_watch_progress` 一致：`interval_sec ≥ 8s` 视为有效，单次计入 `min(实际间隔, 15)` 秒，>15s 封顶但不报错。`reject_rule` 记录本条心跳的判定结果（0 采纳，1~9 对应被哪条规则拒绝）——**此前本表没有任何一列表达"算没算、为什么没算"**，只能从 `interval_sec < 8` 反推规则 2，规则 3/6/7/8/9 的拒绝完全不可追溯，而"学生说学了一小时系统只记了十分钟"正是最典型的争议场景。1 字节换来全链路可回放。**权衡说明**：不记 `sessionId`（规则 9 的会话标识），因为 `CHAR(32)` 在常驻 0.95 亿行上多占约 3 GB（+25%），而排查真正需要回答的是"为什么被拒"，不是"哪个会话"。
...
| reject_rule | TINYINT | N | 0 | 本条心跳的判定结果：0=采纳并计时；1~9=被 §8.2.1 第 N 条规则拒绝。此前本表没有任何一列记录"这条算没算、为什么没算"——只能从 `interval_sec<8` 反推规则 2，其余规则的拒绝完全不可追溯 |
