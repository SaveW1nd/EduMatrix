...
> 本文档与 `backend/src/main/resources/db/migration/V202608120000__baseline.sql` 逐字段一致，与 `docs/DESIGN-CONTRACT.md`（设计契约）共同构成数据层唯一权威。若三者出现出入，以设计契约为准，先改契约再改本文与 DDL。
>
>
...
| **多租户隔离** | 机构 = 租户，机构根节点的 `id` 即该租户的 `tenant_id`。除 `sys_menu`、`sys_tenant` 平台级表外所有表带 `tenant_id`（共 39 张），MyBatis-Plus 租户插件在 SQL 层自动注入过滤；跨租户访问一律 404（不暴露存在性）。**例外：`sys_role` 与 `sys_role_menu` 的注入条件是 `(tenant_id = ? OR tenant_id = 0)`**——四个内置角色及其菜单绑定是全租户共用的定义，按等式过滤会让所有非超管用户零权限（契约 §2.9）。其余承载 `tenant_id = 0` 行的表（`sys_user` / `sys_user_role` / `sys_file` / `sys_login_log` / `sys_oper_log` / `org_node` 哨兵）一律严格过滤，超管读自己的行靠"插件对超管整体放行"这条独立通道。租户隔离是硬边界，与子树权限是**两道独立防线**，不得互相替代。 |
...
| **异步削峰** | 10 秒心跳属高频写：先写 Redis（`vod:hb:{studentId}:{lessonId}`），XXL-Job 每 60s 批量 UPSERT 落盘 `vod_watch_progress`，明细异步追加进按月分区的 `vod_heartbeat_log`。导入（`org_import_task`）与导出（`stat_export_task`）同为异步任务表模式。**转码事件也走同一调度器**：XXL-Job 每 10s 拉取阿里云轻量消息队列并消费（03-课程与视频 §7.2），取代了原先的免登录 HTTP 回调端点——拉模式没有 5 秒超时约束，得以把"引用该视频的 N 个课时 `duration` 冗余 + 各自课程 `total_duration`"这串扇出刷新拆成独立的异步任务，不与状态推进挤在同一个事务里。 |
...
3. **子树规模与锁持有时间**：单租户全树上限约 1.1 万行（每人一节点）。第 5 步 UPDATE 的实测数据与它成立的**环境条件**见下——原文只写了「实测万级行内 100ms 量级」而没写在什么环境上测的，**换个环境会差一个数量级**。

   **R2 压测实测（2026-08-19，11000 节点树、深度 6~8 层、每档 20 次）**：

   | 环境 | 6481 行子树·事务 P50 | 事务 P99 | 第 5 步 P50 | 每行 |
   | --- | --- | --- | --- | --- |
   | **阿里云 RDS MySQL 8.0.36**（生产拓扑：ECS 与 RDS 同 VPC，TCP 建连 P50 1.48ms） | **229.7 ms** | **243.7 ms** | 185.4 ms | **28.6 µs** |
   | 本地 Docker MySQL 8.0.46（开发机 macOS） | 3709.5 ms | 4554.3 ms | — | 572 µs |

   **两者相差约 16 倍，而代码、树形、durability 参数完全相同**
   （两边都是 `innodb_flush_log_at_trx_commit=1` + `sync_binlog=1`）。差异来自环境本身：
   **`innodb_buffer_pool_size` 128 MB vs 1.5 GB（12 倍）**，以及
   **`innodb_flush_method` `fsync` vs `O_DIRECT`** 叠加 macOS 上 Docker 的虚拟化 IO 路径。
   **「100ms 量级」这个结论在生产级实例上成立（229ms，同量级），在开发机 Docker 上不成立** ——
   引用这句话时必须连环境条件一起引。

   **选路实测（同一条 UPDATE，三档 EXPLAIN）**：
   ...
   **最后一行不是缺陷**：子树占到全表 59% 时，「索引区间 + 6481 次随机回表」比「按主键顺序扫一遍」更贵，
   优化器选主键全扫是对的。**前两档确实走 `idx_ancestors`**，把 `LIKE` 那一支单独拿出来测同样是
   `range` / `idx_ancestors`（252 行）。`Using temporary` 是 MySQL 为自引用 UPDATE 先物化命中行
   （避免 Halloween 问题），不是排序开销。

   若某机构子树超 5 万行，应改为**分批 UPDATE（每批 5000 行，同一事务内循环）**，避免单条 UPDATE 长时间持有大量行锁。
   **⚠ 注意分批能解决什么**：InnoDB 行锁**持有到事务提交**，所以同事务内分批**基本不减少锁持有时间**，
   它减少的是单条语句的 undo / redo / binlog 峰值。真要减少随机页访问，关键是**按主键序**分批
   （先用 `idx_ancestors` 取 id，再按 id 升序 `WHERE id IN (...)`），而不是分批本身 ——
   详见 `04-实施计划.md` §D 的 R2 条目。
...
| 落库校验点 | 校验内容 | 违反时 |
| --- | --- | --- |
| **新建人员**（建管理员 / 教师 / 学生） | 父节点 `node_type=0` → 新节点必须为 1；`=1` **且是机构根**（`id = tenant_id`）→ 允许 1/2/3；`=1` **但非机构根** → **仅 2/3**（F-114，违反 `10104`）；`=2` → 必须为 3；`=3` → 一律拒绝（学生必为叶子） | 拒绝，1xxxx 段错误码 |
...
> 字段表均自 `V202608120000__baseline.sql` 逐字段提取。"可空"列：N = NOT NULL，Y = NULL；默认列 `CURRENT_TIMESTAMP` 简写为 `CURRENT_TS`，`ON UPDATE CURRENT_TIMESTAMP` 简写为 `ON UPDATE CURRENT_TS`，无默认写 `—`。以下各表通用字段（`create_by/create_time/update_by/update_time/deleted_at/remark`）语义一致，逐表照列不再重复解释。表的分组顺序与 DDL 中的七个区块一致。
...
| phone | VARCHAR(20) | Y | NULL | 手机号（原用途：视频跑马灯水印展示。**F-114：水印第一版不做**，本列第一版不被播放链路读取；登录与联系方式用途不受影响） |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_perms | (perms, deleted_at) | 权限标识全局唯一（03-01 §4.2）。末尾追加 `deleted_at` 是契约 §2.2 的统一约定；`perms` 可空且 MySQL 唯一索引不约束 NULL，故目录行（`perms IS NULL`）天然不参与冲突。**由增量脚本 V202608140100 添加，非基线** |
...
用途：承载"租户可配"的业务参数，键白名单见契约 §5 末（complete_rate_threshold 完播判定阈值、watermark_phone_mask 水印手机号脱敏开关）。**⚠ 两个键第一版都不被消费**：完播判定延后（F-113）、水印不做（F-114）；键、默认值与校验原样保留待启用。config_value 一律字符串存储，由服务层按键解析类型与取值范围；读取时优先取本租户配置，无记录则回落平台默认值。云厂商选择属平台部署级参数，不是租户配置键。
...
> **⚠ F-113：`complete_rate_threshold` 第一版不被消费**——完播判定延后（PRD F2-9），`watch_status` 只在 0/1 流转。
> 键本身、默认值 90、合法区间 60~100、租户配置接口的校验**全部原样保留**（`TenantConfigKey.COMPLETE_RATE_THRESHOLD` 已实现），将来启用即可直接用。
> **`TenantConfigKey` 的 Javadoc 写着「修改只影响此后的判定，存量 `watch_status` 不回溯重算」**——那说的是**改阈值**，与本条说的**开功能**不是一回事；需方已定**两者都不回算**，故两句不冲突。

#### 4.1.11 org_node 统一组织树表【全系统核心表】

用途：一棵树到底——**每个节点都是一个人**，不存在独立于人的组织单元节点。node_type 取 0平台超管、1管理员、2教师、3学生（与 sys_user.user_type 取值完全一致），承载规则为：0（超管）只挂 1；1（管理员）**且是机构根**可挂 1/2/3、**非机构根只可挂 2/3**（F-114 定案二：机构下只允许一层管理员，违反 `10104`）；2（教师）下只能挂 3（学生），其直接子节点即"名下学员"；3（学生）必须是叶子。组织层级由管理员节点的**一层**表达——"华东校区"即某位管理员管的那一片，节点名可命名为校区名、ref_user_id 指向该片区负责人。ref_user_id **全部非空**，与 sys_user.node_id 互为反向引用。树顶是全表唯一一行 id=0 的平台根（node_type=0、tenant_id=0、parent_id=-1、ancestors=''），平台超管 sys_user.node_id=0，其子树即全平台，故全系统无需为超管写特例分支。租户约定：机构根节点（即机构最高管理员节点）的 id 即该租户的 tenant_id（契约 2.1），其 parent_id=0 即挂在平台根之下，机构以下所有节点继承同一 tenant_id。

本表是数据权限的唯一依据（契约 2.4）：你能看到的数据 = 你所在节点的子树，判定式 `id = #{myNodeId} OR FIND_IN_SET(#{myNodeId}, ancestors)`，全系统不存在第二套过滤逻辑。ancestors 为逗号分隔的祖级路径，根在前、不含本节点（如 `0,100,101,205`，平台根节点自身为空串），VARCHAR(1000) 约可容纳 50 级（**物理**上界），**业务上界由 F-114 定为 4 级**（不含平台根：机构根 1、管理员 2、教师 3、学生 4）。子树查询有三条路径，服务层按场景选择：a) 逐层展开（默认推荐），由 idx_tenant_parent_sort 支撑按 parent_id 逐层 IN 查询，适用于树懒加载、面包屑、子树 BFS，单层毫秒级；b) ancestors 前缀 LIKE，用于一次性取全子树，`WHERE ancestors LIKE CONCAT(X.ancestors, ',', X.id, ',%')` 可命中 idx_ancestors 的左前缀索引；c) FIND_IN_SET 语义最准但无法走索引、全表扫描，仅在已被 tenant_id 收敛到小结果集或离线巡检任务中使用。ancestors 重算与移动防成环的实现细节详见 §3.1。
...
| ancestors | VARCHAR(1000) | N | '0' | 祖级路径（逗号串，根在前、不含本节点，如 0,100,101,205；平台根节点自身为空串 ''）：子树判定 FIND_IN_SET(#{nodeId},ancestors)；批量取子树用 LIKE CONCAT(ancestors,',',id,',%') 走前缀索引；1000 字符约容纳 50 级（物理上界），业务上界 4 级（F-114） |
...
| archive_reason | TINYINT | Y | NULL | 归档原因：1 正常毕业归档 2 因监护人删除请求归档。决定是否启动 30 日脱敏倒计时（契约 §7.2 第 3 条） |
| anonymized_at | DATETIME | Y | NULL | 脱敏完成时间，NULL=未脱敏。**脱敏不可逆**，本列非空的学员不可归档恢复 |
| tenant_id | BIGINT | N | — | 租户（机构）ID |
| create_by | BIGINT | Y | NULL | 创建人 user_id |
| create_time | DATETIME | N | CURRENT_TS | 创建时间 |
| update_by | BIGINT | Y | NULL | 更新人 user_id |
...
| owner_node_id | BIGINT | N | — | 归属节点ID（→org_node.id，创建时写入创建者所在节点）：管理端可见性按本列做子树过滤，此后不随创建人调岗漂移；学生端可见性走所属课时→课程→课程授权 |
...
用途：**阿里云 VOD**（本期唯一实现，`provider` 保留腾讯兼容位）媒资的本地镜像，记录云端媒资 ID、时长、封面、加密 HLS 地址与转码状态。上传流程为「**发上传凭证时即写入 `vod_file_id`**（阿里云返回的 `cloudVideoId`，故非空）→ `FileUploadComplete` 事件推进 `status` 0→1 → `TranscodeComplete(success)` 事件回填 `duration`／`hls_url`」——**事件经 XXL-Job 拉取轻量消息队列消费，不是 HTTP 回调**（03-课程与视频 §7.2）。唯一索引 `uk_provider_file` 保证事件消费幂等（`vod_file_id` 为 NULL 的上传中记录因 MySQL 唯一索引不约束 NULL 而可多条并存）。`owner_node_id`（上传时写入上传者所在节点），是契约 §2.5 的**授权前提**——只有 owner 或被显式授权者才能再向下授权。视频是三类**受管资源**之一（`resource_type=3`），对节点与学生的授权统一走 `org_resource_grant`，逐级显式、无继承。授权模型细节见主文档 §3.3 专栏。

| 字段 | 类型 | 可空 | 默认 | 说明 |
| --- | --- | --- | --- | --- |
| id | BIGINT | N | — | 媒资ID（雪花算法） |
| owner_node_id | BIGINT | N | — | 归属节点ID（→org_node.id，上传时写入上传者所在节点）：契约 2.5 授权前提，视频亦为受管资源（resource_type=3） |
| provider | TINYINT | N | 2 | 云厂商：1腾讯 2阿里（默认 2 阿里云，与 DDL 一致；uk_provider_file 以本列为首列，默认值写错会让整批媒资落在错误的唯一键分区上） |
| encrypt_type | TINYINT | N | **由观测回填** | 加密方式：0 不加密 / 1 HLS标准加密(AES-128) / 2 阿里云私有加密。<br>**⚠ F-114 第二半起，本列是【观测结果】不是【配置意图】**：由 `VodEventConsumeService` 在转码完成时按**实际挑中那一路**的 `Encrypt` 回填（`VodPlayStream.resolvedEncryptType()`）；上传时 `VodVideoService` 写的 `0` 只是**占位**——那一刻我们并不知道模板组会产出什么。<br>**⚠ DDL 的 DEFAULT 仍是 1 且不会改**（基线冻结，契约 §7.3）。该列先后出现过三个"该写什么"的口径（默认 1 → R1a 定案 2 → F-114 改为观测回填），**正因如此不能依赖默认值**。 |
| template_group_id | VARCHAR(64) | Y | NULL | **上传时【请求】的转码模板组 ID（意图，非观测）。F-114 新增，迁移 `V202608210300`。**<br>**重新发起转码（接口 33）必须复用它**：用当前配置的那个会在**同一个视频上叠出第二套流** → `GetPlayInfo` 返回两路 → 「恰好一路」守卫失败，而它报的原因（「模板组配了多档」）与真实原因对不上。<br>**⚠ 与 `encrypt_type` 分工不同，不要合并**：那一列是转码完成后填的观测值，而**转码失败的视频（`status=3`）那时还只是占位**——正是要重转的那些行，它给不出可信依据。<br>NULL = 本列上线前建的历史行，重转时回落默认组并记 WARN。 |
| decrypt_key_uri | VARCHAR(500) | Y | NULL | 解密密钥 URI（写入 m3u8 的 `EXT-X-KEY`，指向 `/api/v1/vod/decrypt-key`）。**只存 URI，不存密钥本身**。<br>**〔该端点已退役，本列随之恒为 `NULL`〕** 加密路线改为阿里云私有加密后，解密由播放器 SDK 拿 `VidAuth` 完成、密钥由点播服务托管，接口 29 已删除（F-112）。**列与注释一字不改**：注释原文在 `V202608120000__baseline.sql` 里，而基线是**冻结内容**（契约 §7.3），改它会变 Flyway 校验和 —— 那是启动失败，不是一条干净的红。本文档镜像 DDL，所以同样保留原文、只在旁边加注 |
| vod_file_id | VARCHAR(100) | Y | NULL | 云端媒资唯一ID（阿里 VideoId / 腾讯 FileId）。**阿里云路径下发上传凭证时即写入**（响应的 `cloudVideoId`），故非空；NULL 仅出现在腾讯路径的预创建态 |
| video_name | VARCHAR(200) | N | — | 视频名称 |
| duration | INT | N | 0 | 视频时长（秒，`TranscodeComplete` 事件消费时回填） |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_provider_file | (provider, vod_file_id, deleted_at) | 同一云厂商媒资ID唯一（转码事件消费的幂等定位，追加 deleted_at 兼容逻辑删除；vod_file_id 为 NULL 的记录不参与唯一冲突，阿里云路径下发凭证时即写入故通常非空） |
...
| event_type | TINYINT | N | 1 | 发放类型：1 播放凭证（03-03 §8.1）2 解密密钥（§8.2）。标准加密下密钥接口才是真正的鉴权闸口，只审计 1 会漏掉最关键的调用 |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| idx_create_time | (create_time) | 归档清理专用：按 create_time 分批删除；另外三个索引前导列都不是 create_time，缺它则每批 DELETE 全表扫 |
...
用途：学生 × 课时的进度聚合态，是学习时长统计的唯一读取入口。客户端心跳先写 Redis，XXL-Job 每 60s 批量 UPSERT 落盘本表（依据 `uk_student_lesson`）。

> **⚠ F-113（2026-08-21 需方定案）整段改写了本表的语义，原说明已失效：**
>
> - **`watched_duration` 由前端计时器给出，服务端不再按心跳间隔累加。** 服务端做 `delta = max(0, 本次 sessionPlayedSeconds − 该会话上次值)`，再 `delta = min(delta, Δt + 5)` 做**物理上限截断**（兜前端 bug，**是上界不是第二个真相**），然后累加。原说明「单次心跳最多计入 `min(实际间隔,15)` 秒」**已失效**。
> - **原说明「从而压制倍速与挂机刷时长」已失效**：需方定案**不防刷课**（原话「刷课不需要防」）。间隔 <8s 丢弃**保留**，剩下的理由是**限流**。
> - **`max_position` 不再用于禁快进**（规则 3 已废止，拖拽完全放开）。它换了用途：**第一版唯一能反映"看到多深"的数据**，与 `watched_duration` 配合可识别"反复看开头刷时长"（时长 60 分钟 + `max_position` 10 分钟 = 一直在看开头）。
> - **完播判定第一版不执行**：`watch_status` 只在 0/1 流转、不写 2，`complete_time` 恒空。判定逻辑与阈值 `complete_rate_threshold`（默认 90，范围 60~100）原样保留待启用，**且本项目不做历史回算**。
>
> 心跳接口签名见契约 §6.4（**F-113 新增 `sessionPlayedSeconds`**）。
...
| watched_duration | INT | N | 0 | 已看时长（墙钟有效累计秒，复看已看区间同样计入）。**F-113：由前端计时器累计上报，服务端只做 `min(delta, Δt+5)` 上界截断后累加**。<br>**⚠ 冻结基线（`V202608120000__baseline.sql`）该列注释写着「单次心跳最多计入 min(间隔,15)s」——已失效，以本行为准。****基线是冻结内容（契约 §7.3，已发布脚本永不修改），改它会变 Flyway 校验和 → 启动失败，不是一条干净的红**，故原文一个字不动（照 F-100 / C18 先例） |
| max_position | INT | N | 0 | 最远触达位置（秒）。**F-113：不再用于禁快进**（规则 3 已废止，拖拽完全放开）；新用途是**第一版唯一能反映「看到多深」的数据**，与 `watched_duration` 配合识别"反复看开头"。<br>**⚠ 冻结基线该列注释写着「禁快进：仅允许回看 ≤ 此值区间」——已失效，以本行为准**（不改基线的理由同 `watched_duration` 行） |
| last_position | INT | N | 0 | **上次离开位置（秒，断点续播起点）。F-113 新增，随模块 13 的迁移 `V202608230000` 建成**。<br>**直写不取大**，与 `max_position` 分工不同：那一列是历史最远触达（取大不回退），本列是上次停在哪。<br>**⚠ 类型必须与 `max_position` 同为 `INT`，不得用 DECIMAL** —— 两列都表示「视频里的某个位置」，精度不同会让「上次离开的位置是不是就是最远位置」这类判断**永远为假**，而这不报错、只是静默失灵。<br>**⚠ 唯一出口是接口 31** `GET /api/v1/vod/progress` 的 `lastPosition`（PRD F2-8 规则 1 逐字：「调用**进度查询接口**」）。**接口 28 回的 `maxPosition` 不是续播起点** —— 拿错了不报错，只是学生从没看过的地方开始播 |
| watch_status | TINYINT | N | 0 | 完播状态：0未开始 1学习中 2已完成。**F-113：第一版只在 0/1 之间流转，不写 2**（完播判定延后，PRD F2-9）。将来启用时 `watched_duration ≥ duration × complete_rate_threshold` 置 2 |
| complete_time | DATETIME | Y | NULL | 完播时间（watch_status=2 时写入）。**F-113：第一版恒为空** |
| last_heartbeat_time | DATETIME | Y | NULL | 最后一次有效心跳时间（规则 2 限流的间隔基准；**F-113 后不再是计时基准**——计时挪到前端） |
| tenant_id | BIGINT | N | — | 租户（机构）ID |
| create_by | BIGINT | Y | NULL | 创建人 user_id |
| create_time | DATETIME | N | CURRENT_TS | 创建时间 |
| update_by | BIGINT | Y | NULL | 更新人 user_id |
...
> **✅ 已建 `last_position`（F-113 提出，模块 13 落地时建成，迁移 `V202608230000`）**
>
> ```sql
...
> ```
>
> **用途**：断点续播起点（PRD F2-8）。原设计取 `max_position`（最远触达），**放开拖动后那可能只是学生随手拖到过的位置，从那里续播会跳过他实际没看的中间部分**。
>
> **类型必须与 `max_position` 同为 `INT`，不得用 DECIMAL**：两列都表示"视频里的某个位置"，**精度不同会让「上次离开的位置是不是就是最远位置」这类判断永远为假且不报错**——最坏的一类缺陷。`max_position` 是老列改不了，**新列跟旧列走**。
>
> **✅ 已执行（2026-08-23，模块 13）**：迁移 `V202608230000__add_vod_watch_progress_last_position.sql`，该列已补进上方字段表。**本段保留不删** —— 它记着这一列当初为什么要建、以及「类型必须是 INT」那条约束的由来，只写「已建」下一个人看不出这些。
...
用途：逐条心跳原始流水，是防刷审计与进度争议回溯的唯一依据，可按学生×课时回放完整心跳序列。有效性规则与 `vod_watch_progress` 一致：**周期心跳**（`trigger=tick`）`interval_sec ≥ 8s` 视为有效。**⚠ 原文后半句「单次计入 `min(实际间隔, 15)` 秒，>15s 封顶但不报错」已随 F-113 失效** —— 时长改由前端计时器累计上报，服务端只做 `min(delta, Δt + 5)` 上界截断，**不再按到达间隔计时**。本列现在的用途是限流基准、截断的 Δt 来源、日志回放，**以及分辨规则 2 内部两种拒绝的唯一依据**（`reject_rule=2` 且 `interval_sec < 8` = 间隔闸；`≥ 8` = 撞会话级上限，F-116 定案四）。事件心跳（`pause`/`resume`/`ended`/`leave`）**不走间隔闸**，本列仍记真实间隔。`reject_rule` 记录本条心跳的判定结果（0 采纳，**实际取值 2/5/6/7/9**；冻结基线的列注释写的是 2/3/5/6/7/8/9，**3 已随 F-113、8 已随 F-117 废止留墓碑**，以本节字段表为准）——**此前本表没有任何一列表达"算没算、为什么没算"**，只能从 `interval_sec < 8` 反推规则 2，规则 6/7/9 的拒绝完全不可追溯，而"学生说学了一小时系统只记了十分钟"正是最典型的争议场景。1 字节换来全链路可回放。

**取值域的三点约定**（不写清楚，这一列的诊断价值会在实现阶段流失）：

- **编号锚定**：本列的取值就是 03-03 §8.3.1 的规则编号，PRD F2-7 的校验规则列表亦用同一套编号，**三处必须一致**。编号已随本列**持久化进审计表**，本表按月分区保留 6 个月、累计留存数年——任一侧改号，历史日志中的 `reject_rule=N` 就会指向另一条规则，且**无法回溯修正**。
- **1 与 4 保留不使用**：规则 1（凭证有效）在进入心跳处理前即被拦下（返回 20001、前端续签后重试），此时学生与课时的绑定尚未通过校验，写一行归属到某学生的审计记录本身就是错的；规则 4（倍速不加速）是计时口径、不拒绝任何心跳，`playRate` 越界由规则 5 承担。实现方无需为这两个值写分支，运维在日志里找不到它们也**不是埋点漏了**。
- **2/3 与 5 的分工**：规则 2/3 判定失败时虽经规则 5 的响应路径统一返回 `20002`，本列记的是**首次判定失败的那条规则**（2 或 3），5 专指"参数明显非法"。若一律记 5，本列就退化为"要么 0 要么 5"，丧失全部诊断价值。

**权衡说明**：不记 `sessionId`（规则 9 的会话标识），因为 `CHAR(32)` 在常驻 0.95 亿行上多占约 3 GB（+25%），而排查真正需要回答的是"为什么被拒"，不是"哪个会话"。

`seeked` 记录本次心跳前是否发生 seek 或暂停恢复——它决定服务端是否对本次执行推进一致性校验（03-课程与视频 §8.3.1 规则 6：`seeked=1` 重置校验基准、本次不判推进）。**不落这一列就无法回放"这条心跳当时为什么被采纳或丢弃"**，而本表正是争议回溯的唯一依据；据此统计 seeked 频次（连续 >2 次、或单课时**每小时 >180 次**即判异常——**F-113 由 20 提高**，20 是禁快进前提下定的，放开拖动后一小时拖 20 次是正常观看行为）。**⚠ F-113：规则 6 已降级为只留痕，本限频保护的是日志的诊断价值，不再是时长的准确性。**当前播放位置字段命名为 `current_time_sec` 以避开 MySQL 保留字，对应请求体的 `currentTime`。本表按契约第 4 节例外**不带 `deleted_at`**，且精简 `update_by` / `remark`；为满足 MySQL 分区键必须包含在每个唯一键中的约束，主键采用 `(id, created_time)` 联合形式。按 `RANGE (TO_DAYS(created_time))` 每月一个分区，预建 2026-08 ~ 2027-01 共 6 个月分区 + `pmax` 兜底分区；清理一律走 `DROP PARTITION` 秒级裁剪，严禁 `DELETE` 大批量删除。分区轮转与归档的运维流程见主文档 §5。
...
| interval_sec | INT | N | 0 | 与上次心跳的实际间隔（秒）。**F-113：`>=8s 视为有效` 仍成立（规则 2 限流）；`单次计入 min(实际间隔,15)` 已失效**——时长改由前端计时器累计上报，服务端只做 `min(delta, Δt+5)` 上界截断。本列现在的用途是**限流基准 + 上界截断的 Δt 来源 + 日志回放**。<br>**⚠ 冻结基线（`V202608120000__baseline.sql`）该列注释写着「>=8s 视为有效，单次计入 min(间隔,15)」——后半句已失效，以本行为准。****基线是冻结内容（契约 §7.3，已发布脚本永不修改），改它会变 Flyway 校验和 → 启动失败，不是一条干净的红**，故原文一个字不动（照 F-100 / C18 先例） |
| trigger_type | TINYINT | N | 0 | **本条心跳的触发原因（对应请求体 `trigger`，F-116 新增；`TRIGGER` 是 MySQL 保留字，故加 `_type` 后缀，照本表 `current_time_sec` 避开 `CURRENT_TIME` 的先例）**：`0`=缺失或取值非法 `1`=tick 周期 `2`=pause 暂停 `3`=resume 恢复 `4`=ended 播完 `5`=leave 离开页面。<br>**为什么必须有这一列**：F-116 定案「取枚举不取布尔」的理由原话是「它会落进 `vod_heartbeat_log`，顺带说明每条心跳是为什么来的」——**而这张表原来没有这一列**，定案的落地位置清单里也没有它。不建的话那条理由整个落空，**且不报错**：接口照收 `trigger`、照它分流，只是事后一条都查不到，与「枚举退化成布尔」在库里不可区分。<br>**`0` 不是「默认成 tick」**：`trigger` 缺失或非法一律按规则 5 丢弃（F-116 逐字：不得默认成 `tick`），而被丢弃的心跳**明细仍写本表**，所以必须有一个值表示「它报来的东西不能用」。**不复用 `1`** —— 复用就再也分不出「学生在放着」和「客户端根本没报」。<br>**存 TINYINT 不存字符串**：常驻 0.95 亿行上 `VARCHAR(8)` 存 `"resume"` 要 7 字节、TINYINT 要 1 字节，差约 570MB；本表已为同一个理由拒绝过存 `sessionId`（见上方权衡说明）。编码→语义写在列注释里，与 `reject_rule` 同一套办法 |
| play_rate | DECIMAL(4,2) | Y | NULL | **本条心跳上报的倍速（对应请求体 `playRate`）。⚠ 不参与计时** —— 规则 4：`playRate` 不得用来调整前端报来的时长，本列**只供行为分析**。03-03 §8.3.1 规则 4 与契约 §6.4 都写着「仅落 `vod_heartbeat_log`」，**而这张表原来也没有这一列**，与 `trigger_type` 是同一个漏法，一并补上。<br>**⚠ 为什么是 `DECIMAL(4,2)` 而不是 `(3,1)`**：播放器常用倍速含 **0.75 与 1.25**，一位小数会把它们**静默舍成** 0.8 / 1.3，而本列的用途正是「回放当时到底发生了什么」——**两位小数是下限，不是余量**。<br>**⚠ 为什么总长取 4 位（±99.99）而不是刚好够用的 3 位（±9.99）**：规则 5 判 `playRate` 越界时**明细仍写本表、且记它报来的真值**。存不下会让 INSERT 直接失败 —— 那是**一条非法心跳把整个请求打挂**，比丢一行日志坏得多。超出 ±99.99 的极端值写 `NULL`（`reject_rule=5` 已经说明它被拒了），故本列可空 |
| seeked | TINYINT | N | 0 | 本次心跳前是否发生 seek 或暂停恢复：0否 1是（对应请求体 `seeked`）。决定本次是否执行推进一致性校验，必须落库才能回放判定过程。<br>**F-113 两处变化**：① 规则 6 **降级为只留痕**，本列不再影响时长；② 每小时上限 **20 → 180**（放开拖动后一小时拖 20 次是正常观看行为，旧值会把被允许的行为持续判为异常）。<br>**⚠ 冻结基线该列注释整段解释规则 6 与「每小时 20 次上限」——已失效，以本行为准。****基线是冻结内容（契约 §7.3，已发布脚本永不修改），改它会变 Flyway 校验和 → 启动失败，不是一条干净的红**，故原文一个字不动（照 F-100 / C18 先例） |
| reject_rule | TINYINT | N | 0 | 本条心跳的判定结果：0=采纳；**2/5/6/7/9**=命中 03-03 §8.3.1 对应编号的规则（**1、3、4、8 保留不使用**：1 在绑定校验前即被拦下、**3 自 F-113 废止留墓碑不复用**、**8 自 F-117 废止留墓碑不复用**、4 是计时口径不拒绝）。<br>**⚠ F-113：`6` 的含义变了**——规则 6 已降级，命中它的心跳**仍被采纳、时长照算**，本列的 `6` 从此表示「**留痕**」而非「拒绝」。历史行的解读不受影响：新旧两种含义下 `6` 都读作「**没有真实播放**」，这是允许改造而不改号的前提。<br>此前本表没有任何一列记录"这条算没算、为什么没算"——只能从 `interval_sec<8` 反推规则 2，其余完全不可追溯 |
...
**为什么不与 `hw_answer_sheet` 合表（已评估，本期明确不做）**

两表确实各自存着同一份 `(homework_id, student_id)` 集合、且都以它为唯一键：发布时为每个目标学生同时写一行 target 与一行 status=0 的答卷（03-04 §3.5）。target 相对答卷只多出 `grant_source` / `source_ref_id` 两个审计字段，合并后可省一张表、发布时的写入量减半。

**不做的理由不是"合不了"，而是收益与代价不成比例**：

- 这是**纯优化**，不做完全不影响正确性——两表现在没有任何语义冲突，只是有冗余。
- 代价是**扩散性的**：表数 41 → 40 会牵动契约第 4 节表清单、第 9 节的表总数声明、README、一致性脚本的 C6 断言与分域计数，还要重写 03-04 分册的发布/撤回流程与 05 分册的分母口径（"应交人数"当前取 target 计数）。改动面远大于收益。
- 合表后**撤回分发的语义会变复杂**：现在撤回只需逻辑删除 target 行、答卷行保留；合表后"撤回但保留已提交答卷"要靠额外状态位表达，反而不如两张表清晰。

**两者不一致时以 `hw_homework_target` 为准**：它是"这份作业分发给了谁"的唯一真相源，答卷行是它的派生物（发布时预建、按 `UK(homework_id, student_id)` 一一对应）。若出现有答卷无 target 的行，属于发布事务未完整回滚的脏数据，由一致性巡检报出；统计口径中的"应交人数"一律数 target，不数答卷——答卷行可能因重考作废被复位，target 不会。
...
**三个冗余列（`student_id` / `teacher_node_id` / `submit_date`）**：三者都是 `hw_answer_sheet` 已有的信息，在**学生提交答卷时与答卷头同源同时刻**写入本表，**不得二次计算**——若分两处各算一次，错题榜与单次作业分析会对同一份作答得出两个不同的归属。

冗余的理由是 F4-1 高频错题榜的执行路径。该榜的口径是"某导师名下学员、某时间区间内、按 `question_id` 聚合"，而本表稳态 **3600 万行**。不冗余时唯一可行的执行方式是：先按 `teacher_node_id` 从 `hw_answer_sheet` 捞出该导师的答卷 ID（且 `idx_teacher_node_homework` 不含 `submit_time`，拿不到时间范围，只能全捞）→ 用**上万个 `sheet_id`** 去 `IN` 查 3600 万行明细 → 再分组聚合。看板首屏 2s 的目标达不到。

有这三列后走 `idx_teacher_date_question(teacher_node_id, deleted_at, submit_date, question_id, is_correct, student_id)`：两个等值列在前定位、`submit_date` 范围裁剪、其后三列让 `GROUP BY question_id` 与 `COUNT(DISTINCT student_id)` 在索引内完成，**全程不回表**。

索引列的取舍都是被查询逼出来的，逐个说明：

- `student_id` 必要——错题榜的分子与分母都是 `COUNT(DISTINCT student_id)`（同一学生可能在两次作业里做过同一题），少了它索引不覆盖，仍要为每行回表。
- `deleted_at` 必要——它出现在 `WHERE` 里（软删的明细不计入），漏掉同样使覆盖失效。实测对照：六列版 `EXPLAIN` 为 `Using index`，去掉 `deleted_at` 的五列版退化为 `Using index condition`。
- `GROUP BY` 的 `question_id` 位于范围列 `submit_date` 之后，因此仍有一次 `filesort`。这是刻意的取舍：日期区间比题目更有选择性，应优先用于裁剪；而分组集只是单个导师区间内出现过的题目数（百量级），排序代价可忽略。

`submit_date` 为 NULL 即"尚未提交"，因此**本列非空等价于答卷 `status ∈ {2,3}`**，聚合时无需再 join 回答卷头判状态。

> 这三列**必须在编码前加**：现在是三行 DDL，上线后是 3600 万行的 `ALTER` 加回填，按契约 §7.3 得走 gh-ost。
...
| student_id | BIGINT | N | — | **【冗余】**学生ID（→org_student.id），与所属答卷的 `student_id` 恒等 |
| teacher_node_id | BIGINT | Y | NULL | **【冗余】**作答时刻导师节点ID快照（→org_node.id，node_type=2）；NULL=作答时尚未分配导师 |
| submit_date | DATE | Y | NULL | **【冗余】**提交日期（=答卷 `submit_time` 的日期部分）；NULL=尚未提交，非空即等价于答卷 status ∈ {2,3} |
...
| idx_teacher_date_question | (teacher_node_id, deleted_at, submit_date, question_id, is_correct, student_id) | **【高频错题榜主路径，F4-1】**两个等值列在前定位，日期范围裁剪，其后三列在索引内完成分组与去重计数。六列恰好覆盖该查询引用的全部列，`EXPLAIN` 为 `Using index` |
| idx_question_correct | (question_id, is_correct) | 全机构维度按题目聚合错误数（不限导师，如题库质量分析）；导师维度走上一条 |

关系：逻辑外键 `answer_sheet_id → hw_answer_sheet.id`、`question_id → qb_question.id`、`student_id → org_student.id`、`teacher_node_id → org_node.id`，`(question_id, question_version)` 组合逻辑指向 `qb_question_version`；被 `hw_wrong_book.source_answer_detail_id` 引用。本项目不建物理外键。**冗余列的一致性以 `hw_answer_sheet` 为准**：`student_id` / `teacher_node_id` / `submit_date` 与答卷头不一致时，以答卷头为唯一真相源，本表的值仅供聚合加速；重考作废（撤回后重新发布）时原明细整行逻辑删除、不改这三列。
...
| finished_lesson_count | INT | N | 0 | 【累计状态型】【完播率分子】累计快照：截至本日已完播课时数（不是当日新增）；区间取值取末日行，严禁跨天求和<br>**⚠ F-113：第一版不写入，恒为 0；不可用于绘制完播率趋势。** 完播判定第一版不执行（PRD F2-9），且**本项目不做历史回算**——那个 0 与「真的一个都没完播」逐字节相同。需方已知情并接受（不看该趋势图），**故不做「启用日期」机制**，以本注记为准。 |
...
| watch_seconds | BIGINT | N | 0 | 【当日流量型】当日名下学员观看总秒数；区间取值为跨天求和。缺此列时导师看板的学习时长需回事实表全量求和 |
| finished_lesson_count | INT | N | 0 | 【累计状态型】【完播率分子】名下学员累计快照合计：截至本日已完播课时数；区间取值取末日行，严禁跨天求和<br>**⚠ F-113：第一版不写入，恒为 0；不可用于绘制完播率趋势。** 完播判定第一版不执行（PRD F2-9），且**本项目不做历史回算**——那个 0 与「真的一个都没完播」逐字节相同。需方已知情并接受（不看该趋势图），**故不做「启用日期」机制**，以本注记为准。 |
...
| late_submit_count | INT | N | 0 | 【当日流量型】当日到期作业中逾期提交的份数；区间取值为跨天求和 |
| should_submit_count | INT | N | 0 | 【当日流量型】【按时提交率分母】名下学员当日应提交份数合计；当日新增；区间取值为跨天求和 |
| avg_complete_rate | DECIMAL(5,2) | N | 0.00 | 当日完播率（0-100，两位小数）= finished_lesson_count/should_lesson_count×100，派生冗余列【仅供单日直读】；区间完播率取末日行的分子分母相除，禁止 AVG(本列)<br>**⚠ F-113：第一版不写入，恒为 0；不可用于绘制完播率趋势。** 完播判定第一版不执行（PRD F2-9），且**本项目不做历史回算**——那个 0 与「真的一个都没完播」逐字节相同。需方已知情并接受（不看该趋势图），**故不做「启用日期」机制**，以本注记为准。 |
...
| watch_seconds | BIGINT | N | 0 | 【当日流量型】子树内学员当日观看总秒数；区间取值为跨天求和。大屏总学习时长 KPI 直读本列，避免回事实表全子树求和（那会废掉本表"读大屏 1 行命中"的设计初衷） |
| finished_lesson_count | INT | N | 0 | 【累计状态型】【完播率分子】子树内学员累计快照合计：截至本日已完播课时数；区间取值取末日行，严禁跨天求和<br>**⚠ F-113：第一版不写入，恒为 0；不可用于绘制完播率趋势。** 完播判定第一版不执行（PRD F2-9），且**本项目不做历史回算**——那个 0 与「真的一个都没完播」逐字节相同。需方已知情并接受（不看该趋势图），**故不做「启用日期」机制**，以本注记为准。 |
...
| `vod_play_auth_log` | 6 个月 | 按 `create_time` 分批删除（每批 ≤ 1 万行，低峰执行），走新增的 `idx_create_time`；或改造为月分区 | 凭证审计与防刷排查用 |
...
**测算参数**：与 §6.1 基础假设**同一套数字**——100 机构 × **每机构 1 万学生**（500 教师 × 每教师 20 学员）× 人均 10 门课，全平台 100 万学生。

> 此处曾误用"每机构 50 教师 = 1000 名学员"，比 §6.1 的全局假设**小了整整一个数量级**，据此得出的"可以忽略不计""不需要分表"是建立在错误基数上的。两节各自的算术都对，错的是基数不一致——这类错误在单节内自洽，只有把两节并排看才会暴露。

| 授权层级 | 计算口径 | 单机构行数 | 100 机构行数 |
| --- | --- | --- | --- |
| **学生级·课程** | 500 教师 × 20 学员 × 10 门课 | 100,000 | 10,000,000 |
| 教师级·课程 | 500 教师 × 30 门课（教师持有量是下发给学员的超集） | 15,000 | 1,500,000 |
| 管理员级·课程 | 1 机构管理员 × 100 门 + 50 下级管理员 × 60 门 | 3,100 | 310,000 |
| 教师级·题目 | 500 教师 × 500 道题（题目下发到教师即止，学生通过作业作答，无需逐题授权） | 250,000 | 25,000,000 |
| 教师级·视频 | 500 教师 × 80 个独立媒资（随课程消费的视频无需单独授权） | 40,000 | 4,000,000 |
| **稳态合计** | | **~41 万** | **~4100 万** |
| **3 年累计**（含撤销留存的软删行、人员流动重授、模板重复应用） | 稳态 × 2~2.5 | **~80~100 万** | **~8000 万~1 亿** |

**放大倍数**：若采用继承模型，只需在顶层授权约 5000 行/机构；逐级显式授权放大到 41 万行/机构，**放大约 80 倍**（倍数不随基数变化，因为分子分母同步放大）。

**存储结论**：3 年累计约 **1 亿行** × ~250 B（含 4 个索引摊销）≈ **25 GB**。这不是"可以忽略不计"的量级——它比 `hw_answer_detail`（1.8 TB）与 `vod_heartbeat_log`（常驻 1.2 TB）小两个数量级，但**它是心跳之外命中最频繁的鉴权表**，25 GB 意味着索引已放不进常见的 16~32 GB buffer pool、开始产生磁盘 IO。

**因此结论收敛为**：目标规模（100 机构 × 1 万学生）下**仍不需要分表**，但**必须纳入压测范围**。

**预警指标盯"单节点授权行数"，不盯总容量**——总容量不是这张表的性能命门：

- 主鉴权索引 `idx_target_resource` 五列约 40 B + 主键 8 B，稳态 4100 万行约 **2 GB**，在任何生产配置下都能常驻 buffer pool。25 GB 是三年累计的**表**大小（含 JSON 之外的全部列与软删行），它决定的是磁盘与备份成本，不决定点查延迟。
- 真正决定点查扫描行数的是**单节点持有的授权行数**（学生 ~10~30、教师 ~600、管理员 ~200~5000）。而这个上界**不随机构人数变化**——它由"人均 10 门课""教师持有 500 道题"这类**人均持有量**决定。10× 修正基数动的是总行数，动不了它。

**监控项：单节点授权行数 P99 > 2000 告警。** 触发它意味着有人在往单个节点上堆资源（典型场景：把整个题库一次性授给某个教师），那才是点查开始变慢的真实前兆。总容量只保留一条提示：三年后约 25 GB，拆库时它跟着 `tenant_id` 走。

> 行均 250 B 是按"4 个索引"估的，而 `idx_target_resource` 现为五列，实际行开销略高于此。不必现在改数字——压测时以实测覆盖估算值即可。
>
> 原先"可以忽略不计"的措辞会让人在压测计划里直接跳过这张表，那才是真正的风险。

**索引可行性结论**：

1. **主鉴权索引 `idx_target_resource(target_node_id, resource_type, resource_id, deleted_at, valid_end)` 支撑两类查询**。`target_node_id` 基数极高（全平台 110 万节点），单节点持有的授权行数有明确上界：学生 ~10~30 行、教师 ~600 行、管理员 ~200~5000 行（顶层机构管理员持有量最大）。因此"我能用哪些课程"这类查询的扫描行数是**百级**，不随平台总量增长。
...
5. **目标规模下不需要分表，但必须纳入压测**。100 机构 × 1 万学生（§6.1 基础假设）3 年累计约 1 亿行、约 25 GB。**但 25 GB 不是性能命门**：主鉴权索引 `idx_target_resource` 稳态约 2 GB（五列 ~40 B + 主键 8 B × 4100 万行），任何生产配置都能常驻 buffer pool；25 GB 是含全部列与软删行的表大小，决定的是磁盘与备份成本。**真正决定点查扫描行数的是单节点持有的授权行数**，而它由人均持有量（人均 10 门课、教师 500 道题）决定，**不随机构人数变化**——基数放大 10× 动的是总行数，动不了它。**故预警指标为「单节点授权行数 P99 > 2000」**，触发即排查是否有人把整个题库授给了单个节点；总容量只在拆库时有意义（跟着 `tenant_id` 走）。**不要再写"可以忽略不计"**——那句措辞会让人在压测计划里直接跳过这张表，而它是心跳之外命中最频繁的鉴权表。
...
3. **组织类表为轻量表，授权表需单独关注**：`org_node`（0.4 GB @100 机构）、模板与标签表（MB 级）不构成容量风险点；而 `org_resource_grant` 按 §6.1 基础假设（每机构 1 万学生）3 年累计约 **1 亿行 / 25 GB @100 机构**，虽仍在单实例舒适区，但**必须纳入压测与容量监控**（预警线见 §6.2）——它是心跳之外命中最频繁的鉴权表；
4. `hw_answer_detail.student_answer`、`qb_question_version.content` 为 JSON 大字段，列表查询必须走覆盖索引避免回表拖行；
5. `org_node.ancestors` 为 VARCHAR(1000) 变长列，前缀索引 255 字符（utf8mb4 下 1020 字节，未超 InnoDB DYNAMIC 行格式 3072 字节上限）。**注意 255 字符只覆盖约 12 级**（雪花 ID 19 位 + 逗号 = 20 字符/级），不是 50 级：更深的子树在前缀比对时只能区分到最近的 12 级祖先，选择性下降但**不影响正确性**——前缀 LIKE 命中的是超集，服务层仍按完整 `ancestors` 复核。真正的物理硬上限是列长本身的 50 级（见 §3.1），而**业务上限是 F-114 定的 4 级**——4 级远在 12 级之内，前缀索引的选择性问题因此不再出现；
...
（全文完。本文档任何字段/枚举/命名如需变更，先改 `DESIGN-CONTRACT.md`，再同步 `V202608120000__baseline.sql` 与本文档。）
