...
用途：逐条心跳原始流水，是防刷审计与进度争议回溯的唯一依据，可按学生×课时回放完整心跳序列。有效性规则与 `vod_watch_progress` 一致：`interval_sec ≥ 8s` 视为有效，单次计入 `min(实际间隔, 15)` 秒，>15s 封顶但不报错。`reject_rule` 记录本条心跳的判定结果（0 采纳，2/3/5/6/7/8/9 对应被哪条规则拒绝）——**此前本表没有任何一列表达"算没算、为什么没算"**，只能从 `interval_sec < 8` 反推规则 2，规则 3/6/7/8/9 的拒绝完全不可追溯，而"学生说学了一小时系统只记了十分钟"正是最典型的争议场景。1 字节换来全链路可回放。
...
| reject_rule | TINYINT | N | 0 | 本条心跳的判定结果：0=采纳；**2/5/6/7/8/9**=命中 03-03 §8.3.1 对应编号的规则（**1、3、4 保留不使用**：1 在绑定校验前即被拦下、**3 自 F-113 废止留墓碑不复用**、4 是计时口径不拒绝）。<br>**⚠ F-113：`6` 的含义变了**——规则 6 已降级，命中它的心跳**仍被采纳、时长照算**，本列的 `6` 从此表示「**留痕**」而非「拒绝」。历史行的解读不受影响：新旧两种含义下 `6` 都读作「**没有真实播放**」，这是允许改造而不改号的前提。<br>此前本表没有任何一列记录"这条算没算、为什么没算"——只能从 `interval_sec<8` 反推规则 2，其余完全不可追溯 |
