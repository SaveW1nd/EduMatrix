...
>
> **本轮不执行**：模块 13 尚未开工，`backend/` 本轮零改动。该列**随模块 13 自己的迁移一并创建**，届时再补进上方字段表（照 F-12「落地脚本」的先例——脚本先登记在 04-实施计划 §E，字段表等迁移执行后同步，否则 **C14「DDL 列集合 = 文档字段表」会红**）。
