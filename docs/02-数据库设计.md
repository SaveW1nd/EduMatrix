...
| 原则 | 落地方式 |
| --- | --- |
| **统一组织树** | 机构/管理单元、管理员、教师、学生**全部是 `org_node` 的节点**（`node_type` 1/2/3/4），一棵树到底。师生关系由树的父子结构表达（学生挂在教师节点下 = 该导师名下学员），不设独立的分组表与师生关系表。树顶是唯一一行 `id=0` 的虚拟平台根（`parent_id=-1`、`ancestors=''`、`tenant_id=0`）。 |
...
| **软删除（逻辑删除）** | 核心业务数据禁止物理删除，统一置 `is_deleted = 1`。唯一索引与软删的冲突采用【方案A：唯一索引末尾追加 `is_deleted`】，详见 §7。日志/心跳明细表（`sys_login_log`、`sys_oper_log`、`vod_play_auth_log`、`vod_heartbeat_log`）按契约例外允许按时间/分区物理归档；其中仅 `vod_heartbeat_log` 不带 `is_deleted`。 |
...
| **通用字段** | 所有业务表表尾统一携带 `tenant_id / create_by / create_time / update_by / update_time / is_deleted / remark`（契约 §2.2）；平台级表不带 `tenant_id`；日志/心跳明细表按例外精简 `update_by / remark`。 |
...
| 11 | **`org_node`** | **统一组织树（机构/管理员/教师/学生同表）** | 系统与组织域 | ~1.1 万（每人一个节点） |
...
2. **软删除语义冲突**：本系统"删除"是 `is_deleted=1`，父行逻辑删除后子行仍物理存在，物理外键的 `RESTRICT/CASCADE` 语义与之天然矛盾；
...
erDiagram
    ...
    org_node ||--|| org_teacher : "org_teacher.node_id UK（node_type=3）"
    org_node ||--|| org_student : "org_student.node_id UK（node_type=4）"
    ...
    org_node {
        bigint id PK "平台根固定 0；机构节点 id = tenant_id"
        bigint parent_id "-1=平台根自身，0=机构节点"
        varchar ancestors "祖级路径逗号串，根在前，不含本节点"
        tinyint node_type "1机构 2管理员 3教师 4学生"
        bigint ref_user_id UK "→sys_user.id，机构节点为 NULL"
    ...
    org_student {
        bigint node_id UK "→org_node.id（node_type=4）"
...
erDiagram
    ...
    org_student ||--o{ vod_play_auth_log : "student_id"
...
UPDATE org_node SET student_count = student_count - #{movedStudentCnt}
 WHERE id = #{oldParentId} OR FIND_IN_SET(id, #{oldParentAncestors});
UPDATE org_node SET student_count = student_count + #{movedStudentCnt}
 WHERE id = #{targetParentId} OR FIND_IN_SET(id, #{targetParentAncestors});
...
1. **校验、移动、重算、计数、轨迹必须同事务**（契约 §2.3 约束 4、§8.2 铁律 1）。若重算脱离事务，中途失败会留下"`parent_id` 已改、`ancestors` 未改"的裂开状态——而 `ancestors` 正是鉴权依据，裂开即**越权**。
...
| 落库校验点 | 校验内容 | 违反时 |
| --- | --- | --- |
| **新建节点** | 父节点 `node_type=3` → 新节点必须 `node_type=4`；父节点 `node_type=4` → 一律拒绝（学生必为叶子）；父节点 `node_type=1/2` → 允许 1/2/3/4 | 拒绝，1xxxx 段错误码 |
| **移动节点** | 同上，按**目标父节点**的 `node_type` 判定；另需校验被移动节点若为 `node_type=2/3`，目标父不得为 3 或 4 | 拒绝 |
| **修改 node_type** | **直接禁止**。人员类型变更（教师转管理员）应走"改角色 + 移动节点"，不得原地改 `node_type`，否则其既有子树可能瞬间违规 | 接口不提供该能力 |
| **删除节点** | `child_count > 0` 拒绝删除（必须先移走子节点）；`node_type=3` 且 `student_count > 0` 时提示"请先转移名下学员" | 拒绝 |
| **建账号** | `node_type=2/3/4` 的节点必须有 `ref_user_id`；`node_type=1` 必须为 NULL（由 `uk_ref_user_id` 兜住"一人一节点"） | 拒绝 |
| **导入学生** | Excel 批量导入时逐行套用"新建节点"规则，失败行进 `org_import_task` 的失败报告，不阻断整批 | 单行失败 |
...
| 机构管理员 | 机构节点 id | `'0,机构id'` | 本机构全部：下级管理员、教师、学生 |
...
SELECT s.id, s.student_no, u.real_name, u.phone, s.status, n.parent_id
  FROM org_student s
  JOIN org_node   n ON n.id = s.node_id AND n.is_deleted = 0
  JOIN sys_user   u ON u.id = s.user_id AND u.is_deleted = 0
...
| 对比项 | 逐级显式授权（本方案） | 继承模型 |
| --- | --- | --- |
| **鉴权查询** | **只需一跳**：`target_node_id = 我 AND resource_id = X AND 有效期内`，`idx_target_type_valid` 单索引命中，O(1) 且执行计划稳定 | 需回溯整条祖先链：`target_node_id IN (我, 父, 祖父, ..., 根)`，深度越深越慢，且 IN 列表长度不定 |
...
-- 列出"我能用哪些课程"：同一个索引 idx_target_type_valid(target_node_id, resource_type, valid_end)
SELECT g.resource_id, c.course_name, c.cover_file_id, g.valid_end
  FROM org_resource_grant g
  JOIN crs_course c ON c.id = g.resource_id AND c.is_deleted = 0 AND c.status = 1
...
> **实测**：2 万行样本上 `EXPLAIN` 该查询为 `type=range, key=idx_target_type_valid, rows=135`，未退化为全表扫描。

#### 3.3.3 级联回收的实现思路（必须实现）

撤销某节点的资源授权时，**必须同时撤销该资源在目标节点整棵子树内的全部授权**。否则会出现"父级已无权、子级仍持有"的**悬挂授权**，逐级收缩被破坏（契约 §2.5 规则 5、§8.2 铁律 3）。
...
3. **撤销是逻辑删除**：置 `is_deleted=1` 而非物理删除，保留"谁在什么时候撤了谁的权"的审计链；注意唯一索引 `uk_resource_target` 末尾含 `is_deleted`，同一业务键**最多容纳一条已删除记录**，因此重新授权时应走 `INSERT ... ON DUPLICATE KEY UPDATE is_deleted=0`（复用原行）而不是插新行，详见 §7。
...
SELECT g.id, g.resource_id, g.target_node_id, n.parent_id, g.grant_by, g.grant_time
  FROM org_resource_grant g
  JOIN org_node n ON n.id = g.target_node_id AND n.is_deleted = 0
 WHERE g.resource_type = 1
   AND g.is_deleted    = 0
   AND (g.valid_end IS NULL OR g.valid_end >= NOW())
   AND n.parent_id > 0                                    -- 机构节点(parent_id=0)与平台根不参与
   -- 父节点不是该资源的 owner
   AND NOT EXISTS (SELECT 1 FROM crs_course c
                    WHERE c.id = g.resource_id AND c.owner_node_id = n.parent_id AND c.is_deleted = 0)
   -- 且父节点没有该资源的有效授权
   AND NOT EXISTS (SELECT 1 FROM org_resource_grant p
                    WHERE p.resource_type = 1 AND p.resource_id = g.resource_id
                      AND p.target_node_id = n.parent_id AND p.is_deleted = 0
...
模板解决的是"新招一名教师要逐个勾选几百个资源"的运维负担。核心约束：**模板只是批量快捷方式，应用时取交集，绝不放大权限**（契约 §8.2 铁律 3）。
            ...
            OR EXISTS (  -- ② 或授权人自己就是资源 owner
                 SELECT 1 FROM crs_course c
                  WHERE i.resource_type = 1 AND c.id = i.resource_id
                    AND c.owner_node_id = #{granterNodeId} AND c.is_deleted = 0)
            OR EXISTS (
                 SELECT 1 FROM qb_question q
                  WHERE i.resource_type = 2 AND q.id = i.resource_id
                    AND q.owner_node_id = #{granterNodeId} AND q.is_deleted = 0)
            OR EXISTS (
                 SELECT 1 FROM vod_video v
                  WHERE i.resource_type = 3 AND v.id = i.resource_id
                    AND v.owner_node_id = #{granterNodeId} AND v.is_deleted = 0)
  ) owned ON TRUE
...
INSERT INTO org_resource_grant
      (id, resource_type, resource_id, target_node_id, valid_start, valid_end,
       ...
       tenant_id, create_by, create_time, update_time, is_deleted)
...
SELECT #{snowflakeId},
       s.id,
       CASE WHEN p.node_type = 3 THEN p.id ELSE NULL END,   -- 父为教师才算导师，否则 NULL
       p.id,                                                -- 直属归属节点（管理员或教师）
       #{statDate},
 ...
 WHERE s.is_deleted = 0 AND s.status = 0 AND s.tenant_id = #{tenantId}
...
```sql
-- 对每个管理单元/管理员节点 X，聚合其整棵子树内学员的当日事实行
...
SELECT ..., x.id, #{statDate}, COUNT(*),
 ...
 WHERE x.node_type IN (1, 2) AND x.is_deleted = 0 AND x.tenant_id = #{tenantId}
 GROUP BY x.id;
```
...
> 字段表均自 `edumatrix_ddl.sql` 逐字段提取。"可空"列：N = NOT NULL，Y = NULL；默认列 `CURRENT_TIMESTAMP` 简写为 `CURRENT_TS`，`ON UPDATE CURRENT_TIMESTAMP` 简写为 `ON UPDATE CURRENT_TS`，无默认写 `—`。以下各表通用字段（`create_by/create_time/update_by/update_time/is_deleted/remark`）语义一致，逐表照列不再重复解释。表的分组顺序与 DDL 中的七个区块一致。
...
| is_deleted | TINYINT | N | 0 | 逻辑删除：0否 1是 |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_name | (name, is_deleted) | 机构名称唯一（追加 is_deleted 兼容逻辑删除） |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_username | (username, is_deleted) | 登录账号全平台唯一（错误码 10001 依据） |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_tenant_role_key | (tenant_id, role_key, is_deleted) | 同租户内角色标识唯一（追加 is_deleted 兼容逻辑删除） |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_user_role | (user_id, role_id, is_deleted) | 同一用户不可重复绑定同一角色（追加 is_deleted 兼容逻辑删除） |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_role_menu | (role_id, menu_id, is_deleted) | 同一角色不可重复绑定同一菜单（追加 is_deleted 兼容逻辑删除） |
...
用途：记录所有登录尝试（成功与失败）。按契约 2.2 的日志表例外，本表不带 create_by / update_by / remark，以 login_time 作为业务时间列，允许按时间物理归档清理；is_deleted 仍保留但业务上恒为 0。登录失败且账号不存在时 user_id 为 NULL，仅记录尝试的 username。
...
| tenant_id | BIGINT | N | 0 | 租户（机构）ID |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| is_deleted | TINYINT | N | 0 | 逻辑删除：0否 1是（日志业务恒为 0，清理走物理归档） |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_tenant_config_key | (tenant_id, config_key, is_deleted) | 同租户同配置键唯一（配置读写按此点查/UPSERT，追加 is_deleted 兼容逻辑删除） |

关系：tenant_id 逻辑指向 sys_tenant.id；complete_rate_threshold 被 vod_watch_progress 的完播判定读取。

#### 4.1.11 org_node 统一组织树表【全系统核心表】

用途：一棵树到底——机构/管理单元、管理员、教师、学生全部是本表的节点。node_type 取 1机构/管理单元、2管理员、3教师、4学生，承载规则为：1/2 可挂 1/2/3/4 任意类型；3（教师）下只能挂 4（学生），其直接子节点即"名下学员"；4（学生）必须是叶子，不得有任何子节点。ref_user_id 在 node_type=2/3/4 时必填并与 sys_user.node_id 互为反向引用，node_type=1 时为 NULL。树顶是全表唯一一行 id=0 的虚拟平台根（node_type=1、tenant_id=0、parent_id=-1、ancestors=''），平台超管 sys_user.node_id=0，其子树即全平台，故全系统无需为超管写特例分支。租户约定：机构节点的 id 即该租户的 tenant_id（契约 2.1），其 parent_id=0 即挂在平台根之下，机构以下所有节点继承同一 tenant_id。
...
| 字段 | 类型 | 可空 | 默认 | 说明 |
| --- | --- | --- | --- | --- |
| id | BIGINT | N | — | 节点ID（雪花算法；两个特例：平台根节点固定 id=0；机构节点的 id 即该租户的 tenant_id，契约 2.1） |
| parent_id | BIGINT | N | 0 | 父节点ID（-1=平台根节点自身，全表唯一；0=其父为平台根，即机构节点；其余为上级节点 id：管理员的父为机构/上级管理员，教师的父为管理员，学生的父为管理员或教师） |
...
| node_name | VARCHAR(100) | N | — | 节点名称（机构/管理单元填机构名或部门名；管理员/教师/学生节点填其真实姓名，与 sys_user.real_name 同步） |
| node_type | TINYINT | N | — | 节点类型：1机构/管理单元 2管理员 3教师 4学生（契约 §5；承载规则：3 下只能挂 4，4 必为叶子） |
| ref_user_id | BIGINT | Y | NULL | 关联账号 user_id（→sys_user.id）：node_type=2/3/4 时必填且与 sys_user.node_id 互为反向引用；node_type=1（机构/管理单元）为 NULL |
...
| student_count | INT | N | 0 | 子树内在读学生节点总数（冗余计数：教师节点=名下学员数，管理员/机构节点=其子树学员总数；异动时自底向上逐级维护） |
| tenant_id | BIGINT | N | — | 租户（机构）ID，平台根节点为 0 |
| create_by | BIGINT | Y | NULL | 创建人 user_id |
| create_time | DATETIME | N | CURRENT_TS | 创建时间 |
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| is_deleted | TINYINT | N | 0 | 逻辑删除：0否 1是（child_count>0 时禁止删除，须先移走子节点） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_ref_user_id | (ref_user_id, is_deleted) | 一个账号在树上仅占一个节点（用户→节点反查亦走此索引最左前缀；机构节点 ref_user_id 为 NULL，MySQL 唯一索引不约束 NULL 故可多条并存） |
...
用途：教师的档案属性表，与 org_node 的教师节点（node_type=3）1:1、与 sys_user 1:1。教师即一个树节点，其直接子节点就是"名下学员"，师生关系只由树的父子结构表达，不设独立的师生关系表。本表只承载工号/科目/职称等档案属性，不承载任何权限或归属语义。student_count 与 org_node.student_count 同源同步。

| 字段 | 类型 | 可空 | 默认 | 说明 |
| --- | --- | --- | --- | --- |
| id | BIGINT | N | — | 教师ID（雪花算法） |
| node_id | BIGINT | N | — | 教师节点ID（→org_node.id，node_type=3，1:1；名下学员 = 该节点的直接子节点） |
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| is_deleted | TINYINT | N | 0 | 逻辑删除：0否 1是（节点仍有子节点即名下仍有学员时禁止删除，须先移走学员） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_node_id | (node_id, is_deleted) | 契约 UK(node_id)：一个教师节点仅一份档案（追加 is_deleted 兼容逻辑删除） |
| uk_user_id | (user_id, is_deleted) | 契约 UK(user_id)：一个账号仅对应一份教师档案（追加 is_deleted 兼容逻辑删除） |
...
用途：学生的档案属性表，与 org_node 的学生节点（node_type=4）1:1、与 sys_user 1:1。学生即一个叶子节点，归属完全由树的位置表达——挂在管理员节点下表示已归属该管理员但尚未分配导师，挂在教师节点下即该导师名下学员；本表没有 node_id 之外的第二个归属字段，"谁是我的导师"由 org_node.parent_id 且该父节点 node_type=3 判定。退课与归档字段（status / quit_time / quit_reason / archive_time），学籍状态枚举见契约 §5 student_status。

| 字段 | 类型 | 可空 | 默认 | 说明 |
| --- | --- | --- | --- | --- |
| id | BIGINT | N | — | 学生ID（雪花算法） |
| node_id | BIGINT | N | — | 学生节点ID（→org_node.id，node_type=4，1:1；父节点即当前归属的管理员或导师） |
| user_id | BIGINT | N | — | 账号ID（→sys_user.id，1:1） |
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| is_deleted | TINYINT | N | 0 | 逻辑删除：0否 1是（学习记录禁止随之删除） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_node_id | (node_id, is_deleted) | 契约 UK(node_id)：一个学生节点仅一份档案（组织树 → 学员档案的点查入口，追加 is_deleted 兼容逻辑删除） |
| uk_user_id | (user_id, is_deleted) | 契约 UK(user_id)：一个账号仅对应一份学生档案（追加 is_deleted 兼容逻辑删除） |
...
用途：记录节点在树上的每一次移动与状态变更。所有异动都统一为"节点在树上的移动"，因此只需记录 from_parent_id → to_parent_id，无需再区分"转导师/转节点"两套字段；教师带学员整体调岗时只记教师节点一行，其学员子树跟随移动但不逐个记录。本表是"这个人为什么归他管"的唯一追溯依据，业务上禁止删除（is_deleted 恒为 0）。归属结算以自然日切分（契约 2.6：转导师当日归新导师）。

| 字段 | 类型 | 可空 | 默认 | 说明 |
| --- | --- | --- | --- | --- |
| id | BIGINT | N | — | 主键ID（雪花算法） |
| node_id | BIGINT | N | — | 发生异动的节点ID（→org_node.id；教师调岗时为教师节点，其学员子树跟随移动但不逐个记录） |
| change_type | TINYINT | N | — | 异动类型：1建档 2分配导师 3转交管理员 4教师调岗 5毕业归档 6归档恢复 7退课 8节点移动（机构/管理员节点自身改父）（契约 §5 change_type） |
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| is_deleted | TINYINT | N | 0 | 逻辑删除：0否 1是（轨迹业务上禁止删除，恒为 0） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
用途：承载契约 2.5 的资源逐级下发模型。受管资源为课程（resource_type=1）、题目（=2）、视频（=3），每一级都必须显式授权、不向下继承——上级拥有不等于下级自动拥有，任一节点实际可用的资源 = 其父级显式授予它的那一份，天然逐级收缩，不存在跨级穿透。判定"某节点能否使用某资源"只需一跳命中、不回溯祖先链：按 target_node_id + resource_type + resource_id + is_deleted=0 + 有效期区间查询，由最高频索引 idx_target_type_valid(target_node_id, resource_type, valid_end) 单索引命中，无回表回溯、无递归；该索引的列序与"我能用哪些课程/题目/视频"及"我能否用资源 X"两类查询完全对应，而唯一键 uk_resource_target(resource_type, resource_id, target_node_id) 是**全库唯一不追加 is_deleted 的唯一索引**（契约 §2.2 特例）——授权是反复授予/撤销的高频对象，若追加 is_deleted 则第二次撤销即撞唯一键；去掉后重新授权只能走 INSERT ... ON DUPLICATE KEY UPDATE is_deleted=0 复活原行，正确行为被固化进约束本身。服务层强制四条约束：授权人必须已拥有该资源（自己是 owner_node_id 或自己已被有效授权）；目标节点必须在授权人子树内；撤销必须级联回收——递归撤销目标节点整棵子树内该资源的全部授权行（走 idx_resource_type_id 定位），否则会出现"父级已无权、子级仍持有"的悬挂授权。级联回收的实现细节详见 §3.3。已产生的学习记录（vod_watch_progress / hw_answer_sheet / hw_wrong_book）一律保留不删，仅失去继续访问权。
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| is_deleted | TINYINT | N | 0 | 逻辑删除：0否 1是（撤销授权=置 1，级联回收对子树内同资源行批量置 1） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_resource_target | (resource_type, resource_id, target_node_id) | 同一资源对同一节点仅一行。**全库唯一不追加 is_deleted 的唯一索引**（契约 §2.2 特例）：授权反复增删，追加 is_deleted 会在第二次撤销时撞键；去掉后重新授权只能走 UPSERT 复活原行 |
| idx_target_type_valid | (target_node_id, resource_type, valid_end) | 【全系统最高频鉴权查询】"我能用哪些课程/题目/视频" + "我能否用资源X"：按目标节点 + 资源类型 + 有效期单索引命中，不回溯祖先链 |
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| is_deleted | TINYINT | N | 0 | 逻辑删除：0否 1是（删除模板不影响已生成的授权行） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_tenant_template_name | (tenant_id, template_name, is_deleted) | 机构内模板名唯一（追加 is_deleted 兼容逻辑删除） |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_template_resource | (template_id, resource_type, resource_id, is_deleted) | 契约 UK(template_id,resource_type,resource_id)：同一模板不可重复收录同一资源（追加 is_deleted 兼容逻辑删除） |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_tenant_tag_name | (tenant_id, tag_name, is_deleted) | 同机构内标签名唯一（追加 is_deleted 兼容逻辑删除） |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_student_tag | (student_id, tag_id, is_deleted) | 契约 UK(student_id,tag_id)：同一学生同一标签仅一条（追加 is_deleted 兼容逻辑删除） |
...
用途：机构内课程的定义主表，承载课程名称、封面、科目、简介与上架状态，并冗余课时总数与视频总时长供列表页直读。`owner_node_id`（归属节点，创建时写入创建者所在节点），它是契约 §2.5 的**授权前提**——只有 owner 本人、或已被显式授权且在有效期内的节点，才能把该课程再向下授权。课程是三类**受管资源**之一（`resource_type=1`），对节点与学生的授权统一走 `org_resource_grant`，逐级显式、无继承，本表不设配套授权表。授权模型细节见主文档 §3.3 专栏。课程禁止物理删除，一律 `is_deleted=1`。
...
| description | VARCHAR(1000) | Y | NULL | 课程简介 |
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| is_deleted | TINYINT | N | 0 | 逻辑删除：0否 1是（课程禁止物理删除） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| is_deleted | TINYINT | N | 0 | 逻辑删除：0否 1是（课时禁止物理删除） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_provider_file | (provider, vod_file_id, is_deleted) | 同一云厂商媒资ID唯一（转码回调幂等定位，追加 is_deleted 兼容逻辑删除；vod_file_id 为 NULL 的上传中记录不参与唯一冲突，可多条并存） |
...
用途：每次发放播放凭证（PlayAuth／Key）的审计流水，用于排查取证频次异常与凭证盗用。`expire_time` 是错误码 20001（凭证过期）的校验依据，`auth_token` 脱敏截断后存储。本表按契约 2.2 的日志例外精简 `update_by` / `remark` / `create_by`（写入方为系统，创建人即请求学生本人，记于 `student_id`），仍携带 `is_deleted`（业务上恒为 0），清理走按时间物理归档而非逻辑删除。
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| idx_student_time | (student_id, create_time) | 审计某学生取证频次（防刷排查） |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_student_lesson | (student_id, lesson_id, is_deleted) | 契约 UK(student_id,lesson_id)：一名学生一个课时仅一条进度（心跳落盘 UPSERT 依据，追加 is_deleted 兼容逻辑删除） |
...
| interval_sec | SMALLINT | N | 0 | 与上次心跳的实际间隔（秒，>=8s 视为有效，单次计入 min(实际间隔,15)） |
...
| creator_id | BIGINT | N | — | 出题人 user_id |
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| is_deleted | TINYINT | N | 0 | 逻辑删除：0否 1是（题目禁止物理删除） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
关系：逻辑外键 `owner_node_id → org_node.id`、`category_id → qb_category.id`、`parent_id → qb_question.id`（自引用）、`creator_id → sys_user.id`；被 `qb_question_version.question_id`、`hw_homework_question.question_id`、`hw_answer_detail.question_id`、`hw_wrong_book.question_id` 引用；授权关系由 `org_resource_grant`（`resource_type=2`）表达。本项目不建物理外键。

#### 4.3.3 qb_question_version 题目版本快照表

用途：题目内容的**不可变快照**。每次编辑题目写入一行新版本（`version` 从 1 起递增）并回写主表 `current_version`；任何已写入的历史版本**不可修改、不可删除**（`is_deleted` 业务上恒为 0）。作业固化的 `question_version`、作答明细的 `question_version`、错题本首次做错时刻的 `question_version` 全部指向本表的具体行，从而保证"当时看到的题目"可精确复原。`content` 与 `correct_answer` 以 JSON 存储（题干／选项／子题排序、标准答案或简答评分要点），客观题自动判卷即以 `correct_answer` 比对；`analysis` 按作业的 `answer_visible_type` 时机对学生展示；`score_default` 是组卷时的默认分值，可在 `hw_homework_question.score` 内覆写。版本创建人与创建时间统一使用表尾通用字段 `create_by` / `create_time`，不另设 `created_by` / `created_time`（避免双真相源）。
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| is_deleted | TINYINT | N | 0 | 逻辑删除：0否 1是（版本快照业务上禁止删除，恒为0） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_question_version | (question_id, version, is_deleted) | 契约 UK(question_id,version)：同一题目版本号唯一（追加 is_deleted 保持全库统一方案） |
...
| creator_id | BIGINT | N | — | 创建人 user_id（→sys_user.id；教师与管理员均可创建作业） |
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| is_deleted | TINYINT | N | 0 | 逻辑删除：0否 1是（作业禁止物理删除） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| idx_creator_status | (creator_id, status) | 教师端我的作业列表按状态筛选 |
...
关系：逻辑外键 `owner_node_id → org_node.id`、`course_id → crs_course.id`、`creator_id → sys_user.id`；被 `hw_homework_question.homework_id`、`hw_homework_target.homework_id`、`hw_answer_sheet.homework_id`、`hw_wrong_book.source_homework_id` 引用。本项目不建物理外键。
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_homework_question | (homework_id, question_id, is_deleted) | 契约 UK(homework_id,question_id)：同一作业不可重复选同一题（追加 is_deleted 兼容逻辑删除） |
...
用途：作业的分发对象清单，**全量精确到学生**。批量入口不落库为"批量行"，而是在服务层展开为逐个学生行，再由 `grant_source` + `source_ref_id` 记录来源以支持整批撤回与追溯。`grant_source` 枚举为 **1手动选择 2按节点批量 3按标签批量 4按名下全体**——作业分发**不使用 5按权限模板**（模板只管课程/题目/视频三类受管资源）。展开时目标学生必须落在分发者子树内（契约 §2.4），标签仅用于筛选，不得扩大可见范围；请求体中显式指定的学生越界时返回业务码 10107 而非静默忽略。撤回分发走逻辑删除（`is_deleted=1`）。
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| is_deleted | TINYINT | N | 0 | 逻辑删除：0否 1是（撤回分发走逻辑删除） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_homework_student | (homework_id, student_id, is_deleted) | 契约 UK(homework_id,student_id)：同一学员不可重复分发同一作业（追加 is_deleted 兼容逻辑删除） |
...
用途：学生 × 作业的答卷头表，记录答卷状态流转（0未开始 → 1作答中 → 2已提交待批改 → 3已批改，逾期未交为 4）与三个分数字段：`objective_score` 提交时由系统自动判分写入，`subjective_score` 教师批改完成后写入，`total_score` 为二者之和。重复提交返回错误码 30003。`teacher_node_id` 是**作答时刻的导师节点快照**（`org_node`，`node_type=3`），提交时固化，学员此后转导师也不改写本行——契约 §2.6"历史归原导师"由此落地，原导师看板的历史业绩不会随学员流动而回溯变化。该列**可为 NULL，表示作答时该学员直挂管理员节点、尚未分配导师**。归属结算的完整口径（聚合数值走快照、个体明细叠加当前子树过滤，即"数能算进去，人看不见"）见主文档 §3.5 专栏。答卷禁止物理删除，资源授权被撤销时亦一律保留。
...
| homework_id | BIGINT | N | — | 作业ID（→hw_homework.id） |
| student_id | BIGINT | N | — | 学生ID（→org_student.id） |
| teacher_node_id | BIGINT | Y | NULL | 作答时刻导师节点ID快照（→org_node.id，node_type=3；NULL=作答时学员直挂管理员节点、尚未分配导师；转导师不影响历史业绩口径） |
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| is_deleted | TINYINT | N | 0 | 逻辑删除：0否 1是（答卷禁止物理删除） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_homework_student | (homework_id, student_id, is_deleted) | 契约 UK(homework_id,student_id)：一名学生一份作业仅一份答卷（追加 is_deleted 兼容逻辑删除） |
...
关系：逻辑外键 `homework_id → hw_homework.id`、`student_id → org_student.id`、`teacher_node_id → org_node.id`（`node_type=3`）、`grade_teacher_id → sys_user.id`；被 `hw_answer_detail.answer_sheet_id` 引用；是 `stat_student_daily` 作业类指标的上游事实来源。本项目不建物理外键。
...
| update_by | BIGINT | Y | NULL | 更新人 user_id |
| update_time | DATETIME | N | CURRENT_TS / ON UPDATE CURRENT_TS | 更新时间 |
| is_deleted | TINYINT | N | 0 | 逻辑删除：0否 1是（答卷明细禁止物理删除） |
| remark | VARCHAR(500) | Y | NULL | 备注 |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_sheet_question | (answer_sheet_id, question_id, is_deleted) | 同一答卷同一题仅一条作答记录（追加 is_deleted 兼容逻辑删除） |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_student_question | (student_id, question_id, is_deleted) | 契约 UK(student_id,question_id)：一名学生一道题仅一条错题记录（重复做错走计数，追加 is_deleted 兼容逻辑删除） |
...
SELECT ROUND(finished_lesson_count / NULLIF(should_lesson_count, 0) * 100, 2) AS complete_rate
FROM   stat_teacher_daily
WHERE  teacher_node_id = ? AND stat_date <= ? /* 区间末日 */ AND is_deleted = 0
ORDER  BY stat_date DESC
LIMIT  1;
...
FROM   stat_teacher_daily
WHERE  teacher_node_id = ? AND stat_date BETWEEN ? AND ? AND is_deleted = 0;

-- 正确③：区间去重活跃人数——汇总表只有人日，必须回事实表
SELECT COUNT(DISTINCT student_id) AS active_students
FROM   stat_student_daily
WHERE  teacher_node_id = ? AND stat_date BETWEEN ? AND ? AND is_active = 1 AND is_deleted = 0;
...
| id | BIGINT | N | — | 主键ID（雪花算法） |
| student_id | BIGINT | N | — | 学生ID（→org_student.id） |
| teacher_node_id | BIGINT | Y | NULL | 结算时刻导师节点ID快照（→org_node.id，node_type=3；NULL=结算时直挂管理员节点、尚未分配导师；已结算行永不重算） |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_student_date | (student_id, stat_date, is_deleted) | 契约 UK(student_id,stat_date)：一名学生一天一条汇总（定时任务 UPSERT 依据，追加 is_deleted 兼容逻辑删除） |
...
| --- | --- | --- | --- | --- |
| id | BIGINT | N | — | 主键ID（雪花算法） |
| teacher_node_id | BIGINT | N | — | 导师节点ID（→org_node.id，node_type=3） |
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_teacher_node_date | (teacher_node_id, stat_date, is_deleted) | 契约 UK(teacher_node_id,stat_date)：一位导师一天一条汇总（追加 is_deleted 兼容逻辑删除） |
| idx_tenant_date | (tenant_id, stat_date) | 机构管理端导师排行榜按日期聚合全部导师 |

关系：来源 `stat_student_daily`（按 `idx_teacher_node_date` 上卷）；`teacher_node_id → org_node.id`（node_type=3），对应教师档案 `org_teacher.node_id`。
...
用途：定时任务按 `stat_student_daily.node_id` 快照二次聚合得到的组织节点 × 自然日汇总，供机构/校区大屏按管理单元取数。**每行 = 该节点整棵子树的聚合**：结算时按 `org_node.ancestors` 展开子树并把其中全部学员汇总成一行，读某节点大屏 1 行命中，查询期无需展开子树求和。
...
| --- | --- | --- |
| PRIMARY | (id) | 主键 |
| uk_node_date | (node_id, stat_date, is_deleted) | 契约 UK(node_id,stat_date)：一个节点一天一条汇总（追加 is_deleted 兼容逻辑删除） |
...
| 业务表（is_deleted=1 的软删数据） | 永久在线 | 不清理 | 契约禁止物理删除；确需瘦身时仅可迁移至同构归档库并保留主键 |
...
| `org_node`（统一组织树） | 每人一个节点：1 万学生 + 数百教师 + 数十管理员 + 机构节点 | ~1.1 万 | ~110 万 | ~350 B（ancestors 变长） | ~4 MB / ~0.4 GB |
| `stat_teacher_daily` | 300 导师 × 750 日 | ~22 万 | ~2200 万 | ~200 B | ~45 MB / ~4.4 GB |
| `stat_node_daily` | ~50 个管理单元/管理员节点 × 750 日 | ~4 万 | ~400 万 | ~200 B | ~8 MB / ~0.8 GB |
...
| **稳态合计** | | **~4.1 万** | **~410 万** |
| **3 年累计**（含撤销留存的 `is_deleted=1` 行、人员流动重授、模板重复应用） | 稳态 × 2~2.5 | **~8~10 万** | **~800~1000 万** |
...
1. **主鉴权索引 `idx_target_type_valid(target_node_id, resource_type, valid_end)` 完全够用**。`target_node_id` 基数极高（全平台 110 万节点），单节点持有的授权行数有明确上界：学生 ~10~30 行、教师 ~600 行、管理员 ~200 行。因此"我能用哪些课程"这类查询的扫描行数是**百级**，不随平台总量增长。
   - **实测验证**：在 2 万行样本上 `EXPLAIN` 该查询，结果为 `type=range, key=idx_target_type_valid, rows=135`，未退化为全表扫描，符合预期。
...
**问题**：业务表禁止物理删除，仅置 `is_deleted=1`。若唯一索引不含 `is_deleted`（如 `uk_username(username)`），软删一条记录后，再新建同业务键的记录会触发唯一冲突——被"删除"的数据挡住了新数据。

**本项目统一采用【方案A：唯一索引末尾追加 `is_deleted` 列】**（与 DDL 文件头声明一致）：所有含 `is_deleted` 的表，其唯一索引一律在末尾追加 `is_deleted`，如 `uk_username(username, is_deleted)`、`uk_student_lesson(student_id, lesson_id, is_deleted)`、`uk_resource_target(resource_type, resource_id, target_node_id, is_deleted)`。

**效果**：

- 存活记录（`is_deleted=0`）之间唯一性照常强约束；
- 软删后（`is_deleted=1`），可再插入一条同业务键的存活记录，互不冲突；
- UPSERT（`INSERT ... ON DUPLICATE KEY UPDATE`，如心跳落盘、日汇总、资源授权续期）继续依赖唯一索引幂等命中存活行。

**已知局限与约束**：同一业务键**最多容纳一条已删除记录**（两条 `is_deleted=1` 的同键记录仍会唯一冲突）。因此：若需再次删除同键记录，应用层须先将旧的已删除记录归档转移，**或恢复复用原记录**（把旧记录改回 `is_deleted=0` 并更新内容，不再插入新行）。该约束由服务层统一封装（删除入口先探测同键已删记录），DDL 不再额外处理。

**特别提示：`org_resource_grant` 必须走"复用原行"路径**。资源授权是**高频反复授予/撤销**的对象（转导师、模板重复应用、有效期续期），若每次重新授权都插新行，很快会因"已存在一条 `is_deleted=1` 的同键行"而唯一冲突。正确写法是重新授权时用 UPSERT 把原行复活：

```sql
INSERT INTO org_resource_grant (id, resource_type, resource_id, target_node_id, ...)
VALUES (...)
    ON DUPLICATE KEY UPDATE is_deleted = 0,          -- 复活原行，而非插新行
...
> 注意：`uk_resource_target` 末尾含 `is_deleted`，因此上面这条 UPSERT 在"存在存活行"时命中存活行（续期），在"只存在已删行"时**不会**命中（`is_deleted` 值不同）。服务层需先按 `(resource_type, resource_id, target_node_id)` 探测是否存在已删行：存在则走 `UPDATE ... SET is_deleted=0`，不存在才 INSERT。该逻辑必须收敛到统一的 `ResourceGrantService.grant()` 入口，禁止各业务方自行拼 SQL。

**未选用的备选方案（供评审知悉）**：

| 备选 | 做法 | 不采用原因 |
| --- | --- | --- |
| 方案B：删除时间戳/墓碑值列 | 用 `delete_time`（存活=0，删除=删除时刻）替代 `is_deleted` 进唯一索引，可容纳无限次同键删除 | 与 MyBatis-Plus `@TableLogic` 布尔语义不兼容，全部查询条件需改写，侵入面大 |
| 方案C：删除时改写业务键 | 软删时把唯一列改写为 `原值#deleted#雪花id` | 污染原始数据，违背"历史数据追踪链路完整"的需求基线 |
| 方案D：MySQL 8.0 函数索引过滤 | 仅对 `is_deleted=0` 建函数唯一索引 | MySQL 无原生部分索引，函数索引方案可读性差且工具链（迁移/同步）兼容性风险高 |
