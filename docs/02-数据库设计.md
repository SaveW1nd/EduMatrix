...
5. `org_node.ancestors` 为 VARCHAR(1000) 变长列，前缀索引 255 字符（utf8mb4 下 1020 字节，未超 InnoDB DYNAMIC 行格式 3072 字节上限）。**注意 255 字符只覆盖约 12 级**（雪花 ID 19 位 + 逗号 = 20 字符/级），不是 50 级：更深的子树在前缀比对时只能区分到最近的 12 级祖先，选择性下降但**不影响正确性**——前缀 LIKE 命中的是超集，服务层仍按完整 `ancestors` 复核。真正的硬上限是列长本身的 50 级（见 §3.1）；
