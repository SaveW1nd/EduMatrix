...
| encrypt_type | TINYINT | N | **2** | 加密方式：1 HLS标准加密(AES-128) **2 阿里云私有加密（契约 §1 订正后的定案，R1a 需方选路）** 0 不加密。**⚠ DDL 的 DEFAULT 仍是 1 且不会改**——基线是冻结内容（契约 §7.3，已发布脚本永不修改），故由 `VodVideoService` 建行时**显式写 2**，不依赖默认值：依赖一个与事实不符的默认值，迟早有人绕过 Service 直接 INSERT 出一行 `encrypt_type=1` 的媒资 |
