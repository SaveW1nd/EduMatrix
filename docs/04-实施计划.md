...
- **⚠ 开工前先看 `F-48` 是否已定案（本条是它在模块 14 侧唯一的强制检查点）**：
  ...
  - **⚠ 不要连 `20014` 一起改**：§6.x 链上「不存在 / 已删除 / `status=0` 隐藏 / 类型不匹配」四种情形
    同为 `20014`、彼此不可区分，本身不泄露；真正的可区分点只在 `20013` 与它的并排码之间。改了是过度修正。
