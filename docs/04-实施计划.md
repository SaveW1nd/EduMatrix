...
| **F-41**<br>**部分定案（执行器已补，调度中心待需方拍板）** | **XXL-Job 执行器全库未装配** —— `xxl.job.*` 五个配置项从模块 01 起就在，却<b>没有任何东西在读</b>；两个 `@XxlJob` 注解（`anonymizeArchivedStudent` / `tempFileCleanup`）**是惰性的** | `application.yml` 有 `xxl.job.enabled` / `admin-addresses` / `access-token` / `executor.appname` / `executor.port`；`deploy/.env.example` 有 `XXL_JOB_ENABLED=false`，读起来像是「置 `true` 即可启用」。而全库 grep 不到 `XxlJobSpringExecutor` —— **置 `true` 也不会有任何事发生**。<br>这与 `@OperLog` 在模块 05 之前的状态是同一个形态，是本项目「以为存在、实际从未生效的保障」的**第六例**（前五：基线 charset 子句、`@AssertTrue` 拦不住不传、`useSSL=true` 而服务端没开 TLS、§7.4 下载头在 302 路径不生效、契约 §7.2 第 5 条日志保留期零承载）。<br>**这个状态从模块 01 一直活到模块 05 之后**，期间没有任何检查、测试或启动日志提到过它 | **① 执行器侧已补（模块 05）**：`common/config/XxlJobConfig`，受 `xxl.job.enabled` 门控（默认关），启动日志打出 appname / port / admin 地址 / handler 清单；`admin-addresses` 或 `access-token` 为空时各有一条说清后果的 WARN。新增 `XxlJobHandlerRegistryTest`（4 条）：执行器 `@Bean` 存在且挂了 `start`/`destroy`、受门控、handler 名不重复、**handler 名集合被钉住**（加一个 Job 即红，提醒去调度中心登记 —— 漏登记的后果是「任务永远不跑且无人报告」）。<br>**② 调度中心（`xxl-job-admin`）待需方拍板** —— 它是一个<b>独立的第三方 Web 应用</b>，上生产要连带决定四件事：<br>　a. **来源**：Maven Central 没有可直接运行的发行包，需从 GitHub 源码构建 —— 属往生产引入第三方制品；<br>　b. **默认口令 `admin/123456`** 的 Web 控制台，而本机**尚无 HTTPS**（备案接入变更未做，Caddy 只有 `:80`）；<br>　c. 需在生产 RDS 上**新建 `xxl_job` 库**（约 8 张表）；<br>　d. **内存**：3.4G 可用的机器上再起第二个 JVM（当前可用 2.1G，装得下，但 `edumatrix.service` 头注释对内存预算有明确论证）。<br>**③ 一个必须一并考虑的替代**：契约 §J1 是**单实例部署**，而 XXL-Job 的价值（分布式调度、故障转移、分片）**在单节点上全部不成立**；Spring `@Scheduled` 能做同一件事且零新增基础设施。**但 XXL-Job 写在契约里**（§7.1「traceId 经 MDC 贯穿……XXL-Job」、§2.8「XXL-Job 定时任务」）与 `05-工程结构.md` §H，**换掉它是对上位文档的偏离，实施方不自行决定**。<br>**卡住什么**：两个 Job（模块 07 的脱敏、模块 05 的 7 天清理）**在②落地之前都不会自动触发**。两者都不影响在线业务，但 7 天保留期与 30 日脱敏窗口这两条**合规承诺目前没有自动执行的机制**。 |
