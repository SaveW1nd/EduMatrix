...
>
> **交付时实测**：单元 155 / IT 422 执行（+3 被 `EDUMATRIX_R2` 门控跳过）；
...
> **⚠ F-49 ~ F-69 是模块 09 / 10 并行开发的【临时编号保留区】，两个模块合并后作废。**
> 模块 09（媒资与转码事件消费）用 F-49 / F-51 / F-52 / F-62，模块 10（题库）从 **F-70** 起，
> 中间的空号不是漏登记。`check_consistency.py` 的 C19 缺号检查对这一段免检
> （`RESERVED_GAPS = set(range(49, 70))`）——**两个模块都合并后，删掉那一行与本段**，
> 让 C19 重新覆盖 49~69。定案见 **F-74**。
> 不写成永久豁免的理由：永久豁免会让此后真在 49~69 之间漏登记的条目对所有工具隐形，
> 而那正是 **F-25** 那次的成因。
