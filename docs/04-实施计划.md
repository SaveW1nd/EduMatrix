...
- **⚠ 开工前先看 `F-48` 是否已定案（本条是它在模块 12 侧唯一的强制检查点）**：
  ...
  - **未定案** → **按现状实现**（§8.1 / §8.2 的错误码栏原样），并在交付报告里**点名说明「F-48 未定案，本模块按现状实现」** ——
    不点名等于让它悄悄消失在两条已经写死的校验链里。
  ...
  - **⚠ 无论定不定案，§8.1 与 §8.2 的校验链必须逐条相同**（契约 §1、§8.2 正文）：改一条不改另一条，
    正是契约 §1 那段越权面论证要防的形态。
