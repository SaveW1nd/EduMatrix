...
| **F-42**<br>**未定案（只登记，实现按分册）** | 课程按 id 访问时，「不存在」与「存在但不可见」返回**不同的码**，两者合起来**泄露存在性** | `03-03 §0.3` 把 `20004` 定为「课程不存在、已逻辑删除」；`§1.2` 的数据权限段写「均不满足时返回 **HTTP 404**（不暴露存在性）」。两条各自成立，**合起来**却让调用方能区分：拿到 `20004` = 这个 id 在本租户不存在；拿到 404 = **存在但你看不到**。而 `03-01 §7.2` 恰恰用「雪花 ID 同租户内时间相邻、可近邻枚举」论证过详情接口不能下发直链——同一条推理在这里指向相反的结论。<br>跨租户不受影响（被插件过滤，与「不存在」同为 `20004`，无差异）；泄露面仅限**本租户内**「某个 id 是不是一门课」 | **实现按分册**（`CourseAccessGuard` 的判定顺序表逐条对应 §0.3 与 §1.2），不自作主张合并。<br>**需方决定的是**：要不要把「不可见」也改成 `20004`（代价：与契约 §2.4 三分法第 1 行「路径上的资源不在我的子树内 → 404」不一致，且 403/404/业务码的三分法要为本模块开一个例外）；或者把「不存在」也改成 404（代价：`20004` 在 §1.2 / §1.4 / §1.5 / §1.6 五处的「相关业务错误码」栏都要删，而它在请求体指定 `courseId` 的场景——如 §2.2 创建章节——仍然需要）。<br>**卡住哪个模块**：无。**但模块 12 的播放凭证链路（§8.1）会照抄这套判定顺序，定案越晚要改的地方越多。** |
