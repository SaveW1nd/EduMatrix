...
```sql
-- 登录校验：拆 ancestors 得祖先 id 列表（约 5 个），主键 IN 查询
SELECT 1 FROM org_node
 WHERE id IN (#{myNodeId}, ...祖先 id 列表...)
   AND node_type = 1 AND status = 1 LIMIT 1;   -- 命中即拒登（10016）
```

- 停用只写 1 行，恢复也只写 1 行，子树立即同步，**不存在"分不清谁是被级联停的、谁是本来就单独停的"**
- 级联写库方案要为一个 1.1 万人的分支写 2.2 万行，中途失败留下半停用状态且无法自愈
- 注意条件里的 `node_type = 1`：只有**管理员**节点的停用会冻结子树，教师节点的 `status=1` 不影响其学员
...
Body: {"lessonId":"...","videoId":"...","currentTime":123.4,"duration":600,"playRate":1.0,"seeked":false}
...
它存在的唯一理由是解开一处规则冲突：系统**明确允许**把进度条拖回 `maxPosition` 之前自由复看（PRD F2-5 规则 1），复看同样计时长（F2-7 规则 3）；而防刷的推进一致性校验（03-03 §8.2.1 规则 6）要求 `Δ currentTime` 落在 `[0.5,1.5] × Δt × playRate`。学生从 500s 拖回 120s 后，下一次心跳的 `Δ currentTime = -380`，直接出区间被判异常丢弃——**一个被明确允许的行为会被持续判为作弊且不计时长**。暂停后原地续播同理（`Δt` 大而 `Δ currentTime` 近 0）。
