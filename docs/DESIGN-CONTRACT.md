...
> 回调接口另有三条与租户无关但同样必须的约束：
>
> 1. **签名校验 + 出口 IP 白名单双重防线**。阿里云 VOD 的 HTTP 回调在签名机制上与腾讯不对等——**上线前必须在控制台确认本账号实际可用的校验手段**：若只能依赖回调地址中的密钥参数，则"签名校验代替身份认证"这句在本选型下**不成立**，IP 白名单会成为唯一防线，届时必须补偿：**回调只允许推进状态与写入我们自己从云端 API 反查到的字段，不得直接采信报文中的任何 URL**。
> 2. **签名/鉴权失败返回 HTTP 401 而非 200**（03-课程与视频 §7.2）。云厂商按状态码判投递成败，2xx 即视为已消费——返回 200 会让本可靠重试自愈的事件永久丢失。
> 3. **幂等键 `vod_file_id + EventType + Status`**。`TranscodeComplete` 的成功与失败共用同一 `EventType`，只用前两段会把"先失败重试后成功"的第二条回调误判为重复而丢弃。
