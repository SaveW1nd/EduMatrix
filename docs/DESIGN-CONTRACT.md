...
| `1949000000000300103` | `1949000000000300100` | ~~修改授权有效期~~ | F | — | `org:grant:edit` | A12 | org_admin、teacher **⚠ 孤儿权限**：03-02 接口 40 已删除（需方 2026-08-21 定案取消授权有效期），菜单行与两条角色绑定**仍在初始化数据里**、后端已无对应端点。**只登记不擅自删**（权限的真相在初始化数据里，改它是需方的决定），见 04-实施计划.md F-104 |
