...
>
> **但契约里没有验收标准这种东西**——上面第 1、2、3 条实质上是**功能需求**，只是被写在合规语境里，于是没有任何人负责测。它们的验收标准在 **PRD §7.6**，实现与测试以那里为准。这个分工不是形式：写 PRD 验收标准时才发现第 3 条**根本落不了地**（归档接口只改 `status`，没有脱敏动作、没有定时任务、表里也没有承载它的列），而契约这五条读起来毫无破绽。**"能不能写出验收标准"是检验条款是否真的存在的唯一办法。**
