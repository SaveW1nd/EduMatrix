...
> **`seeked` 由客户端提供，因此必须防止它被用来永久关闭规则 6**：脚本只要每次都置 `true` 就能以 1:1 墙钟速度刷时长而无需下载任何分片。约束见 03-03 §8.3.1 规则 6——连续 `seeked = true` 不得超过 2 次、且单课时每小时不超过 20 次，超出按异常丢弃并计入 `vod_heartbeat_log.reject_rule`，由风控报表按 `(student_id, lesson_id, 时间窗)` 聚合呈现。**不设风控分列**：该名词若建成字段就要定义算法、衰减、阈值与清零时机，而这四项全无定义；`vod_heartbeat_log` 已是完备的原始事实（按月分区留 6 个月），任何评分口径将来都能从它算出来。这与「存分子分母、不存比率」（§4 统计表铁律）同源。真实用户 seek 后会继续播放，不存在"每 10 秒 seek 一次"的模式。
