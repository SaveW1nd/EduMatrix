...
| 播放器 | **Aliplayer**（二开：~~禁拖拽~~ **F-113 取消** / ~~跑马灯水印~~ **F-114 第一版不做** / 前端计时器 / 心跳）—— ~~DPlayer + hls.js~~，见下方订正 | ~~DPlayer~~ |
| 视频云 | **阿里云 VOD（主，本期唯一实现）**；腾讯云 VOD 保留 `provider` 列以备兼容，本期不实现 | — |
| 视频加密 | **阿里云私有加密（`AliyunVoDEncryption`）+ `VidAuth`** —— ~~HLS 标准加密（AES-128）~~，见下方订正 | — |
| ID 生成 | 雪花算法（bigint，Java 侧生成），前端以字符串传输防精度丢失 | xzs |

**视频加密方式定案：⚠ 已改为阿里云私有加密 + Aliplayer（订正，2026-08-20）**

> **〔订正：原文为「HLS 标准加密（AES-128），不用阿里云私有加密」，本节以下整段的论证与结论均按原选型写就〕**
>
> **需方定案：走阿里云私有加密 + Aliplayer，不是 HLS 标准加密 + DPlayer + hls.js。**
>
> **依据的实测状态，如实照抄，不得写成更肯定的说法**（见 `poc/r1a-aliplayer/RESULT.md`）：
>
> > 两条路线均在 Android XWEB 上实测、均未被实测推翻；路线 A ③ 全屏项存疑未复测、
> > 路线 B ④ 日志佐证待补；iOS 零覆盖。**选路线 B 是产品侧决策，不是被实测逼的。**
>
> **⚠ `RESULT.md` 的结论勾选框（可行 / 不可行）与签字栏至今仍是空的**，
> 该文件自己写着「结论未定，且已明确还缺什么」。也就是说：
> **本条定案是产品侧的选路决策，不是一份已签字的可行性结论。**
> `RESULT.md` 的签字条件里，**iOS 微信四项零覆盖、必测不可豁免**那一条仍未闭合 ——
> 风险是全屏被系统播放器接管（`webkitbeginfullscreen`）导致 DOM 水印与自定义守卫一并失效，
> 以及播放路径退化为原生 HLS。
>
> **本次订正改变的**：`vod_video.encrypt_type` 默认值 1 → **2**；模块 12 **不再需要**自建密钥分发接口
> （私有加密的密钥由点播服务托管），故 `ALIYUN_KMS_*` 两项本期不消费；模块 14 的播放器改用 Aliplayer。
> **本次订正不改变的**：`§1` 部署约定四条（单一清晰度、挑选规则、挑不到置 3、区域约束）
> **原样成立** —— POC 实测 `GetPlayInfo` 返回 `Format=m3u8`、`Encrypt=1`、
> `EncryptType=AliyunVoDEncryption`，挑选规则 `Format == "m3u8" && Encrypt == true` 照样命中。
> **不要因为换了加密方式就去改挑流条件。**

**〔以下为原选型的论证，保留供回溯〕视频加密方式（原定案）：HLS 标准加密（AES-128），不用阿里云私有加密**
...
> **〔本段随加密路线改写而失去载体，但论证本身仍然成立 —— F-112〕** 下面这段说的是「标准加密下密钥接口才是闸口」。改走私有加密后**密钥接口已删除**（03-03 §8.2 墓碑小节），**闸口前移到 `POST /api/v1/vod/play-auth` 签发 `VidAuth` 这一步** ——**越权面没有消失，只是换了守门人**，所以 03-03 §8.1 的五步校验链一条都不能少。
>
...
2. **挑选规则为 `Format == "m3u8"`——加不加密都收**（**F-114 第二半，2026-08-21 需方定案改**）。

   > **原文是 `Format == "m3u8" && Encrypt == true`，那一条是错的，且错得很安静。**
   > 它把「模板组配的是加密输出」这个**假设写进了代码**，而配置在需方侧。
   > 需方一旦把模板换成不加密，阿里云转码**成功**、我们判「挑不出加密流」→ 置 `status=3` →
   > **控制台一切正常、我们这边显示失败，那个视频永远用不了。**
   >
   > **新口径：挑流不问加密，落库时记下实际是什么。** `vod_video.encrypt_type` ←
   > 实际挑中那一路的 `Encrypt`（`≠1` → `0`；`=1` 且 `EncryptType=AliyunVoDEncryption` → `2`；`=1` 其余 → `1`）。
   > 于是配加密模板 → 记 2 → Aliplayer 收 1 → 能播；配不加密 → 记 0 → 收 0 → 能播；
   > 需方改 `ALIYUN_VOD_TEMPLATE_GROUP_ID` → **下一个视频自动跟上，代码一个字不动**。
   >
   > **原文那句「不能只判 `Encrypt`，模板组误配成未加密 MP4 时会得到空集」的担心仍然成立**——
   > 它由 `Format == "m3u8"` 这一半承担：MP4 照样挑不中。变的只是不再要求加密。
...
4. **区域约束（两个独立的下拉框，选对一个不代表选对另一个）**：
   ...
   **加速区域这一项比存储区域更容易漏**，因为它不在点播的存储配置里，而在域名管理侧。就本系统而言，风险等级比"个人信息出境"低一档——**第一版根本没有水印**（F-114），分片里自然不含未成年人姓名与手机号；
   将来启用水印时原论证仍成立：跑马灯水印由前端渲染在视频层之上（PRD F2-6 规则 3），**不烧录进视频流**。
   **⚠ 但那个前提是有条件的**：`RESULT.md` 实测 iOS 全屏下 DOM 水印不成立，若将来为此改走「转码时把水印烧进画面」，
   **本条论证立刻失效** —— 烧录进去的分片就带着姓名手机号出境了。改水印方案的人必须回来重读这一段。但它仍是**教学资产出境**，且与 §7.2 第 4 条「不得开启任何境外节点」逐字冲突：写了这句话却把加速域名配成全球，是自己违反自己的合规条款。
...
5. 树深度**上限 4 级**（原为 50 级；`ancestors` 为 `VARCHAR(1000)`，雪花 ID 19 位 + 逗号 = 20 字符/级，50 级即写满 1000 字符）。建节点与移动节点时**必须在服务层校验深度**，超限返回 `400`——不校验的话超出物理上界会在写入时 `Data too long` 让整个移动事务回滚，而移动节点是要连带重算整棵子树 `ancestors` 的复合事务，失败点难以定位。

   > **F-114 定案二（需方 2026-08-22）由 50 收到 4 级**（不含平台根：`L1 机构根 → L2 普通管理员 → L3 教师 → L4 学生`，即定案里说的"5 层"含平台根）。50 是 `ancestors VARCHAR(1000)` 的**物理**上界，物理上界没变；4 是业务上界。**必须与"机构下只允许一层管理员"合起来看**：光有深度上限的话管理员仍可嵌套；光有一层管理员的话教师/学生仍可被挪到更深处。详见 03-02 §2.2。
...
每请求鉴权：取 node:anc:{myNodeId} 拆出祖先 id 列表（约 5 个）
           → 自身 + 祖先链与 frozen:{tenantId} 求交集（SISMEMBER × 6，或本地缓存的集合直接判）
           → 未命中即放行；命中后按下面【命中之后怎么判】的两段规则判定，判定为冻结则返回 10017
写侧：停用 → SADD frozen:{tenantId} {nodeId}；启用 → SREM
     移动节点 → 递归删除被移动子树的 node:anc:* 键（与 ancestors 重算同事务后执行）
```

**命中之后怎么判（两段规则，与登录侧那条 SQL 完全相同）**：

> ① **自身命中即拒**（教师/学生的"仅本人"停用由此生效）；
> ② **祖先命中且该祖先 `node_type = 1` 才拒**（管理员的分支冻结由此生效）。

**漏掉②的 `node_type = 1` 会怎样**：停用教师 T → 学员 S 的祖先链含 T → 求交命中 → **S 被拒**，
正是本节停用语义表称为**业务事故**的那件事（教师停用不得级联，PRD F1-2 亦有验收标准）。
**表现形态格外隐蔽**：登录侧那条 SQL 是对的，所以 S 被踢出去后**重新登录能进来、进来一发请求又被踢**——
死循环，两边都"按各自的规则执行"，日志里没有任何报错。

**冻结集是快速否定过滤器，不是判定本身**：未命中直接放行（绝大多数请求走这条，零查库）；
命中时才需要知道那个祖先的 `node_type`——而 `node_type` 是**不变量**（全系统没有任何接口能改它），
可按 nodeId 无限期记忆化，于是"每次命中查一次"实际是"每个被停用的节点查一次"。

**`status` 仍以冻结集为准，不回头查库**。命中后只解析 `node_type`，**绝不重新查 `status`**——
"先 SADD 再提交"的那一瞬间库里还是 `status = 0`，回头查 status 就等于把刚刚建立的抢跑保护又拆掉。
...
1. **只能授权自己拥有的资源**：授权人必须已拥有该资源（自己是 `owner_node_id`，或该资源已显式授权给自己所在节点）。~~且在有效期内~~ —— **授权已无有效期**，需方 2026-08-21 定案，见规则 7。
...
4. **查询语义**：判定"某节点能否使用某资源"只需一条 `org_resource_grant` 命中（`target_node_id = 我 AND resource_id = X AND deleted_at = 0`），**不回溯祖先链**——比继承模型更快，且不存在祖先链断裂导致的判定歧义。
5. **级联回收（必须实现）**：撤销对某节点的资源授权时，**必须级联撤销该资源在目标节点整个子树内的全部授权**。否则会出现"父级已无权、子级仍持有"的悬挂授权，逐级收缩被破坏。

   > **⚠ 级联本身不变；变的是「什么时候拦得住正在播的那个人」**（需方 2026-08-21 定案三，F-106）：
   >
   > 需方原话：「我觉得无需判授权，授权只需要最初判断一次，也就是说看这个视频的时候，但如果没权限了下一个视频就不允许看了」。
   >
   > **定案：撤销从「下一次点开视频」起生效。正在播放的那个视频可以看完。心跳不重判授权；播放凭证续签也不重判。**
   >
   > 需方的理由：公司内部学生使用，最坏是多看半节课的后半段，不值得为它在**每 10 秒一次的心跳**上加一次授权查询。
   >
   > **这是明知地推翻，不是遗漏**。被推翻的条文逐条列在 04-实施计划.md 的 F-106，包括本文档第 1 节那段「凭证过期、授权被撤销都拦不住他继续解密播放」的论证 —— 那段论证**本身仍然成立**（它说的是「不做 8.2 就连下一次都拦不住」），只是需方接受了「到下一次为止」这个窗口。
   >
   > **〔已完成：§8.1 已按本条改写、§8.2 已删除为墓碑小节，见 F-112〕** 原文：**03-03 §8.1 / §8.2 本轮不动**：那两节马上要因为加密路线从标准加密改为阿里云私有加密而整章重写，现在改一遍、下一轮再改一遍是白做。改写留给加密路线订正那一轮一次做完。
...
7. ~~**有效期不得超过上级（防时间维度悬挂）**：下级授权的 `valid_end` **自动截断为不晚于授权人自身对该资源的 `valid_end`**。若不约束，会出现"上级授权已到期、下级仍有效"的悬挂授权，而级联回收只在显式撤销时触发、管不到时间维度。授权人自身为 `owner_node_id` 时不受此限。~~
   ...
   > **本条已失去适用对象**（需方 2026-08-21 定案）：**授权一律永久有效**，原话「课程权限我不希望有时间，我觉得不需要有时间，这还不如直接收回权限」。失效手段只有两个 —— 显式撤销（03-02 接口 39 撤销资源授权（级联子树），级联整棵子树）与**学籍状态**（播放凭证校验链第一步「学生在读」）。
   >
   > `org_resource_grant.valid_start` / `valid_end` 两列**保留但永远不写**（生产表已存在，删列要迁移而收益为零：**留列删码**）；03-02 接口 40「修改授权有效期」已删除，§9.4 保留为墓碑小节。
   >
   > **正文原样保留，不是遗漏**：将来若恢复有效期（例如「试听一个月」），上面这段论证仍然成立，且它是**唯一**写清了「为什么截断必须逐资源、为什么级联回收管不到时间维度」的地方。相关的两条实施记录见 04-实施计划.md 的 F-92 与 D7。
...
### 2.8 无会话上下文的写入（事件消费、定时任务、异步 Worker）
...
1. **上下文必须由数据显式携带，不得依赖线程残留**。事件消费从 `vod_video.vod_file_id` 反查行、取其 `tenant_id`（阿里云路径下该列在**发上传凭证时**即已写入 `cloudVideoId`，故反查链路自建号起就是闭合的，不存在"事件先于写入到达"的竞态）；定时任务从任务参数或被处理行取；Worker 从 `org_import_task` / `sys_export_task` 行取。取到后用 `TenantHelper.setTenantId(...)` 显式设置，**处理完成后必须 `clear()`**——线程池会复用线程，不清理就是下一个任务串租户。
2. **跨租户的任务必须按租户分片执行**，每片开始前设上下文、结束后清理。禁止"一个事务里跨租户批量写"——那样任何一行的租户判定错误都会被整批放大。
3. **无法确定租户的写入一律拒绝并告警**，绝不"猜一个"或退化为忽略租户条件。事件消费若反查不到媒资行，记 `sys_oper_log` + `vod_callback_orphan_total` 指标并**删除该消息**——不删的话它会在不可见时长到期后重新可见，被无限重投。这是一次**静默的数据丢失**，必须有人看见。
   （原文写的是"返回 200 让云厂商停止重试"——那是 HTTP 回调时代的写法，**拉模式下根本没有"返回"这个动作**，能让消息停止重投的只有删除它，方向正好反了。落地口径见 03-课程与视频 §7.2。）
...
> 4. **攻击面**：原回调端点是全部免登录接口里**唯一一个接受任意外部 POST 并据此写库**的。去掉后白名单只剩登录、验证码、刷新令牌与解密密钥四条〔**现为三条**：解密密钥接口已随加密路线改为阿里云私有加密而删除，见 04 §E F-112；**系统从此没有任何免登录的业务接口**〕，其中前三条不写业务表、第四条是只读且被 `MtsHlsUriToken` 限定到单个学生与单个课时——**再没有一个免登录入口能写库**。
>
...
0. **学生端接口一律不加 `@SaCheckPermission`，因而不发 `perms`。** 只按角色（`sys_user.user_type`）与数据权限（子树 / 本人）判定。这些接口的归属校验本就各自完备（文件走 §7.4 的 `bizType` 校验、学习与作答走「本人」判定），再加一道 `perms` 门只是多一处会配错的地方——而配错的表现（接口 200、`perms` 空数组、学生传不了作答附件）与 §2.9 那个「系统开箱即不可用」的故障**一模一样**，排查时极易混淆。
...
**遗留例外（本规范生效前已写入分册，未擅自改写）**：`stat:board:teacher` 的第三段是角色名而非动作，不符合本格式；`system:log:login` / `system:log:oper` 的第三段实为日志种类。三者已在多处分册引用，改名需同步修订分册、菜单初始化脚本与后端注解三处，而收益只是命名整齐，故**定案为永久豁免、原样保留并在此登记**。

> **例外仅限上列三个，不得新增。** 新写 `perms` 一律照动作词表；若需要的动作词表里没有，**先扩词表再用**，不要照这三个的先例造第四个。
...
| `watermark_phone_mask` | int | 1 | 0/1 | **【F-114：第一版不被消费】水印第一版不做**，本键保留待启用。原义：水印手机号是否脱敏；1 中间四位打星（**默认**），0 不脱敏。**本键只作用于管理端预览**（`viewer_type ∈ {1,2}`）；**学生端播放一律脱敏、不受本键影响**（见 §7.2 第 2 条） |

**平台部署级参数（定案）**：云厂商选择、阿里云 AK/SK、转码模板组 ID、轻量消息队列的队列名与 Endpoint、KMS 配置——一律走**环境变量 / 配置中心，不入库**，也**不是**租户配置键。

**全平台单一账号，不支持按机构分配。** 这不是省事，是因为按机构分配的连锁反应是结构性的：
① 播放凭证签发要按租户取不同 AK（§6.4 与 03-03 §8.1 的签名链路要按租户分叉）；
② `vod_video` 要新增一列记录归属哪个账号（动基线表结构）；
③ **事件通知是按存储区域配置的、不是按租户配的**（§1 区域约束），多账号意味着多套队列与多套消费者，而 §2.8 的事件消费任务是单一入口；
④ 加速域名与 ICP 备案各自独立，每个机构都要单独走一遍周级前置流程。
这是跨越 §2.8 / §5 / §7 的改动，将来真有大客户诉求时作为独立项目评估，不在当前设计内。
...
| `/api/v1/vod/**` | 视频上传凭证、媒资、解密密钥、播放凭证、**心跳上报**、进度查询 | 03-03 |
...
watched_duration += delta → 写 Redis (key: vod:hb:{studentId}:{lessonId}) → XXL-Job 每 60s 批量落盘
vod_watch_progress。【第一版不判完播】：watch_status 只在 0/1 流转，不写 2（F-113 定案四）。
```

**`sessionPlayedSeconds` 是必填整数（F-113 新增）**：本会话累计真播秒数，由**前端计时器**给出。

> **这是对本节「签名固定」的一次明知推翻，不是补充。** 记在这里是因为契约把它称作固定签名，
> 而 F-113 定案「观看时长改由前端计算、后端照单收」之后，没有这个字段后端就无从记账。
> **C15 会核契约 §6.4 / PRD F2-7 / 03-03 §8.3 参数表三处的字段集与字段序**，改漏一处即报错。

**三条实现约束，缺一条就会算错**（详见 F-113 §2.2）：

1. **累计的是墙钟，不是媒体时间**。只在 `playing` 且页面可见时走表；`playRate` **不参与计时**——
   更容易想到的写法是监听 `timeupdate` 把 `currentTime` 增量加起来，那会让 2 倍速播 5 分钟算成 10 分钟，
   **正好推翻规则 4**。
2. **报累计值，不报增量**。重发时 `delta = 0` 天然幂等；丢包时下一次的累计值把量带回来。
   页面刷新 = 新会话，累计从 0 重起；**规则 9 会话接管时基准重置为 0**，否则新设备第一条心跳
   会因为累计值小于上一端而恒 `delta = 0`。
3. **服务端做物理上限截断 `min(delta, Δt + 5)`**：浏览器会冻结后台标签页的定时器，
   页面醒来时写得糙的计时器可能一口气报出几小时——**这是前端 bug，不是作弊**。
   **它是上界，不是第二个真相**：服务端不重算时长，只拒绝物理上不可能的增量。
   **一个数、一个封顶，不构成两个数字打架**——不要把它"补全"成第二套计时。
   截断发生时写 `vod_heartbeat_log` 留痕，**不占用 `reject_rule`**（心跳被采纳了，只是增量被削）。

**`seeked` 是必填布尔**：前端在**用户拖动进度条后**或**暂停恢复后**的第一次心跳置 `true`，其余一律 `false`。

它存在的理由是解开一处冲突：**F-113 之后拖动完全放开**（规则 3 已废止），学生从 500s 拖回 120s 或跳到 400s 后，下一次心跳的 `Δ currentTime` 必然落在推进一致性校验（03-03 §8.3.1 规则 6）的 `[0.5,1.5] × Δt × playRate` 区间之外。暂停后原地续播同理（`Δt` 大而 `Δ currentTime` 近 0）。

服务端据此处理：`seeked = true` 的心跳**重置推进校验基准**，本次不做推进判定；推进一致性只在**连续两次 `seeked = false`** 的心跳之间校验。

> **F-113 起，规则 6 的判定结果不再影响时长。** 时长由前端给出，后端一旦因规则 6 说"这条我不给你算"，
> **后端就等于又在自己算时长**——那就变回**两个数字表示同一件事**。规则 6 只写 `vod_heartbeat_log` 留痕。
> 因此本段不再出现「不计时长」这一后果，**这是刻意的，不是漏写**。

**`sessionId` 是必填字符串**：取自 8.1 播放凭证响应，标识**本次播放**（续签凭证不换会话）。服务端按 `{studentId}:{lessonId}` 维护当前活跃会话，实现**同课时会话择一**：同会话正常计时；不同会话且当前会话 60s 内活跃则返回 `20020`；超 60s 无心跳则新会话接管，并重置规则 6 的推进基准**与 `sessionPlayedSeconds` 的累计值基准（重置为 0）**——后者不重置，新设备第一条心跳会因为累计值小于上一端而恒 `delta = 0`，时长永远不涨。
...
> **被拒绝的心跳一律不推进任何基准**（时间基准、位置基准、`maxPosition`、**`sessionPlayedSeconds` 基准**）。唯一例外是会话接管：接管时必须重置位置基准**与累计值基准**，否则新设备第一条心跳就被判位置跳变、且时长恒不增长，接管等于没发生。

> **`seeked` 由客户端提供，因此必须防止它被用来永久关闭规则 6**。约束见 03-03 §8.3.1 规则 6——连续 `seeked = true` 不得超过 2 次、且单课时每小时不超过 **180** 次（**F-113 由 20 提高**：20 是在**禁快进**前提下定的，那时只能小幅回拖；放开拖动后一小时拖 20 次是完全正常的观看行为，再按旧值卡会**把被明确允许的行为持续判为异常**——与规则 9 那个「永久归零」同类），超出按异常丢弃并计入 `vod_heartbeat_log.reject_rule`，由风控报表按 `(student_id, lesson_id, 时间窗)` 聚合呈现。**F-113 起，这个上限保护的对象变了，理由要写实**：时长已经不由规则 6 决定，所以它**保护不了时长的准确性**；它保护的是**日志的诊断价值**——脚本若恒置 `true`，规则 6 永不判定，`vod_heartbeat_log` 就再也看不出这个学生有没有在真的播放，而**日志是第一版唯一的行为事实来源**。<br>**不设风控分列**：该名词若建成字段就要定义算法、衰减、阈值与清零时机，而这四项全无定义；`vod_heartbeat_log` 已是完备的原始事实（按月分区留 6 个月），任何评分口径将来都能从它算出来。这与「存分子分母、不存比率」（§4 统计表铁律）同源。真实用户 seek 后会继续播放，不存在"每 10 秒 seek 一次"的模式。
...
| `vod_callback_orphan_total` | Counter | **> 0** | 事件消费反查不到媒资行（§2.8 规则 3），每一次都是静默的数据丢失。**指标名保留 `callback` 字样**：改名会让既有告警规则与历史数据断档，代价大于命名精确性 |
...
2. **最小必要**：不收集与教学无关的信息（住址、身份证号、人脸、精确位置一律不采集）。**这一半第一版照常适用。**
   ~~跑马灯水印**对学生端播放一律展示"姓名 + 手机号中间四位打星"**（如 `李子墨 138****5678`），**不得展示完整手机号，且不受任何租户配置影响**。~~
   > **⚠ F-114（2026-08-21 需方定案）：水印第一版不做**，故本条水印部分第一版**没有适用对象**（`watermark_phone_mask` 不被消费）。
   > **规则原样保留待启用** —— 它的论证（受《个人信息保护法》第 31 条约束的是未成年人信息，「允许租户选择不脱敏」这个选项本身不成立）**没有失效**，只是暂时没有承载物。
...
>
> **`1949000000000100207` / `1949000000000100208` 是两个「读」权限标识，与 §3.1 边界 2 的关系已核对**：
> 边界 2 说「只有写操作才单独发按钮标识，同一页面内的辅助读接口随该页 `:list` 一并放行」，理由是「否则按钮表会膨胀到与接口数等长」。
> 这两行不是那种膨胀：它们的成因是**同一页面内两个接口的角色集不同**——
> `03-02 §4.1` 管理员分页列表**仅** `org_admin`，`§5.1` 教师分页列表**还包含** `teacher`，
> 而拆分前两者共用页面级 `org:staff:list`，**一个开关管两盏灯**（绑 `teacher` 则教师能列出全机构管理员，不绑则教师调不了 §5.1）。
...
| `1949000000000100207` | `1949000000000100200` | 管理员列表 | F | — | `org:admin:list` | A4 | super_admin、org_admin |
| `1949000000000100208` | `1949000000000100200` | 教师列表 | F | — | `org:teacher:list` | A4 | super_admin、org_admin、teacher |
...
| `1949000000000200101` | `1949000000000200100` | 新建课程 | F | — | `course:course:add` | A9 | **org_admin**（A 批：需方 2026-08-21 定案「三类受管资源的写权限，教师一条不留」，`teacher` 的绑定已由 `V202608210200` 撤销 —— 见表下注 ③） |
| `1949000000000200102` | `1949000000000200100` | 修改课程 | F | — | `course:course:edit` | A9 | **org_admin**（A 批：需方 2026-08-21 定案「三类受管资源的写权限，教师一条不留」，`teacher` 的绑定已由 `V202608210200` 撤销 —— 见表下注 ③） |
| `1949000000000200103` | `1949000000000200100` | 删除课程 | F | — | `course:course:remove` | A9 | **org_admin**（A 批：需方 2026-08-21 定案「三类受管资源的写权限，教师一条不留」，`teacher` 的绑定已由 `V202608210200` 撤销 —— 见表下注 ③） |
| `1949000000000200104` | `1949000000000200100` | 课程上下架 | F | — | `course:course:status` | A9 | **org_admin**（A 批：需方 2026-08-21 定案「三类受管资源的写权限，教师一条不留」，`teacher` 的绑定已由 `V202608210200` 撤销 —— 见表下注 ③） |
| `1949000000000200200` | `1949000000000200000` | 课程编排 | C | /course/courses/outline | `course:lesson:list` | A10 / B6 | org_admin、teacher |
| `1949000000000200201` | `1949000000000200200` | 新建章节 | F | — | `course:chapter:add` | A10 | **org_admin**（A 批：需方 2026-08-21 定案「三类受管资源的写权限，教师一条不留」，`teacher` 的绑定已由 `V202608210200` 撤销 —— 见表下注 ③） |
| `1949000000000200202` | `1949000000000200200` | 修改章节 | F | — | `course:chapter:edit` | A10 | **org_admin**（A 批：需方 2026-08-21 定案「三类受管资源的写权限，教师一条不留」，`teacher` 的绑定已由 `V202608210200` 撤销 —— 见表下注 ③） |
| `1949000000000200203` | `1949000000000200200` | 删除章节 | F | — | `course:chapter:remove` | A10 | **org_admin**（A 批：需方 2026-08-21 定案「三类受管资源的写权限，教师一条不留」，`teacher` 的绑定已由 `V202608210200` 撤销 —— 见表下注 ③） |
| `1949000000000200204` | `1949000000000200200` | 章节排序 | F | — | `course:chapter:sort` | A10 | **org_admin**（A 批：需方 2026-08-21 定案「三类受管资源的写权限，教师一条不留」，`teacher` 的绑定已由 `V202608210200` 撤销 —— 见表下注 ③） |
| `1949000000000200205` | `1949000000000200200` | 新建课时 | F | — | `course:lesson:add` | A10 | **org_admin**（A 批：需方 2026-08-21 定案「三类受管资源的写权限，教师一条不留」，`teacher` 的绑定已由 `V202608210200` 撤销 —— 见表下注 ③） |
| `1949000000000200206` | `1949000000000200200` | 修改课时 | F | — | `course:lesson:edit` | A10 | **org_admin**（A 批：需方 2026-08-21 定案「三类受管资源的写权限，教师一条不留」，`teacher` 的绑定已由 `V202608210200` 撤销 —— 见表下注 ③） |
| `1949000000000200207` | `1949000000000200200` | 删除课时 | F | — | `course:lesson:remove` | A10 | **org_admin**（A 批：需方 2026-08-21 定案「三类受管资源的写权限，教师一条不留」，`teacher` 的绑定已由 `V202608210200` 撤销 —— 见表下注 ③） |
| `1949000000000200208` | `1949000000000200200` | 图文资料查看 | F | — | `course:material:list` | A10 | org_admin、teacher |
| `1949000000000200209` | `1949000000000200200` | 新建图文资料 | F | — | `course:material:add` | A10 | **org_admin**（A 批：需方 2026-08-21 定案「三类受管资源的写权限，教师一条不留」，`teacher` 的绑定已由 `V202608210200` 撤销 —— 见表下注 ③） |
| `1949000000000200210` | `1949000000000200200` | 修改图文资料 | F | — | `course:material:edit` | A10 | **org_admin**（A 批：需方 2026-08-21 定案「三类受管资源的写权限，教师一条不留」，`teacher` 的绑定已由 `V202608210200` 撤销 —— 见表下注 ③） |
| `1949000000000200211` | `1949000000000200200` | 删除图文资料 | F | — | `course:material:remove` | A10 | **org_admin**（A 批：需方 2026-08-21 定案「三类受管资源的写权限，教师一条不留」，`teacher` 的绑定已由 `V202608210200` 撤销 —— 见表下注 ③） |
...
| `1949000000000200302` | `1949000000000200300` | 删除媒资 | F | — | `vod:video:remove` | A11 | **org_admin**（A 批：需方 2026-08-21 定案「三类受管资源的写权限，教师一条不留」，`teacher` 的绑定已由 `V202608210200` 撤销 —— 见表下注 ③） |
| `1949000000000200303` | `1949000000000200300` | 重新发起转码 | F | — | `vod:video:retranscode` | A11 | **org_admin**（A 批：需方 2026-08-21 定案「三类受管资源的写权限，教师一条不留」，`teacher` 的绑定已由 `V202608210200` 撤销 —— 见表下注 ③） |
| `1949000000000200304` | `1949000000000200300` | 媒资禁用/启用 | F | — | `vod:video:status` | A11 | **org_admin**（A 批：需方 2026-08-21 定案「三类受管资源的写权限，教师一条不留」，`teacher` 的绑定已由 `V202608210200` 撤销 —— 见表下注 ③） |
...
| ~~`1949000000000300103`~~ | ~~`1949000000000300100`~~ | ~~修改授权有效期~~ | ~~F~~ | — | ~~`org:grant:edit`~~ | ~~A12~~ | **整行已删除**（需方 2026-08-21 定案「没人用的权限 直接删掉」，F-104）：它的端点 03-02 接口 40 已随取消授权有效期删除，菜单行与 `org_admin` / `teacher` **两条**角色绑定由 `V202608210100__drop_grant_edit_perm.sql` 一并删掉。**行留在这里划掉而不是抹去**：附表 A 与迁移脚本同源，读者需要看得出「这个 ID 与这个 perms 标识已退役、不要重用」——`uk_perms` 一旦被重用会撞唯一键 |
...
| `1949000000000300300` | `1949000000000300000` | 资源授权健康度 | C | /grant/health | `org:grantHealth:list` | A14 | org_admin |
...
| `1949000000000400101` | `1949000000000400100` | 新建题目 | F | — | `question:question:add` | A15 | **org_admin**（A 批：需方 2026-08-21 定案「三类受管资源的写权限，教师一条不留」，`teacher` 的绑定已由 `V202608210200` 撤销 —— 见表下注 ③） |
| `1949000000000400102` | `1949000000000400100` | 修改题目 | F | — | `question:question:edit` | A15 | **org_admin**（A 批：需方 2026-08-21 定案「三类受管资源的写权限，教师一条不留」，`teacher` 的绑定已由 `V202608210200` 撤销 —— 见表下注 ③） |
| `1949000000000400103` | `1949000000000400100` | 删除题目 | F | — | `question:question:remove` | A15 | **org_admin**（A 批：需方 2026-08-21 定案「三类受管资源的写权限，教师一条不留」，`teacher` 的绑定已由 `V202608210200` 撤销 —— 见表下注 ③） |
| `1949000000000400104` | `1949000000000400100` | 启用/停用题目 | F | — | `question:question:status` | A15 | **org_admin**（A 批：需方 2026-08-21 定案「三类受管资源的写权限，教师一条不留」，`teacher` 的绑定已由 `V202608210200` 撤销 —— 见表下注 ③） |
...
| `1949000000000600100` | `1949000000000600000` | 用户管理 | C | /system/users | `system:user:list` | A21 | super_admin、org_admin |
...
| `1949000000000600700` | `1949000000000600000` | 文件管理 | C | /system/files | `system:file:query` | A22 | super_admin、org_admin |
| `1949000000000600701` | `1949000000000600700` | 上传文件 | F | — | `system:file:upload` | A22 | super_admin、org_admin、teacher |
| `1949000000000600702` | `1949000000000600700` | 下载文件 | F | — | `system:file:download` | A22 | super_admin、org_admin、teacher |
...
> **注 ③ —— A 批撤销教师的受管资源写权限（需方 2026-08-21 定案，`V202608210200__revoke_teacher_resource_write_perms.sql`）**
>
> 需方口径：**资源由管理员生产，教师只教学与管理。** 上表 21 行的「允许角色」栏因此由 `org_admin、teacher` 收窄为 `org_admin`：
> 课程 4（`add` / `edit` / `remove` / `status`）、章节 4（`add` / `edit` / `remove` / `sort`）、课时 3、图文资料 3、题目 4、媒资 3。
>
> **与 `V202608210000`（上传凭证收窄，F-105）同源**：权限的真相在 `sys_role_menu` → `sys_menu.perms`，
> **改初始化数据，不在代码里加角色门** —— 角色门会是全库唯一一处不同构的判定。
>
> **菜单行一行都没删**（与 F-104 的 `org:grant:edit` 不同）：那次删的是**孤儿**权限，端点已经没了；
> 本次端点全都还在，只是教师不该有 —— 按钮本身没错，错的是绑给了谁。`org_admin` 对这 21 个按钮的绑定原样保留。
>
> **撤的只有写，`*:list` 一条没动**：`course:course:list` / `course:lesson:list` / `course:material:list` /
> `question:question:list` / `vod:video:list` 全部保留 —— 教师仍要看得见课程、题目、媒资，
> 才能选来授权给名下学员、才能组卷布置作业。`homework:*` 与 `org:grant:grant` / `org:grant:revoke` 同样一条没动
>（需方明确「作业没关系」；把管理员建好的资源授权给名下学员是**教学动作**，不是内容生产）。
>
> **计数**：`sys_role_menu` 200 → **179**；`teacher` 的 `perms` 58 → **37**。`sys_menu` 125 与 `DISTINCT perms` 118 **不变**。

**`student` 角色不绑定任何菜单行（F-1② 定案）。** 学生端接口一律**不加 `@SaCheckPermission`**，只按角色（`sys_user.user_type`）与数据权限（子树 / 本人）判定。
理由：这些接口的归属校验本就各自完备——文件走 00-通用约定 §7.4 的 `bizType` 归属校验（且只在下载接口实现一次），学习与作答走「本人」判定。再加一道 `perms` 门只是多一处会配错的地方，而配错的表现（接口返回 200、`perms` 数组为空、学生传不了作答附件）与 F-1 原本那个故障**一模一样**，排查时极易混淆。
学生端接口在各分册中**只声明角色、不声明权限标识**（03-02 接口 26 学生异动轨迹、03-03 §6 与 §8、03-04 §4 与 §6、03-05 §4.5），本表据此不为其发 `perms`。
