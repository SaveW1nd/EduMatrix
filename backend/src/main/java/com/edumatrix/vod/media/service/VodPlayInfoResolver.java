...
public class VodPlayInfoResolver {
    ...
    public Picked pick(VodEvent event) {
        ...
        if (candidates.size() > 1) {
            ...
            //
            // 【⚠ F-114 明确保留这道守卫，它不是漏改】
            //
            // 【订正一处不准确的说法】原注释写「两个模板组并存时 GetPlayInfo 会返回两路」——
            // 那不对。模板组是【按视频】选的：每个视频在 CreateUploadVideo 时只带一个组，
            // 所以「系统里有两个可选模板组」本身【不会】让任何一个视频出现两路。
            //
            // 真正会返回两路的是【同一个视频被两个模板组各转过一次】，当前有一条可达路径：
            //   接口 33「重新发起转码」用的是【当前配置】的 templateGroupId（VodClient:184）。
            //   切换 ALIYUN_VOD_TEMPLATE_GROUP_ID 之后对老视频重转 → 旧组的流还在、
            //   新组的流叠上来 → 两路 → 本条失败。
            // 【那个失败是对的】（任选一路会让清晰度与加密状态都不可控），
            // 但【它报出来的原因会误导人】：写的是「模板组配了多档」，而真实原因是
            // 「这个视频被两个不同的模板组各转过一次」。已登记进 F-114，本轮不改文案 ——
            // 改它要先定「重转时该用哪个组、要不要先清掉旧产物」，那是需方定案。
